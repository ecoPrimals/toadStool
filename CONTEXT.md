...
- **Wave 8 Compute Trio** (S235–S263): `compute.dispatch.submit` trio-standard IPC contract. Phase A–D complete. **111 JSON-RPC methods** (direct).
...
- **Deep Debt** (S240–S288): All Duration literals → named constants. Zero production mocks/TODO/FIXME/unreachable!(). All unsafe SAFETY-documented (confirmed S288). All `#[allow]`/`#[expect]` have `reason`. `cargo deny check bans` clean. Stale socket hygiene (S264). sporePrint Wave 28 (S265). Sandbox `working_dir` production + `data_dependencies` validation (S266). Kernel health preflight (S268). Sovereign driver rotation (S267). Production panic surface eliminated (S273–S288). **S286–S287** — `dispatch.verify_trust`, `dispatch.telemetry.schema`, `DispatchTrustLevel`, `GateOwnership`, yield-to-owner audit. **S288** — Akida MMIO `try_*` paths, BearDog alias removal, `modbus` feature-gated. **Jun 2026 S284** — last 3 production files >800L split; 0 production files >800L. **S285** — server crypto → `crypto_integration`, typed stub errors (`NoProviderRegistered`, `NoEngineRegistered`), `embedded-placeholder-impls` opt-in, `PRIMAL_NAME` naming.
...
- **Tests**: 23,000+ (9,069+ lib-only, 0 failures, unlimited parallelism)
