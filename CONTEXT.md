...
- **S244 Deep Debt Sweep**: Benchmark `println!`→`tracing` (cross-substrate-validation). 15+ hardcoded `Duration` literals→named constants across server/distributed/integration crates. `GlowPlugClient` async test coverage. Clippy fixes (`bool_to_int_with_if`, `unchecked_time_subtraction`). 8,289 tests
- **S243 Vestigial Cleanup**: Legacy `swap_device()` removed from `GlowPlugClient` (zero callers). `compute.dispatch.capabilities` enhanced with `render_node`/`device_id` for DRM GPUs. Phase C coral-driver recon complete
...
- **Tests**: 22,843+ (8,289+ lib-only, 0 failures, unlimited parallelism)
