...
- **Tests**: 20,000+ (7,842 lib-only S211, 0 failures, unlimited parallelism)
- **Unsafe**: 49 blocks (all in hw-safe/GPU/VFIO/display/plugin containment, all SAFETY-documented); workspace `unsafe_code = "deny"`, 41 crates `forbid` + 5 hw crates with narrow `#[allow(unsafe_code, reason)]`; all production `#[expect]` have `reason =` (S211)
