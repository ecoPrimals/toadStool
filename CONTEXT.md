...
- **Tests**: 20,000+ (7,789 lib-only S177, 0 failures, unlimited parallelism)
- **Unsafe**: 46 blocks (all in hw-safe/GPU/VFIO/display containment); workspace `unsafe_code = "deny"`, 41 crates `forbid` + 5 hw crates with narrow `#[allow(unsafe_code, reason)]`
