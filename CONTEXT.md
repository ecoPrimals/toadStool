...
- **Deep Debt** (S240–S308): All Duration literals → named constants. Zero production mocks/TODO/FIXME/unreachable!(). All unsafe SAFETY-documented. All `#[allow]`/`#[expect]` have `reason`; zero production `#[allow]` (S291). `cargo deny check bans` clean. Production panic surface eliminated (S273–S293). Zero production files >750L (S307, tightened from 800L). S305 — sync ctor migration (auth/agents fully async). S306 — `ServiceMeshType`/`bar_cartography`/`amd/ioctl` splits. S307 — `registers`/`pm4`/`swap` splits + 25 stale tests removed. ~20 deprecated symbols removed (S304–S306).
