...
- **Deep Debt** (S240–S336+): All Duration literals → named constants (S321). Zero production mocks/TODO/FIXME/unreachable!(). All unsafe SAFETY-documented. All `#[allow]`/`#[expect]` have `reason`; zero production `#[allow]` (S291). `cargo deny check bans` clean. Production panic surface eliminated (S273–S293). Zero production files >750L (S307–S334: 14+ large files refactored via smart splits and test extraction). 100% SPDX AGPL-3.0-or-later headers (S320+). Zero hardcoded cross-primal names in production logic (S320+S333: BTSP relay "BearDog" → "crypto provider"). Dispatch hot-path zero-copy: `Arc<EncryptionKey>` cache, pipeline borrow, borrowed `Deserialize` (S331). S329 — cross-architecture `cfg`-gating (134 files). S331 — borrowed deserialization sweep (6 files, eliminate `from_value(v.clone())`). S332 — Phase 2 Silicon Atheism: `WgpuGpuDiscovery`, `PortableSwapExecutor`, `PortableResourceHandle`. S333 — structural debt: 7 large files refactored via test extraction (−2,188 production lines), hardcoded primal names → capability terms in BTSP relay. S334 — test extraction waves 2–3: 11 files extracted (−1,888 lines), 750L gate cleared, dispatch clone polish. S335 — doc-comment primal name sweep (22 replacements, 9 files), test extraction waves 4-5 (10 files, −1,103L), dead code elimination (DispatchJob.id removed). S336 — security_impl migrated to crypto_integration (#![expect(deprecated)] removed), dead channels feature removed (12 cfg blocks), test extraction wave 6 (2 files, -173L).
...
- **Tests**: 23,000+ (9,232+ lib-only, 0 failures, unlimited parallelism)
