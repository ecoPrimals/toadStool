...
- **Wave 8 Compute Trio** (S235–S249): `compute.dispatch.submit` trio-standard IPC contract. **Phase A: coral-ember absorbed** (S237). **Phase B: glowplug absorbed** (S239). **Phase C Batch 1–4: toadstool-cylinder** (S245–S248) — DRM, MMIO, AMD, NVIDIA, and VFIO hardware modules absorbed from `coral-driver`. 415 cylinder tests. gsp dependency resolved locally. Phase D (wiring dispatch through absorbed layer) pending.
- **Deep Debt** (S240–S249): All Duration literals extracted to named constants (~100+ across 30+ files). Zero library `println!`/`eprintln!`. All production files <800 lines. Zero production mocks/TODO/FIXME/unreachable!(). All unsafe SAFETY-documented. Deprecated template suppressions cleaned.
- **Tests**: 22,843+ (8,704+ lib-only, 0 failures, unlimited parallelism)
