...
- **Deep Debt** (S240–S339): All Duration literals → named constants (S321). Zero production mocks/TODO/FIXME/unreachable!(). All unsafe SAFETY-documented. All `#[allow]`/`#[expect]` have `reason`; zero production `#[allow]` (S291). `cargo deny check bans` clean. Production panic surface eliminated (S273–S293). Zero production files >750L (S307–S338: largest 713L; 14+ large files refactored via smart splits and test extraction). 100% SPDX AGPL-3.0-or-later headers (S320+). Zero hardcoded cross-primal names in production logic (S320+S333: BTSP relay "BearDog" → "crypto provider"). Dispatch hot-path zero-copy: `Arc<EncryptionKey>` cache, pipeline borrow, borrowed `Deserialize` (S331), `Cow<str>` dispatch mode (S337). S329 — cross-architecture `cfg`-gating (134 files). S337 — hot-path `Cow<str>` + structural splits (warm.rs, operations.rs). S338 — structural splits (rm_object_tree, pmu_investigate, opcodes). S339 — Rust 1.96 clippy sweep (251 files, MSRV-safe lint resolution, dead feature removal).
...
- **Tests**: 23,000+ (9,252+ lib-only, 0 failures, unlimited parallelism)
