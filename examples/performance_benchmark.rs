...
use toadstool::{init, ResourceRequirements, ToadStoolResult};
...
async fn benchmark_native_execution(
    ...
    metrics.insert("avg_exec_time_ms".to_string(), total_exec_time.as_millis() as f64 / successful as f64);
...
async fn benchmark_concurrent_jobs(
    ...
    for i in 0..config.iterations {
        ...
        let handle = tokio::spawn(async move {
            ...
            let context = create_benchmark_context(&format!("concurrent_{}", i), SecurityLevel::Standard);
    ...
    let mut metrics = HashMap::new();
    metrics.insert("concurrency_level".to_string(), config.concurrent_jobs as f64);
    metrics.insert("throughput_improvement".to_string(), ops_per_second / config.concurrent_jobs as f64);
...
async fn benchmark_primal_discovery(
    ...
    for i in 0..config.iterations {
        ...
        match platform.find_primals_by_capability(capability).await {
            Ok(providers) => {
                successful += 1;
    ...
    let mut metrics = HashMap::new();
    metrics.insert("avg_providers_found".to_string(), total_providers as f64 / successful as f64);
    metrics.insert("discovery_latency_ms".to_string(), avg_duration.as_millis() as f64);
...
async fn benchmark_resource_allocation(
    ...
    for i in 0..config.iterations {
        ...
        resources.cpu_cores = Some((i % 4 + 1) as f64);
        resources.memory_mb = Some((i % 8 + 1) * 256);
    ...
    let mut metrics = HashMap::new();
    metrics.insert("avg_allocation_time_ms".to_string(), total_allocation_time.as_millis() as f64 / successful as f64);
...
async fn benchmark_security_overhead(
    ...
    for i in 0..config.iterations {
        ...
        match platform.execute_universal_job(job).await {
            Ok(_) => {
                ...
                *level_times.entry(format!("{:?}", level)).or_insert(Duration::new(0, 0)) += level_duration;
            }
            Err(_) => {}
        }
    ...
    for (level, duration) in level_times {
        metrics.insert(format!("{}_avg_ms", level), duration.as_millis() as f64 / (config.iterations / 4) as f64);
    }
...
async fn benchmark_job_types(
    ...
    for (job_type, count) in type_counts {
        metrics.insert(format!("{}_success_rate", job_type), count as f64 / (config.iterations / 4) as f64);
    }
