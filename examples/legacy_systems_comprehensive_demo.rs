...
async fn create_legacy_runtime() -> Result<LegacyRuntimeEngine, Box<dyn std::error::Error>> {
    ...
    config.mainframe_configs.insert(
        "ibm-mainframe".to_string(),
        MainframeConfig {
    ...
    config.embedded_configs.insert(
        "6502-system".to_string(),
        EmbeddedConfig {
    ...
    config.industrial_configs.insert(
        "plc-system".to_string(),
        IndustrialConfig {
    ...
    config.realtime_configs.insert(
        "vxworks-system".to_string(),
        RealtimeConfig {
            ...
            interrupts: vec![InterruptConfig {
                interrupt_number: 0,
                priority: 200,
    ...
    // Configure emulation
    config.emulation_configs.insert(
...
async fn demonstrate_mainframe_systems(
    runtime: &LegacyRuntimeEngine,
    ...
    let connectivity = runtime
        .test_connectivity(LegacySystemType::IBM_zSeries)
        .await?;
...
async fn demonstrate_embedded_systems(
    runtime: &LegacyRuntimeEngine,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test Z80 system
    let z80_connectivity = runtime
        .test_connectivity(LegacySystemType::Zilog_Z80)
        .await?;
...
async fn demonstrate_industrial_systems(
    runtime: &LegacyRuntimeEngine,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test SCADA system
    let scada_connectivity = runtime
        .test_connectivity(LegacySystemType::SCADA_System)
        .await?;
    ...
    info!(
        "🏗️  Supported industrial systems: {:?}",
        industrial_systems
            .iter()
            .filter(|s| matches!(
                s,
...
async fn demonstrate_realtime_systems(
    runtime: &LegacyRuntimeEngine,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test QNX system
    let qnx_connectivity = runtime
        .test_connectivity(LegacySystemType::QNX_Legacy)
        .await?;
...
async fn demonstrate_cross_compilation(
    runtime: &LegacyRuntimeEngine,
) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demonstrate_system_emulation(
    runtime: &LegacyRuntimeEngine,
) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demonstrate_legacy_networking(
    runtime: &LegacyRuntimeEngine,
) -> Result<(), Box<dyn std::error::Error>> {
...
async fn show_runtime_metrics(
    ...
    let supported_systems = runtime.get_supported_systems();
    info!(
        "🎯 Total supported legacy systems: {}",
        supported_systems.len()
    );
...
fn create_mainframe_runtime_requirements() -> LegacyRuntimeRequirements {
    LegacyRuntimeRequirements {
        ...
        communication: CommunicationRequirements {
            protocols: vec![CommunicationProtocol::IBM3270, CommunicationProtocol::SNA],
...
fn create_embedded_runtime_requirements() -> LegacyRuntimeRequirements {
    LegacyRuntimeRequirements {
        ...
        communication: CommunicationRequirements {
            protocols: vec![CommunicationProtocol::Serial {
                baud_rate: 9600,
                data_bits: 8,
                stop_bits: 1,
                parity: Parity::None,
...
fn create_industrial_runtime_requirements() -> LegacyRuntimeRequirements {
    LegacyRuntimeRequirements {
        ...
        storage: StorageRequirements {
            ...
            file_system: FileSystemType::Custom {
                name: "PLC_FS".to_string(),
            },
        },
        communication: CommunicationRequirements {
            protocols: vec![
...
fn create_network_runtime_requirements() -> LegacyRuntimeRequirements {
    LegacyRuntimeRequirements {
        ...
        communication: CommunicationRequirements {
            ...
            network: NetworkRequirements {
                protocols: vec![NetworkProtocol::NetBIOS, NetworkProtocol::IPXSPX],
                bandwidth: Some(10 * 1024 * 1024), // 10 Mbps
        ...
        special_hardware: vec![SpecialHardware::SerialPort, SpecialHardware::Modem],
    }
}
