    ...
    vec![
        ...
        ComputeNode {
            ...
            capabilities: NodeCapabilities {
                ...
                cuda: true,
                oneapi: false,
                llama_cpp_backends: vec!["cpu".to_string(), "vulkan".to_string(), "cuda".to_string()],
        ...
        ComputeNode {
            ...
            capabilities: NodeCapabilities {
                ...
                cuda: false, // No CUDA!
                oneapi: false,
                llama_cpp_backends: vec!["cpu".to_string(), "vulkan".to_string(), "rocm".to_string()],
...
async fn print_ecosystem_overview(ecosystem: &[ComputeNode]) {
    ...
    for node in ecosystem {
        ...
        let open_backends: Vec<String> = vec![
            if node.capabilities.webgpu { Some("WebGPU") } else { None },
            if node.capabilities.vulkan { Some("Vulkan") } else { None },
            if node.capabilities.rocm { Some("ROCm") } else { None },
            if node.capabilities.metal { Some("Metal") } else { None },
        ].into_iter().flatten().map(|s| s.to_string()).collect();
        
        let proprietary: Vec<String> = vec![
            if node.capabilities.cuda { Some("CUDA") } else { None },
            if node.capabilities.oneapi { Some("OneAPI") } else { None },
        ].into_iter().flatten().map(|s| s.to_string()).collect();
        
        println!("    Open: {}", if open_backends.is_empty() { "None".to_string() } else { open_backends.join(", ") });
        println!("    Proprietary: {}", if proprietary.is_empty() { "None".to_string() } else { proprietary.join(", ") });
        println!();
    }
}

async fn demonstrate_ai_workloads(ecosystem: &[ComputeNode]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let llama_request = WorkloadRequest {
        ...
        preferred_frameworks: vec!["vulkan".to_string(), "rocm".to_string(), "metal".to_string()],
    };
...
async fn demonstrate_general_compute(ecosystem: &[ComputeNode]) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demonstrate_cuda_isolation(ecosystem: &[ComputeNode]) -> Result<(), Box<dyn std::error::Error>> {
...
async fn schedule_workload(request: &WorkloadRequest, ecosystem: &[ComputeNode]) -> SchedulingDecision {
...
async fn calculate_node_score(request: &WorkloadRequest, node: &ComputeNode) -> f64 {
    ...
    if node.capabilities.webgpu { score += 500.0; } // WebGPU gets highest bonus - universal
    if node.capabilities.vulkan { score += 400.0; } // Vulkan - industry standard
    if node.capabilities.rocm { score += 450.0; }   // ROCm gets extra for being CUDA alternative
    if node.capabilities.opencl { score += 300.0; } // OpenCL - mature but older
    if node.capabilities.metal { score += 350.0; }  // Metal - Apple ecosystem
    
    // Community framework bonuses (champion open AI)
    if node.capabilities.llama_cpp_backends.iter().any(|b| b != "cuda") {
    ...
    if node.capabilities.candle_support { score += 400.0; } // Rust ML bonus
    if node.capabilities.burn_support { score += 350.0; }
    
    // Cross-platform bonuses (universal compatibility)
    if node.capabilities.wasm_support { score += 200.0; }
    if node.capabilities.edge_optimized { score += 150.0; }
...
fn generate_reasoning(request: &WorkloadRequest, node: &ComputeNode, strategy: &ExecutionStrategy) -> String {
    match strategy {
        ExecutionStrategy::WebGPU => 
            format!("Selected WebGPU on {} - universal standard, works everywhere", node.id),
        ExecutionStrategy::Vulkan => 
            format!("Selected Vulkan on {} - open standard, high performance", node.id),
        ExecutionStrategy::ROCm => 
            format!("Selected ROCm on {} - open alternative to CUDA, AMD ecosystem", node.id),
        ExecutionStrategy::LlamaCppVulkan => 
            format!("Selected llama.cpp with Vulkan on {} - open source AI + open GPU standard", node.id),
        ExecutionStrategy::LlamaCppMetal => 
            format!("Selected llama.cpp with Metal on {} - optimized for Apple Silicon", node.id),
        ExecutionStrategy::LlamaCppROCm => 
            format!("Selected llama.cpp with ROCm on {} - open source stack end-to-end", node.id),
        ExecutionStrategy::ONNXVulkan => 
            format!("Selected ONNX with Vulkan on {} - cross-platform AI standard + open GPU", node.id),
        ExecutionStrategy::ONNXROCm => 
            format!("Selected ONNX with ROCm on {} - industry standard AI + open compute", node.id),
        ExecutionStrategy::CandleWGPU => 
            format!("Selected Candle with WGPU on {} - pure Rust ML stack", node.id),
        ExecutionStrategy::OptimizedCPU => 
            format!("Selected optimized CPU on {} - universal compatibility", node.id),
...
async fn show_strategic_metrics(ecosystem: &[ComputeNode]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cuda_only_nodes = ecosystem.iter().filter(|n| n.capabilities.cuda && !has_open_alternatives(n)).count();
    let mixed_nodes = ecosystem.iter().filter(|n| n.capabilities.cuda && has_open_alternatives(n)).count();
    
    println!("Node Distribution:");
    println!("  WebGPU capable: {}/{} ({:.1}%)", webgpu_nodes, total_nodes, webgpu_nodes as f64 / total_nodes as f64 * 100.0);
    println!("  Vulkan capable: {}/{} ({:.1}%)", vulkan_nodes, total_nodes, vulkan_nodes as f64 / total_nodes as f64 * 100.0);
    println!("  ROCm capable: {}/{} ({:.1}%)", rocm_nodes, total_nodes, rocm_nodes as f64 / total_nodes as f64 * 100.0);
    println!("  CUDA-only: {}/{} ({:.1}%)", cuda_only_nodes, total_nodes, cuda_only_nodes as f64 / total_nodes as f64 * 100.0);
    println!("  Mixed (CUDA + Open): {}/{} ({:.1}%)", mixed_nodes, total_nodes, mixed_nodes as f64 / total_nodes as f64 * 100.0);
    
    println!("\nFramework Support:");
    let llama_cpp_nodes = ecosystem.iter().filter(|n| !n.capabilities.llama_cpp_backends.is_empty()).count();
    let onnx_nodes = ecosystem.iter().filter(|n| !n.capabilities.onnx_providers.is_empty()).count();
    let rust_ml_nodes = ecosystem.iter().filter(|n| n.capabilities.candle_support || n.capabilities.burn_support).count();
    
    println!("  llama.cpp: {}/{} ({:.1}%)", llama_cpp_nodes, total_nodes, llama_cpp_nodes as f64 / total_nodes as f64 * 100.0);
    println!("  ONNX: {}/{} ({:.1}%)", onnx_nodes, total_nodes, onnx_nodes as f64 / total_nodes as f64 * 100.0);
    println!("  Rust ML: {}/{} ({:.1}%)", rust_ml_nodes, total_nodes, rust_ml_nodes as f64 / total_nodes as f64 * 100.0);
    
    println!("\n🎯 Strategic Goals Achievement:");
    let open_coverage = (webgpu_nodes + vulkan_nodes + rocm_nodes) as f64 / (total_nodes * 3) as f64 * 100.0;
    let cuda_isolation = cuda_only_nodes as f64 / total_nodes as f64 * 100.0;
    let cross_platform = ecosystem.iter().filter(|n| n.capabilities.wasm_support).count() as f64 / total_nodes as f64 * 100.0;
    
    println!("  Open Standards Coverage: {:.1}%", open_coverage);
    println!("  CUDA Dependency: {:.1}% (lower is better)", cuda_isolation);
    println!("  Cross-Platform Ready: {:.1}%", cross_platform);
    
    println!("\n💡 The NVIDIA Incentive:");
    println!("  Current reality: {} nodes have CUDA but also open alternatives", mixed_nodes);
    println!("  Missed opportunity: {} CUDA-only nodes could join universal network with open drivers", cuda_only_nodes);
