...
use toadstool_runtime_edge::{
    ...
    platforms::{
        ArduinoBoard, ArduinoDevice, ESP32Device, ESP32Variant, ESP32Framework,
        EdgePlatform, EdgeDevice, DeviceStatus, ConnectionInfo, ConnectionType,
...
async fn demo_device_discovery(edge_runtime: &EdgeRuntimeEngine) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demo_arduino_platform(edge_runtime: &EdgeRuntimeEngine) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        ...
        let light = 512 + (i * 100);
        info!("  📈 Reading {}: Temperature: {:.1}°C, Light: {}", i+1, temp, light);
    }
...
async fn demo_esp32_platform(edge_runtime: &EdgeRuntimeEngine) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        ...
        info!("  🌡️ WiFi Sensor Data {}: Temperature: {:.1}°C, Humidity: {:.1}%", i+1, temp, humidity);
    }
...
async fn demo_cross_compilation(edge_config: &EdgeRuntimeConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (platform, name) in targets {
        ...
        match toolchain.cross_compile(sample_c_code.as_bytes(), &platform).await {
            Ok(binary) => {
                info!("  ✅ Successfully compiled {} bytes for {}", binary.len(), name);
            }
            Err(e) => {
...
async fn demo_multi_device_orchestration(edge_runtime: &EdgeRuntimeEngine) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demo_edge_computing_scenarios(edge_runtime: &EdgeRuntimeEngine) -> Result<(), Box<dyn std::error::Error>> {
...
async fn demo_performance_monitoring(edge_runtime: &EdgeRuntimeEngine) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10 {
        ...
        info!("  📊 Sample {}: CPU: {:.1}%, Memory: {:.1}MB, Network: {:.1}KB/s", 
              i+1, cpu_usage, memory_usage, network_usage);
    }
