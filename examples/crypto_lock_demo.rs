...
async fn main() -> ToadStoolResult<()> {
    ...
    let azure_target = ExternalTarget::CloudProvider {
        ...
        services: vec!["compute".to_string(), "storage".to_string(), "functions".to_string()],
    };
    ...
    crypto_lock.install_crypto_permission(aws_permission).await?;
    ...
    crypto_lock.install_crypto_permission(university_permission).await?;
    ...
    let delegation_request = crypto_lock.request_delegation(
        &from_holder,
        &to_holder,
        &aws_target,
        delegation_scope,
    ...
    println!("📋 Delegation request created: {:?}", delegation_request.request_id);
    ...
    println!("  - External permissions: {}", status.external_permissions.len());
    println!("  - Delegation chains: {}", status.delegation_chains.len());
    println!("  - Expiring permissions: {}", status.expiring_permissions.len());
