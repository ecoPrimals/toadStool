...
async fn print_ecosystem_overview(ecosystem: &[ComputeNode]) {
    ...
    for node in ecosystem {
        println!("📱 {} ({}) - Score: {:.0}", node.id, node.node_type, node.performance_score);
        ...
        let open_backends: Vec<String> = vec![
            ...
            if node.capabilities.opencl { Some("OpenCL") } else { None },
        ...
        println!("   🌟 Open Standards: {}", 
                if open_backends.is_empty() { "❌ None".to_string() } 
                else { format!("✅ {}", open_backends.join(", ")) });
        println!("   🔒 Proprietary: {}", 
                if proprietary.is_empty() { "✅ None".to_string() } 
                else { format!("⚠️ {}", proprietary.join(", ")) });
        
        // Show AI framework support
        let ai_frameworks: Vec<String> = vec![
            if !node.capabilities.llama_cpp_backends.is_empty() { Some("llama.cpp") } else { None },
            if !node.capabilities.onnx_providers.is_empty() { Some("ONNX") } else { None },
            if node.capabilities.candle_support { Some("Candle") } else { None },
            if node.capabilities.burn_support { Some("Burn") } else { None },
        ].into_iter().flatten().map(|s| s.to_string()).collect();
        
        println!("   🤖 AI Frameworks: {}", 
                if ai_frameworks.is_empty() { "None".to_string() } 
                else { ai_frameworks.join(", ") });
        println!();
    }
}
...
async fn calculate_node_score(request: &WorkloadRequest, node: &ComputeNode) -> (f64, f64) {
    ...
    let non_cuda_llama_backends = node.capabilities.llama_cpp_backends.iter()
        .filter(|b| *b != "cuda").count();
    ...
    let non_cuda_onnx_providers = node.capabilities.onnx_providers.iter()
        .filter(|p| *p != "cuda").count();
    ...
    match &request.workload_type {
        ...
        WorkloadType::ScientificComputing { .. } => {
            if request.requires_proprietary && node.capabilities.cuda {
                ExecutionStrategy::CudaIsolated { 
                    warning: "🔒 Legacy application locked to CUDA - consider open alternative migration".to_string() 
                }
            } else if node.capabilities.vulkan {
                ExecutionStrategy::Vulkan
            } else if node.capabilities.rocm {
...
fn generate_reasoning(request: &WorkloadRequest, node: &ComputeNode, strategy: &ExecutionStrategy) -> String {
    match strategy {
        ...
        ExecutionStrategy::ROCm => 
            format!("⭐ ROCm on {} - Open alternative breaking CUDA dependency", node.id),
        ExecutionStrategy::LlamaCppVulkan => 
            format!("🌟 llama.cpp + Vulkan on {} - Open AI + open GPU = freedom", node.id),
        ExecutionStrategy::LlamaCppMetal => 
            format!("🍎 llama.cpp + Metal on {} - Optimized for Apple Silicon", node.id),
        ExecutionStrategy::LlamaCppROCm => 
            format!("🔥 llama.cpp + ROCm on {} - Pure open source AI stack", node.id),
        ExecutionStrategy::ONNXVulkan => 
            format!("✅ ONNX + Vulkan on {} - Cross-platform AI standard + open GPU", node.id),
        ExecutionStrategy::ONNXROCm => 
            format!("⭐ ONNX + ROCm on {} - Open AI standard + CUDA alternative", node.id),
        ExecutionStrategy::CandleWGPU => 
            format!("🦀 Candle + WebGPU on {} - Pure Rust ML with universal GPU", node.id),
        ExecutionStrategy::BurnWGPU => 
            format!("🔥 Burn + WebGPU on {} - Next-gen Rust ML framework", node.id),
        ExecutionStrategy::OptimizedCPU => 
...
async fn show_strategic_metrics(ecosystem: &[ComputeNode]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pure_open_nodes = ecosystem.iter().filter(|n| !n.capabilities.cuda && has_open_alternatives(n)).count();
    
    println!("🌐 Node Distribution:");
    println!("   WebGPU capable: {}/{} ({:.1}%)", webgpu_nodes, total_nodes, 
             webgpu_nodes as f64 / total_nodes as f64 * 100.0);
    println!("   Vulkan capable: {}/{} ({:.1}%)", vulkan_nodes, total_nodes, 
             vulkan_nodes as f64 / total_nodes as f64 * 100.0);
    println!("   ROCm capable: {}/{} ({:.1}%)", rocm_nodes, total_nodes, 
             rocm_nodes as f64 / total_nodes as f64 * 100.0);
    println!("   Pure open source: {}/{} ({:.1}%)", pure_open_nodes, total_nodes, 
             pure_open_nodes as f64 / total_nodes as f64 * 100.0);
    println!("   Mixed (CUDA + Open): {}/{} ({:.1}%)", mixed_nodes, total_nodes, 
             mixed_nodes as f64 / total_nodes as f64 * 100.0);
    println!("   CUDA-only (problematic): {}/{} ({:.1}%)", cuda_only_nodes, total_nodes, 
    ...
    println!("   llama.cpp support: {}/{} ({:.1}%)", llama_cpp_nodes, total_nodes, 
             llama_cpp_nodes as f64 / total_nodes as f64 * 100.0);
    println!("   ONNX support: {}/{} ({:.1}%)", onnx_nodes, total_nodes, 
             onnx_nodes as f64 / total_nodes as f64 * 100.0);
    println!("   Rust ML frameworks: {}/{} ({:.1}%)", rust_ml_nodes, total_nodes, 
    ...
    println!("   ⚠️ CUDA Dependency: {:.1}% (goal: minimize)", cuda_dependency);
    println!("   🌐 Cross-Platform Ready: {:.1}%", cross_platform);
    
    let ecosystem_health = if cuda_only_nodes == 0 { "🌟 Excellent" } 
                          else if cuda_only_nodes == 1 { "✅ Good" } 
                          else { "⚠️ Needs Work" };
    ...
    println!("     • {} NVIDIA nodes have both CUDA and open alternatives", mixed_nodes);
    println!("     • {} nodes are trapped in CUDA-only ecosystem", cuda_only_nodes);
    ...
    println!("   Current addressable nodes: {}", pure_open_nodes + mixed_nodes);
    println!("   Potential with open NVIDIA drivers: {}", total_nodes);
    println!("   Estimated gaming PC unlock: {:.0}k+ nodes", market_expansion / 1000.0);
    println!("   🎯 Network effect: More nodes → better load balancing → happier users → more adoption");
