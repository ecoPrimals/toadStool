...
mod tests {
    ...
    #[tokio::test]
    async fn test_file_policy_manager() {
        // Use a temporary directory for testing instead of system directory
        let temp_dir = std::env::temp_dir().join("toadstool_test_policies");
        let config = PolicyManagerConfig {
            policy_dir: temp_dir.to_path_buf(),
            cache_enabled: true,
            cache_ttl_hours: 24,
            strict_enforcement: true,
            default_violation_action: ViolationAction::Terminate,
