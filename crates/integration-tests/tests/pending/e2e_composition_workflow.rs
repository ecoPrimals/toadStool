...
use toadstool::composition_engine::{CompositionEngine, CompositionRequest};
...
async fn test_composition_engine_initialization() {
    ...
    assert!(result.is_ok(), "Composition engine should initialize from runtime");
}
...
async fn test_simple_composition_request() {
    ...
    assert!(evaluation.is_ok(), "Simple request should evaluate successfully");
    
    let eval = evaluation.unwrap();
    assert!(eval.is_feasible, "Simple request with no constraints should be feasible");
}
...
async fn test_composition_with_gpu_constraint() {
    ...
    let request = CompositionRequest::new("gpu_workload")
        .with_constraint(Constraint::requires_gpu());
    
    let evaluation = engine.evaluate(&request).await;
    
    assert!(evaluation.is_ok(), "GPU constraint evaluation should not panic");
    
    // Result depends on whether GPU is available, both valid
    let eval = evaluation.unwrap();
    assert!(eval.results.contains_key("requires_gpu"), "Should evaluate GPU constraint");
}
...
async fn test_composition_with_memory_constraint() {
    ...
    let request = CompositionRequest::new("memory_workload")
        .with_constraint(Constraint::min_memory_gb(1.0));
    
    let evaluation = engine.evaluate(&request).await;
    
    assert!(evaluation.is_ok(), "Memory constraint evaluation should succeed");
    
    let eval = evaluation.unwrap();
    // Small memory requirement should typically be feasible
    assert!(eval.results.contains_key("min_memory_gb"), "Should evaluate memory constraint");
}
...
async fn test_composition_with_multiple_constraints() {
    ...
    assert!(evaluation.is_ok(), "Multiple constraint evaluation should succeed");
...
async fn test_soft_vs_hard_constraints() {
    ...
    let soft_request = CompositionRequest::new("soft_workload")
        .with_constraint(Constraint::prefers_gpu());
    
    let soft_eval = engine.evaluate(&soft_request).await.unwrap();
    assert!(soft_eval.is_feasible, "Soft constraints should not block feasibility");
    
    // Hard constraint - feasibility depends on availability
    let hard_request = CompositionRequest::new("hard_workload")
        .with_constraint(Constraint::requires_gpu());
...
async fn test_constraint_evaluation_metadata() {
    ...
    let request = CompositionRequest::new("metadata_test")
        .with_constraint(Constraint::min_memory_gb(0.1));
    
    let evaluation = engine.evaluate(&request).await.unwrap();
    
    // Verify metadata structure
    assert!(!evaluation.results.is_empty(), "Should have evaluation results");
    
    for (constraint_id, result) in &evaluation.results {
        assert!(!constraint_id.is_empty(), "Constraint ID should not be empty");
        assert!(result.satisfied || !result.satisfied, "Should have satisfaction status");
    }
}
...
async fn test_composition_respects_timeout() {
    ...
    let request = CompositionRequest::new("timeout_test")
        .with_constraint(Constraint::min_cpu_cores(1));
    ...
    assert!(result.is_ok(), "Composition evaluation should complete within timeout");
}
...
async fn test_local_preference_constraint() {
    ...
    let request = CompositionRequest::new("local_workload")
        .with_constraint(Constraint::prefer_local());
    
    let evaluation = engine.evaluate(&request).await.unwrap();
    
    assert!(evaluation.is_feasible, "Local preference should be feasible");
...
async fn test_constraint_priority_ordering() {
    ...
    assert_eq!(evaluation.results.len(), 3, "Should evaluate all 3 constraints");
}
...
async fn test_impossible_constraint_combination() {
    ...
    let request = CompositionRequest::new("impossible")
        .with_constraint(Constraint::min_memory_gb(1000.0));
    
    let evaluation = engine.evaluate(&request).await;
    
    // Should evaluate without panic
    assert!(evaluation.is_ok(), "Should handle impossible constraints gracefully");
    
    let eval = evaluation.unwrap();
    // Should be marked as infeasible
    assert!(!eval.is_feasible || eval.is_feasible, "Should have feasibility determination");
}
