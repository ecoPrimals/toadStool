...
use toadstool::{
    execution::{
        ExecutionRequest, ExecutionResponse, ExecutionInput, ExecutionOutput, ExecutionStatus, RuntimeType
    },
    resources::{
        ResourceRequirements, RuntimeMetrics,
...
impl ExecutionRequestBuilder {
    ...
    pub fn build(self) -> ExecutionRequest {
        ExecutionRequest {
            ...
            resources: self.resources.unwrap_or_else(crate::fixtures::create_test_resource_requirements),
            security_context: self.security_context.unwrap_or_else(crate::fixtures::create_test_security_context),
            timeout: self.timeout.or_else(|| Some(Duration::from_secs(TestConstants::DEFAULT_TIMEOUT_SECS))),
            environment: self.environment,
            input_data: self.input_data.unwrap_or_else(crate::fixtures::create_test_execution_input),
...
impl ExecutionResponseBuilder {
    ...
    pub fn build(self) -> ExecutionResponse {
        ExecutionResponse {
            ...
            output: self.output.unwrap_or_else(crate::fixtures::create_test_execution_output),
            metrics: self.metrics.unwrap_or_else(crate::fixtures::create_test_runtime_metrics),
...
mod tests {
    ...
    fn test_runtime_metrics_builder() {
        let metrics = RuntimeMetricsBuilder::new()
            .custom_metric("test_metric", serde_json::Value::Number(serde_json::Number::from(42)))
            .build();
