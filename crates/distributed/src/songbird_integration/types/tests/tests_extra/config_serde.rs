...
use super::helpers::{
    sample_songbird_connection_config_with_endpoints, sample_songbird_integration_config,
};
...
fn test_songbird_connection_config_serde() {
    let config =
        sample_songbird_connection_config_with_endpoints(vec!["http://a:8080".to_string()]);
