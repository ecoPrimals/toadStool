...
pub trait PrimalAdapter: Send + Sync {
    ...
    async fn notify_capability_change(&self, capability: &Capability, available: bool) -> Result<()>;
...
impl PrimalAdapter for SongbirdAdapter {
    ...
    async fn register_capabilities(&self, capabilities: Vec<Capability>) -> Result<()> {
        ...
        let registration = SongbirdRegistrationRequest {
            ...
            capabilities: capabilities.iter().map(|c| SongbirdCapability {
        ...
        if !response.status().is_success() {
            ...
            return Err(anyhow::anyhow!(
                "Songbird registration failed ({}): {}", status, body
            ));
        }
        
        tracing::info!("Successfully registered {} capabilities with Songbird at {}", 
            capabilities.len(), self.endpoint);
    ...
    async fn notify_capability_change(&self, capability: &Capability, available: bool) -> Result<()> {
        ...
        if !response.status().is_success() {
            tracing::warn!("Capability update to Songbird failed: {}", response.status());
        }
    ...
    async fn deregister(&self) -> Result<()> {
        ...
        if !response.status().is_success() {
            ...
            tracing::info!("Successfully deregistered from Songbird at {}", self.endpoint);
        }
