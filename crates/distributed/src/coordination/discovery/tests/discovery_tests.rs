...
//! CoordinationNetworkDiscovery and type serialization tests
...
#[test]
fn test_coordination_discovery_for_test() {
    let (discovery, _) = make_discovery();
    let _ = discovery;
}

#[tokio::test]
async fn test_coordination_discovery_register_node_success() {
    let (discovery, _) = make_discovery();
...
#[tokio::test]
async fn test_coordination_discovery_register_node_empty_id() {
    let (discovery, _) = make_discovery();
...
#[tokio::test]
async fn test_coordination_discovery_register_node_empty_endpoints() {
    let (discovery, _) = make_discovery();
    let mut reg = make_node_registration("x", NodeType::ToadStool, 4.0, 8.0, 100.0);
    reg.endpoints = vec![];
    let result = discovery.register_node(reg).await;
...
#[tokio::test]
async fn test_coordination_discovery_get_network_capacity_empty() {
    let (discovery, _) = make_discovery();
...
#[tokio::test]
async fn test_coordination_discovery_get_network_capacity_with_nodes() {
    let (discovery, _) = make_discovery();
    discovery
...
#[tokio::test]
async fn test_coordination_discovery_get_optimal_distribution_no_nodes() {
    let (discovery, _) = make_discovery();
...
#[tokio::test]
async fn test_coordination_discovery_get_network_status() {
    let (discovery, _) = make_discovery();
...
fn test_node_registration_serialization_roundtrip() {
    let reg = make_node_registration("ser-node", NodeType::Storage, 2.0, 4.0, 50.0);
    let json = serde_json::to_string(&reg).unwrap();
    let parsed: NodeRegistration = serde_json::from_str(&json).unwrap();
    assert_eq!(parsed.node_id, reg.node_id);
    assert!(matches!(parsed.node_type, NodeType::Storage));
}
...
#[tokio::test]
async fn test_coordination_discovery_get_optimal_distribution_with_nodes() {
    let (discovery, _) = make_discovery();
    discovery
        .register_node(make_node_registration(
            "n1",
...
#[tokio::test]
async fn test_coordination_discovery_get_network_status_with_nodes() {
    let (discovery, _) = make_discovery();
    discovery
        .register_node(make_node_registration(
            "stat-node",
...
#[tokio::test]
async fn test_coordination_discovery_clone_creates_fresh_registry() {
    let (discovery, _) = make_discovery();
    discovery
        .register_node(make_node_registration(
            "clone-test",
...
async fn test_node_registry_register_direct() {
    use crate::coordination::types::NodeRegistry;
    let mut registry = NodeRegistry::new();
    let reg = make_node_registration("direct-reg", NodeType::Coordination, 1.0, 2.0, 10.0);
...
async fn test_node_registration_overwrites_same_node_id() {
    ...
    let reg2 = make_node_registration("dup-node", NodeType::Security, 8.0, 16.0, 200.0);
    ...
    assert!(matches!(nodes[0].node_type, NodeType::Security));
}
...
#[tokio::test]
async fn test_get_optimal_distribution_prefers_storage_when_requested() {
    let (discovery, _) = make_discovery();
    discovery
        .register_node(make_node_registration(
            "ts",
    ...
    let subtask = make_subtask();
    let plan = discovery
        .get_optimal_distribution(&[subtask], &[NodeType::Storage])
        .await
        .unwrap();
...
async fn test_registration_response_assigned_channels_include_type() {
    let (discovery, _) = make_discovery();
    let reg = make_node_registration("channels-test", NodeType::Security, 2.0, 4.0, 50.0);
    let resp = discovery.register_node(reg).await.unwrap();
    assert!(resp.assigned_channels.contains(&"global".to_string()));
    assert!(
        resp.assigned_channels
            .iter()
            .any(|c| c.contains("Security"))
    );
}
