...
fn test_node_registry_get_nodes_by_types_multiple() {
    ...
    let storage_nodes = registry.get_nodes_by_types(&[NodeType::Storage]);
...
fn test_node_registry_get_node() {
    let mut registry = NodeRegistry::new();
    let reg = make_node_registration("lookup", NodeType::Coordination, 1.0, 2.0, 10.0);
    registry.register_node(reg).unwrap();
...
#[test]
fn test_node_registry_get_nodes_by_types_coordination() {
    let mut registry = NodeRegistry::new();
    registry
        .register_node(make_node_registration(
            "sb",
        ...
        .unwrap();
    let coordination_nodes = registry.get_nodes_by_types(&[NodeType::Coordination]);
    assert_eq!(coordination_nodes.len(), 1);
    assert_eq!(coordination_nodes[0].node_id, "sb");
}
