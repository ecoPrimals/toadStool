...
use crate::coordination::types::{
    CoordinationConnection, CoordinationDiscoveryConfig, CoordinationNetworkDiscovery,
    NodeCapabilities, NodeMetadata, NodeRegistration, NodeType, ProtocolConfig,
};

use crate::coordination::types::ConnectionHealth;
...
fn make_coordination_connection() -> CoordinationConnection {
...
pub(super) fn make_discovery() -> (CoordinationNetworkDiscovery, std::path::PathBuf) {
    ...
    let discovery = CoordinationNetworkDiscovery::for_test(config, conn, socket_path.clone());
