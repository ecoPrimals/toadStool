...
#[test]
fn test_discovery_client_parse_node_data_storage_type() {
    let conn = Arc::new(make_coordination_connection());
    let temp_dir = tempfile::tempdir().expect("temp dir");
    let socket_path = temp_dir.path().join("coordination.sock");
    let client = DiscoveryClient::for_test(conn, socket_path);
...
#[test]
fn test_discovery_client_parse_node_data_security_type() {
    let conn = Arc::new(make_coordination_connection());
    let temp_dir = tempfile::tempdir().expect("temp dir");
    let socket_path = temp_dir.path().join("coordination.sock");
    let client = DiscoveryClient::for_test(conn, socket_path);
    ...
    let parsed = client.parse_node_data(&node_json).unwrap();
    assert!(matches!(parsed.node_type, NodeType::Security));
}

#[test]
fn test_discovery_client_parse_node_data_coordination_type() {
    let conn = Arc::new(make_coordination_connection());
    let temp_dir = tempfile::tempdir().expect("temp dir");
    let socket_path = temp_dir.path().join("coordination.sock");
    let client = DiscoveryClient::for_test(conn, socket_path);
    ...
    let parsed = client.parse_node_data(&node_json).unwrap();
    assert!(matches!(parsed.node_type, NodeType::Coordination));
}
