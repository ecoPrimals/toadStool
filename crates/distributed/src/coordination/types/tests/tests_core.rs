...
#[test]
fn test_coordination_job_request_constructor() {
...
#[test]
fn test_coordination_job_request_serde_roundtrip() {
    let req = CoordinationJobRequest {
        job_id: Uuid::new_v4(),
        job_payload: bytes::Bytes::from(vec![1, 2, 3]),
        target_nodes: vec![],
...
#[test]
fn test_coordination_feedback_sender_send() {
    let (sender, _receiver) = CoordinationFeedbackSender::new();
    let sent = sender.send(CoordinationFeedback::LoadUpdate {
...
#[test]
fn test_coordination_job_response_success_serde() {
    ...
    let parsed: CoordinationJobResponse = serde_json::from_str(&json).unwrap();
    assert!(matches!(parsed, CoordinationJobResponse::Success { .. }));
}

#[test]
fn test_coordination_job_response_error_serde() {
    let resp = CoordinationJobResponse::Error {
        job_id: Uuid::new_v4(),
        error: "failed".to_string(),
    };
    let json = serde_json::to_string(&resp).unwrap();
    let parsed: CoordinationJobResponse = serde_json::from_str(&json).unwrap();
    assert!(matches!(parsed, CoordinationJobResponse::Error { .. }));
}
...
#[test]
fn test_coordination_broadcast_message_serde() {
    ...
    let parsed: CoordinationBroadcastMessage = serde_json::from_str(&json).unwrap();
    assert_eq!(parsed.channel_name(), "capability-updates");
}

#[test]
fn test_coordination_feedback_variants() {
    let load = CoordinationFeedback::LoadUpdate {
        node_id: "n1".to_string(),
        load: 0.5,
    };
    assert!(matches!(load, CoordinationFeedback::LoadUpdate { .. }));
    let err = CoordinationFeedback::ErrorReport {
        node_id: "n1".to_string(),
        error: "fail".to_string(),
    };
    assert!(matches!(err, CoordinationFeedback::ErrorReport { .. }));
    let cap = CoordinationFeedback::CapacityAvailable {
        node_id: "n1".to_string(),
    };
    assert!(matches!(
        cap,
        CoordinationFeedback::CapacityAvailable { .. }
    ));
}
...
#[test]
fn test_coordination_feedback_sender_default() {
    let _sender = CoordinationFeedbackSender::default();
}
...
#[test]
fn test_coordination_broadcast_message_health_update_serde() {
    let msg = CoordinationBroadcastMessage::HealthUpdate {
        node_id: "n1".to_string(),
        health_status: "ok".to_string(),
        timestamp: SystemTime::now(),
...
#[test]
fn test_coordination_broadcast_message_custom_message_serde() {
