...
//! Message types for Coordination integration
...
/// In-process job control messages for the Coordination integration path.
pub enum CoordinationJobMessage {
    /// Run a job and send the result on `reply_channel`.
    ExecuteJob {
        /// Work to execute.
        job: Box<UniversalJob>,
        /// Channel for the final [`CoordinationJobResponse`].
        reply_channel: mpsc::Sender<CoordinationJobResponse>,
    },
...
pub enum CoordinationBroadcastMessage {
...
impl CoordinationBroadcastMessage {
    /// Derive a routing channel name from the message variant.
    ///
    /// Used by `CoordinationBroadcaster::broadcast()` to route to the correct channel.
    #[expect(
