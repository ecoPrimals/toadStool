...
use super::message_types::CoordinationBroadcastMessage;
...
pub struct BroadcastChannel {
    name: String,
    tx: broadcast::Sender<CoordinationBroadcastMessage>,
}
...
impl BroadcastChannel {
    ...
    pub fn publish(
        &self,
        msg: CoordinationBroadcastMessage,
    ) -> Result<usize, broadcast::error::SendError<CoordinationBroadcastMessage>> {
        self.tx.send(msg)
    }

    /// Subscribe to this channel.
    pub fn subscribe(&self) -> broadcast::Receiver<CoordinationBroadcastMessage> {
        self.tx.subscribe()
    }
}
...
pub struct SubscriptionManager {
    channels: Mutex<HashMap<String, broadcast::Sender<CoordinationBroadcastMessage>>>,
}
...
impl SubscriptionManager {
    ...
    pub fn subscribe(&self, channel: &str) -> broadcast::Receiver<CoordinationBroadcastMessage> {
        let mut guard = self
            .channels
            .lock()
    ...
    pub fn publish(&self, channel: &str, msg: CoordinationBroadcastMessage) -> usize {
        self.channels
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner)
            .get(channel)
