...
/// was updated. Used by `CoordinationLoadBalancer::request_advice` to select
...
/// Lightweight in-process performance counters for a Coordination connection.
///
...
/// Feedback messages sent back to Coordination about local node state.
#[derive(Debug, Clone)]
pub enum CoordinationFeedback {
...
/// Sends feedback events to Coordination's coordination loop.
///
/// Backed by an unbounded mpsc channel. Callers `send()` feedback; a
/// background task (or the Coordination client) drains `CoordinationFeedbackReceiver`.
pub struct CoordinationFeedbackSender {
    tx: mpsc::UnboundedSender<CoordinationFeedback>,
}

/// End of a feedback channel; receives [`CoordinationFeedback`] from the sender half.
pub struct CoordinationFeedbackReceiver {
    /// Unbounded receiver for feedback events.
    pub rx: mpsc::UnboundedReceiver<CoordinationFeedback>,
}

impl CoordinationFeedbackSender {
    /// Create a connected sender/receiver pair for feedback.
    pub fn new() -> (Self, CoordinationFeedbackReceiver) {
        let (tx, rx) = mpsc::unbounded_channel();
        (Self { tx }, CoordinationFeedbackReceiver { rx })
    }

    /// Emit a feedback event. Returns `false` if the receiver has been dropped.
    pub fn send(&self, feedback: CoordinationFeedback) -> bool {
        self.tx.send(feedback).is_ok()
    }
}

impl Default for CoordinationFeedbackSender {
    fn default() -> Self {
        Self::new().0
    }
}
