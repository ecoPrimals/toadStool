...
pub fn load<P: AsRef<Path>>(path: P) -> Result<ModelWeights> {
    ...
    let header_obj = header.as_object().ok_or_else(|| {
        Error::ModelLoad("Header is not a JSON object".to_string())
    })?;
    ...
    for (name, info) in header_obj {
        ...
        let info_obj = info.as_object().ok_or_else(|| {
            Error::ModelLoad(format!("Tensor {name} info is not an object"))
        })?;
        
        // Get dtype
        let dtype_str = info_obj.get("dtype")
            .and_then(|v| v.as_str())
        ...
        // Get data offsets
        let data_offsets = info_obj.get("data_offsets")
            .and_then(|v| v.as_array())
            .ok_or_else(|| Error::ModelLoad(format!("Missing data_offsets for {name}")))?;
        
        let start = data_offsets.first()
            .and_then(|v| v.as_u64())
            .ok_or_else(|| Error::ModelLoad(format!("Invalid start offset for {name}")))? as usize;
        
        let end = data_offsets.get(1)
            .and_then(|v| v.as_u64())
            .ok_or_else(|| Error::ModelLoad(format!("Invalid end offset for {name}")))? as usize;
        ...
        tensors.insert(name.clone(), WeightTensor {
            name: name.clone(),
            shape,
            dtype,
            data: tensor_data,
