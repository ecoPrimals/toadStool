...
//! Configuration structures for `Storage` integration
...
impl Default for StorageConfig {
    fn default() -> Self {
        // ⚠️ DEPRECATED: Use StorageClient::discover() for production
...
/// Cache configuration for `Storage` operations
#[derive(Debug, Clone, Serialize, Deserialize)]
...
mod tests {
    ...
    #[test]
    fn test_storage_ext_config_custom() {
        let config = StorageConfig {
            endpoint: "https://storage.example.com:8080".to_string(),
        ...
        assert_eq!(config.endpoint, "https://storage.example.com:8080");
    ...
    fn test_cache_config_custom() {
        ...
        assert!(!config.enabled);
        assert_eq!(config.cache_dir, Some(PathBuf::from("/tmp/storage-cache")));
