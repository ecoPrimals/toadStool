...
//! Comprehensive tests for Storage integration types
...
fn test_storage_error_pipeline() {
    let error = StorageError::Pipeline("execution failed".to_string());
    assert!(error.to_string().contains("pipeline"));
}

#[test]
fn test_storage_error_versioning() {
    let error = StorageError::Versioning("conflict".to_string());
    assert!(error.to_string().contains("Versioning"));
}

#[test]
fn test_storage_error_network() {
    let error = StorageError::Network("unreachable".to_string());
