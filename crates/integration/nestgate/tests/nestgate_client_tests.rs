...
use toadstool_integration_nestgate::{types::*, *};
...
fn test_nestgate_config_custom_endpoint() {
    ...
    let config = NestGateConfig {
        endpoint: "https://nestgate.example.com".to_string(),
        ..Default::default()
    };

    assert_eq!(config.endpoint, "https://nestgate.example.com");
...
fn test_nestgate_result_err() {
    // Test error result
    let error = NestGateError::Connection("failed".to_string());
    let result: NestGateResult<String> = Err(error);

    assert!(result.is_err());
    if let Err(e) = result {
        assert!(matches!(e, NestGateError::Connection(_)));
    }
}
...
#[test]
fn test_nestgate_client_coverage_summary() {
    println!("============================================");
    println!("NestGate Client Tests Summary:");
    println!("============================================");
    ...
    println!("============================================");
    println!("Target: Increase NestGate client coverage from 0% to 25-30%");
}
