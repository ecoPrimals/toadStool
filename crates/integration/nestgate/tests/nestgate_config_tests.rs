...
//! Comprehensive tests for NestGate configuration
...
#[test]
fn test_nestgate_config_custom_endpoint() {
...
#[test]
fn test_nestgate_config_custom_timeout() {
    let config = NestGateConfig {
...
#[test]
fn test_nestgate_config_custom_retries() {
    let config = NestGateConfig {
        max_retries: 5,
        ..Default::default()
    };
...
#[test]
fn test_nestgate_config_with_auth() {
    let config = NestGateConfig {
...
#[test]
fn test_nestgate_config_without_auth() {
    let config = NestGateConfig::default();
...
#[test]
fn test_nestgate_config_deserialization() {
    let json = r#"{
    ...
    let config: NestGateConfig = serde_json::from_str(json).expect("Failed to deserialize");
...
fn test_cache_config_with_directory() {
    ...
    assert_eq!(
        ...
        PathBuf::from("/tmp/nestgate-cache")
    );
}
...
fn test_full_configuration_scenario() {
    let mut config = NestGateConfig {
        endpoint: "http://production-nestgate:8080".to_string(),
    ...
    config.cache = Some(CacheConfig {
        enabled: true,
        cache_dir: Some(PathBuf::from("/var/cache/nestgate")),
        max_size: 10 * 1024 * 1024 * 1024,  // 10GB
        ttl: Duration::from_secs(3600 * 4), // 4 hours
    });

    assert_eq!(config.endpoint, "http://production-nestgate:8080");
