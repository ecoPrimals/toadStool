...
pub fn build_capabilities(resources: &SystemResources) -> Vec<String> {
    ...
    if gb >= 64 {
        capabilities.push("memory-large".to_string());
    } else if gb >= 16 {
        capabilities.push("memory-medium".to_string());
    } else {
        capabilities.push("memory-small".to_string());
    }
...
mod tests {
    ...
    async fn test_self_discovery() {
        ...
        assert!(caps.primal_type == "toadstool");
    }
    ...
    fn test_system_resources_query() {
        ...
        assert!(resources.architecture.len() > 0);
        assert!(resources.os.len() > 0);
    }
    ...
    fn test_capabilities_build() {
        ...
        assert!(capabilities.contains(&"memory-medium".to_string()));
    }
}
