...
impl DispatchHandler {
    ...
    pub async fn dispatch_submit(
        ...
        let binary_bytes: Vec<u8> = binary.iter().map(|v| v.as_u64().unwrap_or(0) as u8).collect();
...
fn detect_dispatch_mode(params: &serde_json::Value, bdf: &str) -> String {
    if let Some(mode) = params.get("dispatch_mode").and_then(serde_json::Value::as_str) {
        return mode.to_string();
    }

    let gpus = toadstool_sysmon::discover_gpus();
    if gpus.iter().any(|g| g.pci_slot == bdf && g.driver == "vfio-pci") {
        "vfio".to_string()
    } else {
        "drm".to_string()
    }
