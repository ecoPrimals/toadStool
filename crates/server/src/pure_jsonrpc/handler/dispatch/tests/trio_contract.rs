...
// ── compute.fan_out validation ──

#[tokio::test]
async fn fan_out_requires_work_units() {
    let handler = test_handler();
    let params = serde_json::json!({ "dag_session_id": "test-session" });
    let err = handler
        .compute_fan_out(Some(&params))
        .await
        .expect_err("missing work_units should fail");
    assert_eq!(err.code, JsonRpcError::INVALID_PARAMS);
}
...
async fn fan_out_rejects_empty_work_units() {
    ...
    let err = handler
        .compute_fan_out(Some(&params))
        .await
        .expect_err("empty work_units should fail");
    assert_eq!(err.code, JsonRpcError::INVALID_PARAMS);
}

#[tokio::test]
async fn fan_out_assigns_units_without_gpu_filter() {
    let handler = test_handler();
    let params = serde_json::json!({
        "work_units": [
            { "unit_id": "clone-001" },
    ...
    let result = handler.compute_fan_out(Some(&params)).await.unwrap();
    assert_eq!(result["total_units"], 2);
    assert!(result["dispatch_id"].as_str().is_some_and(|s| !s.is_empty()));
...
#[tokio::test]
async fn fan_out_queues_when_gpu_required_no_factory() {
    let handler = test_handler();
    let params = serde_json::json!({
        "work_units": [{ "unit_id": "gpu-task" }],
...
async fn fan_out_auto_generates_unit_ids() {
    let handler = test_handler();
    let params = serde_json::json!({
        "work_units": [{ "data": "payload" }],
    });
    let result = handler.compute_fan_out(Some(&params)).await.unwrap();
    let assigned = result["assigned"].as_array().unwrap();
    let uid = assigned[0]["unit_id"].as_str().unwrap();
    assert!(!uid.is_empty(), "auto-generated unit_id should be non-empty");
}
