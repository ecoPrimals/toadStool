    ...
    R: AsyncBufRead + Unpin,
{
    ...
    loop {
        ...
        if let Some((name, value)) = super::unix::parse_http_header_field(&line) {
...
where
    W: AsyncWrite + Unpin,
{
    let header = super::unix::format_http_response_header(body.len(), closing);
    writer
...
where
    W: AsyncWrite + Unpin,
{
    let reject = super::unix::unsignalled_connection_reject_json();
