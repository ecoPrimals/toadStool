...
use crate::resource_estimator::{ResourceEstimator, ResourceEstimate};
...
impl ResourceOptimizer {
    ...
    pub async fn suggest_optimizations(
        ...
        info!("Analyzing graph for optimization opportunities: {}", graph.id);
        ...
        info!("Found {} bottlenecks and {} optimization opportunities for graph {}",
              bottlenecks.len(), opportunities.len(), graph.id);
    ...
    async fn query_system_capabilities(&self) -> Result<SystemCapabilities, OptimizationError> {
        ...
        let mem_info = sys_info::mem_info()
            .map_err(|e| OptimizationError::SystemQueryFailed(format!("Memory query failed: {}", e)))?;
        let total_memory_bytes = mem_info.total * 1024;
        let available_memory_bytes = mem_info.avail * 1024;
        
        // Query disk
        let disk_info = sys_info::disk_info()
            .map_err(|e| OptimizationError::SystemQueryFailed(format!("Disk query failed: {}", e)))?;
        let total_storage_bytes = disk_info.total * 1024;
        let available_storage_bytes = disk_info.free * 1024;
        
        // GPU (placeholder)
        let (total_gpu_memory_bytes, available_gpu_memory_bytes, gpu_count, gpu_types) = (0, 0, 0, Vec::new());
    ...
    fn identify_bottlenecks(
        ...
        if estimate.max_parallelism == 1 && graph.nodes.len() > 1 {
            bottlenecks.push(Bottleneck {
                ...
                severity: 0.8,
                description: "Graph is entirely sequential. Parallelization may be possible.".to_string(),
        ...
        if memory_gb > 64 {
            bottlenecks.push(Bottleneck {
                ...
                severity: 0.7,
                description: format!("High memory usage: {} GB. Consider streaming or batching.", memory_gb),
        ...
        if capabilities.gpu_count > 0 && estimate.gpu_memory_bytes == 0 {
            let cpu_nodes: Vec<String> = graph.nodes.iter()
    ...
    fn find_high_memory_nodes(&self, _graph: &ExecutionGraph, estimate: &ResourceEstimate) -> Vec<String> {
    ...
    fn discover_opportunities(
        ...
        if capabilities.gpu_count > 0 {
            opportunities.extend(self.find_gpu_acceleration_opportunities(graph, estimate, capabilities));
        }
    ...
    fn find_parallelization_opportunities(
        ...
        for (node_id, node_estimate) in &estimate.node_estimates {
            level_groups.entry(node_estimate.parallelism_level)
                ...
                if let Some(node) = graph.get_node(&nodes[0]) {
                    if node.operation == "cpu_compute" || node.operation == "gpu_compute" {
                        opportunities.push(Opportunity {
                            ...
                            recommendation: "Consider splitting this node into multiple parallel tasks.".to_string(),
        ...
        for (node_id, node_estimate) in &estimate.node_estimates {
            ...
            if memory_gb > 16 {
                ...
                opportunities.push(Opportunity {
                    ...
                    recommendation: "Consider streaming data instead of loading all at once.".to_string(),
    ...
    fn find_batching_opportunities(&self, graph: &ExecutionGraph) -> Vec<Opportunity> {
        ...
        for node in &graph.nodes {
            operation_groups.entry(node.operation.clone())
                .or_insert_with(Vec::new)
                .push(node.id.clone());
        }
        ...
        for (operation, nodes) in operation_groups {
            if nodes.len() >= 3 {
                opportunities.push(Opportunity {
                    ...
                    recommendation: "Consider batching these operations together for better efficiency.".to_string(),
        ...
        for node in &graph.nodes {
            ...
            if dependents.len() >= 2 {
                opportunities.push(Opportunity {
                    ...
                    recommendation: "Consider caching the output of this node to avoid recomputation.".to_string(),
    ...
    fn estimate_improvement(
        ...
        let total_time_savings: u64 = opportunities.iter()
            .map(|o| o.time_savings_secs)
            .sum();
    ...
    fn rank_by_priority(&self, opportunities: &[Opportunity]) -> Vec<String> {
        let mut ranked: Vec<(String, f32)> = opportunities.iter()
            .map(|o| {
...
mod tests {
    use super::*;
    use crate::graph_types::{GraphNode, GraphEdge, EdgeType, NodeResourceRequirements};
    ...
    async fn test_suggest_optimizations_sequential_graph() {
        ...
        assert!(suggestions.bottlenecks.iter().any(|b| 
            b.bottleneck_type == BottleneckType::SequentialExecution
        ));
