...
fn bench_batched_traditional(device: &WgpuDevice) -> BenchmarkResult {
    let batch_size = 100;
    let work_data: Vec<u8> = (0..WORK_UNIT_SIZE).map(|i| (i % 256) as u8).collect();
    let batch_data: Vec<u8> = work_data.iter().cycle().take(WORK_UNIT_SIZE * batch_size).copied().collect();
    ...
    for _ in 0..batches {
        ...
        encoder.copy_buffer_to_buffer(&buffer, 0, &staging, 0, (WORK_UNIT_SIZE * batch_size) as u64);
        device.queue().submit(Some(encoder.finish()));
        device.device().poll(wgpu::Maintain::Wait);
...
fn main() {
    ...
    println!("╚════════════════════════════════════════════════════════════╝");
    println!();
    println!("Config: {} work units × {} KB = {} MB", 
        WORK_UNIT_COUNT, WORK_UNIT_SIZE / 1024, 
