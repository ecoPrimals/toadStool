...
impl ESN {
    ...
    pub async fn new(device: &WgpuDevice, config: ESNConfig) -> BarracudaResult<Self> {
        ...
        let w_in = (0..(config.reservoir_size * config.input_size))
            .map(|i| {
                let seed = config.seed.wrapping_add(i as u32).wrapping_mul(1664525).wrapping_add(1013904223);
        ...
        if input.len() != self.config.input_size {
            return Err(BarracudaError::InvalidInput {
                message: format!(
                    ...
                    self.config.input_size, input.len()
                ),
            });
        }
    ...
    ///
    /// Training error (MSE)
    pub async fn train(&mut self, inputs: &[Vec<f32>], targets: &[Vec<f32>]) -> BarracudaResult<f32> {
        ...
        // Train readout layer
        self.w_out = Some(ridge_regression(
            &self.device.device,
            &self.device.queue,
            &states,
            &flat_targets,
...
mod tests {
    ...
    async fn test_esn_validation() {
        ...
        assert!(ESN::new(&device, ESNConfig {
            spectral_radius: 0.0,
            ..Default::default()
        }).await.is_err());
