// Attention Softmax: Apply softmax to attention scores
...
@group(0) @binding(0) var<storage, read> scores: array<f32>;    // [batch, heads, seq, seq]
@group(0) @binding(1) var<storage, read_write> weights: array<f32>; // [batch, heads, seq, seq]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let batch = idx / (params.num_heads * params.seq_len);
    let head = (idx / params.seq_len) % params.num_heads;
    let i = idx % params.seq_len;
    
    let base_idx = batch * params.num_heads * params.seq_len * params.seq_len
