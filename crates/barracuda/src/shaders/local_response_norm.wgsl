// local_response_norm.wgsl - Local Response Normalization (LRN)
//
...
struct Params {
    ...
    alpha: f32,     // Scaling parameter (typically 1e-4)
    beta: f32,      // Exponent (typically 0.75)
    k: f32,         // Bias (typically 1.0 or 2.0)
}

@group(0) @binding(0) var<storage, read> input: array<f32>;         // [B, C, H, W]
@group(0) @binding(1) var<storage, read_write> output: array<f32>;  // [B, C, H, W]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let scale = pow(params.k + params.alpha * sum_sq / f32(params.size), params.beta);
