...
//
// ESN reservoir update kernel (f64 canonical)
//
...
//
// Provenance: hotSpring v0.6.0 (Stanton-Murillo transport, ESN D*/η*/λ* prediction)
//
// Bindings:
//   0: w_in    [R*I] f64, read  — input weights (row-major, R × I)
//   1: w_res   [R*R] f64, read  — reservoir weights (row-major, R × R)
//   2: input   [I]   f64, read  — current input vector
//   3: state   [R]   f64, rw    — reservoir state (updated in-place)
//   4: params  [4]   f64, read  — [reservoir_size, input_size, leak_rate, _]
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    state[i] = (1.0 - alpha) * state[i] + alpha * tanh_f64(pre);
}
