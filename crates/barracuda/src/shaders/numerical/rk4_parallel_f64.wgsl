// rk4_parallel.wgsl — Parallel Multi-System RK4 Integration (f64 canonical)
//
...
@group(0) @binding(2) var<storage, read_write> state_out: array<f64>;
...
@group(0) @binding(4) var<storage, read_write> scratch: array<f64>;

fn hill(x: f64, k: f64, n: f64) -> f64 {
    let xn = pow_f64(x, n);
    return xn / (pow_f64(k, n) + xn);
}

fn read_state(sys: u32, d: u32) -> f64 {
    return state[sys * params.dim + d];
}

fn write_scratch(sys: u32, stage: u32, d: u32, val: f64) {
    scratch[sys * params.dim * 5u + stage * params.dim + d] = val;
}

fn read_scratch(sys: u32, stage: u32, d: u32) -> f64 {
