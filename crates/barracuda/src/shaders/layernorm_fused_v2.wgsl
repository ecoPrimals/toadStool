// Fused LayerNorm V2: Single-launch, multi-phase layer normalization
//
...
@group(0) @binding(4) var<storage, read_write> partial_stats: array<f32>;  // [mean, m2, count] per workgroup
...
var<workgroup> global_mean: f32;
var<workgroup> global_variance: f32;
...
fn main(
    ...
    if (wg_id == 0u && tid == 0u) {
        ...
        var final_mean: f32 = 0.0;
        var final_m2: f32 = 0.0;
        ...
        for (var i = 0u; i < params.num_workgroups; i = i + 1u) {
            ...
            if (partial_count > 0u) {
                ...
                // Combine means
                final_mean = final_mean + delta * f32(partial_count) / f32(total_count);
                
                // Combine M2
                final_m2 = final_m2 + partial_m2 + delta * delta * f32(final_count) * f32(partial_count) / f32(total_count);
        ...
        let variance = final_m2 / f32(final_count);
