...
@group(0) @binding(1) var<storage, read> displacement_x: array<f32>; // Random displacement field X
@group(0) @binding(2) var<storage, read> displacement_y: array<f32>; // Random displacement field Y
@group(0) @binding(3) var<storage, read_write> output: array<f32>;   // Deformed output
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let dx = displacement_x[disp_idx] * params.alpha;
    let dy = displacement_y[disp_idx] * params.alpha;
