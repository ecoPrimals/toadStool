// graph_norm.wgsl - Graph Normalization
//
...
@group(0) @binding(0) var<storage, read> input: array<f32>;           // [num_nodes, num_features]
@group(0) @binding(1) var<storage, read> gamma: array<f32>;           // [num_features] - scale
@group(0) @binding(2) var<storage, read> beta: array<f32>;            // [num_features] - shift
@group(0) @binding(3) var<storage, read_write> output: array<f32>;    // [num_nodes, num_features]
@group(0) @binding(4) var<uniform> params: Params;

var<workgroup> shared_mean: array<f32, 256>;
var<workgroup> shared_var: array<f32, 256>;
...
fn main(
    ...
    let mean = shared_mean[0] / f32(params.num_nodes);
    ...
    let variance = shared_var[0] / f32(params.num_nodes);
