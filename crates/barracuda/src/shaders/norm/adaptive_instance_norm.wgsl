...
@group(0) @binding(0) var<storage, read> content: array<f32>;
@group(0) @binding(1) var<storage, read> style_mean: array<f32>;
@group(0) @binding(2) var<storage, read> style_std: array<f32>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    content_mean = content_mean / f32(params.spatial_size);
    
    // Second pass: compute variance
    var content_var: f32 = 0.0;
    for (var s = 0u; s < params.spatial_size; s = s + 1u) {
        let content_idx = batch_idx * params.channels * params.spatial_size +
                         channel_idx * params.spatial_size + s;
    ...
    content_var = content_var / f32(params.spatial_size);
    let content_std = sqrt(content_var);
    
    // Apply AdaIN: normalize content, then scale/shift to style
    let content_idx = batch_idx * params.channels * params.spatial_size +
                     channel_idx * params.spatial_size + spatial_idx;
    let normalized = (content[content_idx] - content_mean) / (content_std + 1e-5);
