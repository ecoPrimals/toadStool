...
@group(0) @binding(3) var<storage, read> bias: array<f32>;        // [bias_ih..., bias_hh...] = [4*hidden] + [4*hidden]
@group(0) @binding(4) var<storage, read_write> h_prev: array<f32>; // [batch, hidden]
@group(0) @binding(5) var<storage, read_write> c_prev: array<f32>; // [batch, hidden]
@group(0) @binding(6) var<storage, read_write> h_next: array<f32>; // [batch, hidden]
@group(0) @binding(7) var<storage, read_write> c_next: array<f32>; // [batch, hidden]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var h: u32 = 0u; h < params.hidden_size; h = h + 1u) {
        ...
        for (var gate: u32 = 0u; gate < 4u; gate = gate + 1u) {
            ...
            // Add biases (bias = [bias_ih | bias_hh], each 4*hidden_size elements)
