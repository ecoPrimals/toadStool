// bi_lstm.wgsl - Bidirectional LSTM
//
...
@group(0) @binding(0) var<storage, read> input: array<f32>;        // [seq_len, batch, input_size]
@group(0) @binding(1) var<storage, read> weight_ih: array<f32>;    // [4*hidden, input] (i, f, g, o gates)
@group(0) @binding(2) var<storage, read> weight_hh: array<f32>;    // [4*hidden, hidden]
@group(0) @binding(3) var<storage, read> bias_ih: array<f32>;      // [4*hidden]
@group(0) @binding(4) var<storage, read> bias_hh: array<f32>;      // [4*hidden]
@group(0) @binding(5) var<storage, read_write> h_state: array<f32>; // [batch, hidden]
@group(0) @binding(6) var<storage, read_write> c_state: array<f32>; // [batch, hidden]
@group(0) @binding(7) var<storage, read_write> output: array<f32>;  // [seq_len, batch, hidden]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Process sequence in specified direction
    ...
    loop {
        ...
        // Load input x_t: [batch, input_size]
        ...
        for (var gate: u32 = 0u; gate < 4u; gate = gate + 1u) {
            ...
            // Apply activation
            gates[gate] = select(tanh(sum), sigmoid(sum), gate < 3u); // i, f, o use sigmoid; g uses tanh
        }
        
        let i_gate = gates[0];
        let f_gate = gates[1];
        let g_gate = gates[3]; // Note: g is computed with tanh
        let o_gate = gates[2];
