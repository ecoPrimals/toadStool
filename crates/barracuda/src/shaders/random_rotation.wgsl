// random_rotation.wgsl - Random rotation augmentation
//
...
@group(0) @binding(1) var<storage, read> rotation_matrices: array<f32>; // [B, 4] - [cos, -sin, sin, cos]
@group(0) @binding(2) var<storage, read_write> output: array<f32>;    // [B, C, H, W]
...
fn bilinear_sample(x: f32, y: f32, b: u32, c: u32) -> f32 {
    ...
    // Bounds check
    if (x0 < 0.0 || x1 >= f32(params.width) || y0 < 0.0 || y1 >= f32(params.height)) {
