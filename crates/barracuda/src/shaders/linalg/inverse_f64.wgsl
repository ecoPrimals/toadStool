...
//
// **Precision**: Full f64 for numerical stability on ill-conditioned matrices
//
// Algorithm:
...
@group(0) @binding(1) var<storage, read_write> work_matrix: array<f64>;  // Augmented matrix [A | I]
@group(0) @binding(2) var<storage, read_write> output: array<f64>;      // Final inverse matrix
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var i = 0u; i < n; i = i + 1u) {
        for (var j = 0u; j < n; j = j + 1u) {
            ...
            if (j == i) {
                work_matrix[i * aug_cols + n + j] = f64(1.0);
            } else {
                work_matrix[i * aug_cols + n + j] = f64(0.0);
            }
        }
    }
    
    // Gauss-Jordan elimination with partial pivoting
    let epsilon = f64(1e-14);  // Tight tolerance for f64
