// QR Decomposition (f64) - Shader-First Implementation
//
...
@group(0) @binding(1) var<storage, read_write> A: array<f64>;    // Matrix [m × n], row-major
@group(0) @binding(2) var<storage, read_write> v: array<f64>;    // Householder vector [m]
@group(0) @binding(3) var<storage, read_write> tau: array<f64>;  // Householder scalars [min(m,n)]
...
@group(0) @binding(1) var<storage, read> A_hh: array<f64>;
@group(0) @binding(2) var<storage, read_write> v_hh: array<f64>;
@group(0) @binding(3) var<storage, read_write> tau_hh: array<f64>;
@group(0) @binding(4) var<storage, read> norm_sq_buf: array<f64>;  // norm_sq from previous kernel
...
fn compute_householder(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let norm_sq = norm_sq_buf[0];
    let norm = sqrt(norm_sq);  // native f64 sqrt
    let a_kk = A_hh[k * n + k];
    ...
    if (row == k) {
        ...
        let v_norm_sq = v_0 * v_0 + (norm_sq - a_kk * a_kk);
        if (v_norm_sq > 1e-14) {
...
@group(0) @binding(1) var<storage, read> v_apply: array<f64>;
@group(0) @binding(2) var<storage, read_write> A_apply: array<f64>;
@group(0) @binding(3) var<storage, read_write> w: array<f64>;  // Work array [n]
@group(0) @binding(4) var<storage, read> tau_apply: array<f64>;
...
@group(0) @binding(1) var<storage, read> v_q: array<f64>;
@group(0) @binding(2) var<storage, read_write> Q: array<f64>;  // [m × m]
@group(0) @binding(3) var<storage, read> tau_q: array<f64>;
@group(0) @binding(4) var<storage, read_write> w_q: array<f64>;  // temp [m]
...
fn Q_times_v(@builtin(local_invocation_id) local_id: vec3<u32>,
    ...
    // Store in w_q for this row
    if (tid == 0u) {
        w_q[i] = shared_sum[0];
    }
}

// Update Q[i,j] -= tau * w_q[i] * v[j-k] for j >= k
@compute @workgroup_size(16, 16, 1)
fn update_Q(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let i = global_id.y;
    let rel_j = global_id.x;
    let m = q_params.m;
    let k = q_params.k;
