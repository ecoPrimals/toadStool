// sinkhorn_distance.wgsl - Sinkhorn Distance (Regularized Optimal Transport)
//
...
struct Params {
    size: u32,
    num_iterations: u32,
    epsilon: f32,      // Regularization parameter
    _padding: u32,
}

@group(0) @binding(0) var<storage, read> dist1: array<f32>;        // Source distribution
@group(0) @binding(1) var<storage, read> dist2: array<f32>;        // Target distribution
@group(0) @binding(2) var<storage, read> cost_matrix: array<f32>;  // [size, size] - pairwise costs
@group(0) @binding(3) var<storage, read_write> transport: array<f32>; // [size, size] - transport plan
@group(0) @binding(4) var<storage, read_write> output: array<f32>;    // [1] - distance
@group(0) @binding(5) var<uniform> params: Params;

var<workgroup> shared_u: array<f32, 256>;
    ...
    for (var iter: u32 = 0u; iter < params.num_iterations; iter = iter + 1u) {
        ...
        if (idx < params.size) {
            ...
            for (var j: u32 = 0u; j < params.size; j = j + 1u) {
                ...
                let kernel = exp(-cost / params.epsilon);
