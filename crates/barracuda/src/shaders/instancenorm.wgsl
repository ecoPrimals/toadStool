...
@group(0) @binding(1) var<storage, read> gamma: array<f32>;  // Scale per channel
@group(0) @binding(2) var<storage, read> beta: array<f32>;   // Shift per channel
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let mean = sum / f32(params.spatial_size);
    ...
    let variance = var_sum / f32(params.spatial_size);
