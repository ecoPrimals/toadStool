...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    for (var b = 0u; b < params.batch_size; b = b + 1u) {
        ...
        for (var c = 0u; c < params.channels; c = c + 1u) {
            // Inlined bilinear interpolation: convert from normalized [-1, 1] to pixel coordinates
            let x_pix = (grid_x + 1.0) * f32(w - 1u) * 0.5;
            let y_pix = (grid_y + 1.0) * f32(h - 1u) * 0.5;
