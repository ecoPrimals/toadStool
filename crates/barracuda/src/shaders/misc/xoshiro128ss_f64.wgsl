...
// Generates uniform f64 in [0, 1) by dividing result by 2^32.
...
@group(0) @binding(1) var<storage, read_write> output: array<f64>; // n_threads * n_samples
...
fn generate(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i: u32 = 0u; i < params.n_samples; i = i + 1u) {
        ...
        output[out_base + i] = f64(raw) / 4294967296.0;  // 2^32
    }
