// Interpolate Nearest - Nearest neighbor interpolation for upsampling/downsampling
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let scale_h = f32(params.in_height) / f32(params.out_height);
    let scale_w = f32(params.in_width) / f32(params.out_width);
    
    // Find nearest input pixel (round down)
    let in_y = u32(f32(out_y) * scale_h);
    let in_x = u32(f32(out_x) * scale_w);
