...
//
// triangle_mul_outgoing_f64.wgsl — AlphaFold2 Evoformer triangle multiplicative update (outgoing edges)
//
// Computes: z_ij = sigmoid(gate_ij) * sum_k(a_ik * b_jk)
// Outgoing edges: for each (i,j), aggregate over k where a is indexed by (i,k) and b by (j,k).
//
