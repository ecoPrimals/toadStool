// sdpa_scores.wgsl — QK^T / sqrt(d_k) — Pass 1 of 3-pass SDPA
//
// Computes scaled dot-product scores for each (batch, head, query_pos) triple.
// Output layout: scores[batch, head, q_pos, k_pos] (row-major)
//
// Used by TensorSession::attention() as the first of 3 sequential passes:
//   1. sdpa_scores  → scores[B, H, S, S]
//   2. attention_softmax → weights[B, H, S, S]
//   3. attention_apply   → output[B, H, S, D]

struct AttentionParams {
    batch_size: u32,
    num_heads:  u32,
    seq_len:    u32,
    head_dim:   u32,
}

@group(0) @binding(0) var<storage, read>       query:  array<f32>;  // [B, H, S, D]
@group(0) @binding(1) var<storage, read>       key:    array<f32>;  // [B, H, S, D]
@group(0) @binding(2) var<storage, read_write> scores: array<f32>;  // [B, H, S, S]
@group(0) @binding(3) var<uniform>             params: AttentionParams;

@compute @workgroup_size(256)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    // Each thread handles one (batch, head, q_pos, k_pos) pair
    let total = params.batch_size * params.num_heads * params.seq_len * params.seq_len;
    let idx   = gid.x;
    if idx >= total { return; }

    let bh     = idx / (params.seq_len * params.seq_len);
    let rem    = idx % (params.seq_len * params.seq_len);
    let q_pos  = rem / params.seq_len;
    let k_pos  = rem % params.seq_len;

    let b    = bh / params.num_heads;
    let h    = bh % params.num_heads;

    if b >= params.batch_size { return; }

    let D     = params.head_dim;
    let S     = params.seq_len;
    let H     = params.num_heads;

    let q_base = b * H * S * D + h * S * D + q_pos * D;
    let k_base = b * H * S * D + h * S * D + k_pos * D;

    var dot = 0.0f;
    for (var d = 0u; d < D; d++) {
        dot += query[q_base + d] * key[k_base + d];
    }

    let scale  = sqrt(f32(D));
    let out_idx = b * H * S * S + h * S * S + q_pos * S + k_pos;
    scores[out_idx] = dot / scale;
}
