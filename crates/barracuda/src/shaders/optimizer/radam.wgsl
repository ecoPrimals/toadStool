// RAdam - Rectified Adam Optimizer (Liu et al.)
...
@group(0) @binding(3) var<storage, read_write> momentum: array<f32>;      // m
@group(0) @binding(4) var<storage, read_write> variance: array<f32>;      // v
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let beta1_pow = pow(params.beta1, step_f);
    let beta2_pow = pow(params.beta2, step_f);
    ...
    if (rho_t > 5.0) {
        ...
        let r = sqrt(r_num / r_den);
        update = params.learning_rate * r * m_hat / (sqrt(v_hat) + params.epsilon);
    } else {
