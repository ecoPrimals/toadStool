// LAMB - Layer-wise Adaptive Moments optimizer for Batch training (You et al.)
...
@group(0) @binding(4) var<storage, read_write> variance: array<f32>;
@group(0) @binding(5) var<storage, read_write> adam_step: array<f32>;  // r_t (intermediate)
...
fn compute_adam_step(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let grad = gradients[idx] + params.weight_decay * parameters[idx]; // Add weight decay
    ...
    let m_hat = m / (1.0 - pow(params.beta1, step_f));
    let v_hat = v / (1.0 - pow(params.beta2, step_f));
    
    // Compute Adam step: r_t = m_hat / (sqrt(v_hat) + ε)
    let r_t = m_hat / (sqrt(v_hat) + params.epsilon);
    adam_step[idx] = r_t;
}

// Step 2: Compute trust ratio and apply update
// GPU-only implementation: computes norms and applies trust ratio
@compute @workgroup_size(1)
fn apply_trust_ratio(@builtin(global_invocation_id) global_id: vec3<u32>) {
    // Compute norms (single workgroup for reduction - can be optimized with parallel reduction later)
    var param_norm_sq = 0.0;
    var step_norm_sq = 0.0;
    ...
    let param_norm = sqrt(param_norm_sq);
    let step_norm = sqrt(step_norm_sq);
