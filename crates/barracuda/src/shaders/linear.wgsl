// Linear - Fully connected layer (complete parallel implementation)
// Computes y = xW^T + b
//
// Algorithm:
...
@group(0) @binding(1) var<storage, read> input: array<f32>;   // [batch, in_features]
@group(0) @binding(2) var<storage, read> weight: array<f32>;  // [out_features, in_features]
@group(0) @binding(3) var<storage, read> bias: array<f32>;    // [out_features]
@group(0) @binding(4) var<storage, read_write> output: array<f32>; // [batch, out_features]
