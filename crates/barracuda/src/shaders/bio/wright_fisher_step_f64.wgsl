...
//   2. Drift: Binomial(2N, p') via PRNG sampling
//
// Uses inline xoshiro128** PRNG (u32 state).
...
@group(0) @binding(0) var<storage, read> freq_in: array<f64>;
@group(0) @binding(1) var<storage, read> selection: array<f64>;
@group(0) @binding(2) var<storage, read_write> freq_out: array<f64>;
...
fn wright_fisher(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let w_a = f64(1.0) + sel;
    let p_sel = (p * w_a) / (p * w_a + (f64(1.0) - p));

    // Binomial drift: count successes in 2N trials
    ...
    freq_out[tid] = f64(successes) / f64(two_n);
