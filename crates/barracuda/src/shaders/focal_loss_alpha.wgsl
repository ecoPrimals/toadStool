// focal_loss_alpha.wgsl - Focal Loss with class weighting
//
...
@group(0) @binding(0) var<storage, read> predictions: array<f32>;  // [batch, num_classes] - probabilities
@group(0) @binding(1) var<storage, read> targets: array<u32>;      // [batch] - class indices
@group(0) @binding(2) var<storage, read> alpha: array<f32>;        // [num_classes] - class weights
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let focal_weight = pow(1.0 - p_t_clamped, params.gamma);
    let loss = -alpha_t * focal_weight * log(p_t_clamped);
