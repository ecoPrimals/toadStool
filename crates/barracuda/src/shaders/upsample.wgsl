// Upsample - Upsampling (nearest neighbor or bilinear)
...
@group(0) @binding(1) var<storage, read> input: array<f32>;  // [B, C, H_in, W_in]
@group(0) @binding(2) var<storage, read_write> output: array<f32>; // [B, C, H_out, W_out]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (params.align_corners != 0u) {
        scale_h = f32(params.in_height - 1u) / f32(params.out_height - 1u);
        scale_w = f32(params.in_width - 1u) / f32(params.out_width - 1u);
    } else {
        scale_h = f32(params.in_height) / f32(params.out_height);
        scale_w = f32(params.in_width) / f32(params.out_width);
    }
    
    let fh = f32(oh) * scale_h;
    let fw = f32(ow) * scale_w;
    ...
    if (params.mode == 0u) {
        ...
        let fh_frac = fh - f32(ih0);
        let fw_frac = fw - f32(iw0);
