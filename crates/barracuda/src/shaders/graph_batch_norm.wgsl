// Graph Batch Normalization - Batch normalization adapted for graph data
...
@group(0) @binding(1) var<storage, read> input: array<f32>;      // [num_nodes, num_features]
@group(0) @binding(2) var<storage, read> gamma: array<f32>;      // [num_features] (scale)
@group(0) @binding(3) var<storage, read> beta: array<f32>;       // [num_features] (shift)
@group(0) @binding(4) var<storage, read_write> output: array<f32>; // [num_nodes, num_features]
@group(0) @binding(5) var<storage, read_write> mean: array<f32>;    // [num_features] (computed)
@group(0) @binding(6) var<storage, read_write> variance: array<f32>; // [num_features] (computed)
...
fn compute_mean(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    mean[feature_idx] = sum / f32(params.num_nodes);
}
...
fn compute_variance(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    variance[feature_idx] = sum_sq / f32(params.num_nodes);
}
