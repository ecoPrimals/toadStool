// label_smoothing.wgsl - Label Smoothing for classification
//
...
struct Params {
    batch_size: u32,
    num_classes: u32,
    smoothing: f32,  // ε, typically 0.1
    _padding: u32,
}

@group(0) @binding(0) var<storage, read> labels: array<u32>;       // [batch_size] - class indices
@group(0) @binding(1) var<storage, read_write> output: array<f32>; // [batch_size, num_classes]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let other_prob = params.smoothing / f32(params.num_classes);
