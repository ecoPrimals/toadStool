// giou_loss.wgsl - Generalized IoU Loss for object detection
//
...
@group(0) @binding(0) var<storage, read> pred_boxes: array<f32>;    // [num_boxes, 4]
@group(0) @binding(1) var<storage, read> target_boxes: array<f32>;  // [num_boxes, 4]
@group(0) @binding(2) var<storage, read_write> output: array<f32>;  // [num_boxes]
...
fn box_to_xyxy(box: vec4<f32>, format: u32) -> vec4<f32> {
    if (format == 0u) {
        ...
        return vec4<f32>(box.x, box.y, box.x + box.z, box.y + box.w);
    } else {
        let half_w = box.z / 2.0;
        let half_h = box.w / 2.0;
        return vec4<f32>(box.x - half_w, box.y - half_h, box.x + half_w, box.y + half_h);
    }
}
