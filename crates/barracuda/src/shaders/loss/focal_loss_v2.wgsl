...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var focal_weight: f32;
    if (targ > 0.5) {
        focal_weight = params.alpha * pow(1.0 - p_t, params.gamma);
    } else {
        focal_weight = (1.0 - params.alpha) * pow(p_t, params.gamma);
    }
    
    // Compute BCE
    let bce = -(targ * log(p) + (1.0 - targ) * log(1.0 - p));
