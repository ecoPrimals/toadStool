// Triplet Loss - Learn embeddings where similar items are close, dissimilar items are far
//
...
@group(0) @binding(0) var<storage, read> anchors: array<f32>;       // [batch, embedding_dim]
@group(0) @binding(1) var<storage, read> positives: array<f32>;     // [batch, embedding_dim]
@group(0) @binding(2) var<storage, read> negatives: array<f32>;     // [batch, embedding_dim]
@group(0) @binding(3) var<storage, read_write> output: array<f32>;  // [batch] - loss per sample
@group(0) @binding(4) var<uniform> params: Params;

struct Params {
    batch_size: u32,
    embedding_dim: u32,
    margin: f32,          // Margin for triplet separation (typically 0.2-1.0)
    distance_type: u32,   // 0=L2 Euclidean, 1=cosine distance
}

// Compute L2 (Euclidean) distance squared between two embeddings
fn l2_distance_squared(idx: u32, dim: u32) -> f32 {
    var sum = 0.0;
...
fn l2_distance_squared_neg(idx: u32, dim: u32) -> f32 {
    var sum = 0.0;
    let base_idx = idx * dim;
...
fn cosine_distance(idx: u32, dim: u32, is_negative: bool) -> f32 {
    var dot_product = 0.0;
    var norm_a = 0.0;
    var norm_other = 0.0;
    let base_idx = idx * dim;
    ...
    let cosine_sim = dot_product / (sqrt(norm_a) * sqrt(norm_other) + 1e-8);
