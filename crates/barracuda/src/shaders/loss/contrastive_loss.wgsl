// Contrastive Loss (NT-Xent) - SimCLR, MoCo style
...
@group(0) @binding(0) var<storage, read> embeddings: array<f32>;  // [batch*2, embed_dim]
@group(0) @binding(1) var<storage, read_write> output: array<f32>;  // [batch_size] losses
...
fn cosine_similarity(idx1: u32, idx2: u32, embed_dim: u32) -> f32 {
    var dot_product = 0.0;
    var norm1 = 0.0;
    var norm2 = 0.0;
    ...
    let epsilon = 1e-8;
    return dot_product / (sqrt(norm1) * sqrt(norm2) + epsilon);
}
