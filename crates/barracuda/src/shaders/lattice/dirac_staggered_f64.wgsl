...
//
// One thread per lattice site. f64 precision throughout.
//
// Data layout (interleaved re/im):
//   Fermion:  psi[site * 6 + color * 2 + 0/1]
//   Links:    links[(site * 4 + mu) * 18 + row * 6 + col * 2 + 0/1]
//   Neighbor: nbr[site * 8 + mu * 2 + 0/1] (forward/backward)
//   Phases:   phases[site * 4 + mu] (±1.0 staggered)
//
// Absorbed from hotSpring v0.6.1 lattice/dirac.rs (Feb 2026)
...
fn dirac(@builtin(global_invocation_id) gid: vec3<u32>) {
    let site = gid.x;
