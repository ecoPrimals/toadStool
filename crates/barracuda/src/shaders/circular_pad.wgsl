// Circular Pad - Pad tensor with circular wrapping
//
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Modulo operation for wrapping (handles negative numbers correctly)
    var in_x = x_offset % i32(params.in_width);
    if (in_x < 0) {
        in_x = in_x + i32(params.in_width);
    }
    
    var in_y = y_offset % i32(params.in_height);
    if (in_y < 0) {
        in_y = in_y + i32(params.in_height);
    }
