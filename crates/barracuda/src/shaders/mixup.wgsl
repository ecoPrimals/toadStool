...
@group(0) @binding(0) var<storage, read> input: array<f32>;        // [batch_size, feature_size]
@group(0) @binding(1) var<storage, read_write> output: array<f32>; // [batch_size, feature_size]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Mixup: λ * x_i + (1 - λ) * x_j
    output[idx1] = params.lambda * input[idx1] + (1.0 - params.lambda) * input[idx2];
}
