...
//
// **f64 precision**: Required for high-order terms in scientific computing
...
fn legendre(n: u32, x: f64) -> f64 {
    ...
    var p_prev: f64 = f64(1.0);  // P₀
    var p_curr: f64 = x;          // P₁

    for (var k = 1u; k < n; k = k + 1u) {
        let k_f64 = f64(k);
        let p_next = ((f64(2.0) * k_f64 + f64(1.0)) * x * p_curr - k_f64 * p_prev) / (k_f64 + f64(1.0));
        p_prev = p_curr;
        p_curr = p_next;
    }
...
fn assoc_legendre(n: u32, m: u32, x: f64) -> f64 {
    if (m > n) {
        return f64(0.0);
    }
    if (m == 0u) {
    ...
    // Compute (1 - x²)^(m/2)
    let sin_sq = f64(1.0) - x * x;
    if (sin_sq <= f64(0.0)) {
        return f64(0.0);  // x = ±1, Pₙᵐ = 0 for m > 0
    }
    let sin_theta_m = pow(sqrt(sin_sq), f64(m));
    ...
    let term_2m1 = 2u * m + 1u;
    var pm1m = x * f64(term_2m1) * pmm;
    ...
    for (var l = m + 2u; l <= n; l = l + 1u) {
        let l_f64 = f64(l);
        let m_f64 = f64(m);
        let pl = ((f64(2.0) * l_f64 - f64(1.0)) * x * pl_minus1 - (l_f64 + m_f64 - f64(1.0)) * pl_minus2) / (l_f64 - m_f64);
        pl_minus2 = pl_minus1;
        pl_minus1 = pl;
    }
