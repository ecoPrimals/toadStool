// Normal distribution CDF Φ(x) = P(X ≤ x) where X ~ N(μ, σ²)
...
struct Params {
    size: u32,
    mu: f32,      // Mean (0.0 for standard normal)
    sigma: f32,   // Standard deviation (1.0 for standard normal)
...
fn std_norm_cdf(z: f32) -> f32 {
    return 0.5 * (1.0 + erf_f32(z / SQRT_2));
}

// Standard normal PDF: φ(z) = exp(-z²/2) / √(2π)
fn std_norm_pdf(z: f32) -> f32 {
    return INV_SQRT_2PI * exp(-0.5 * z * z);
}

// General normal CDF: Φ(x; μ, σ)
fn norm_cdf(x: f32, mu: f32, sigma: f32) -> f32 {
    let z = (x - mu) / sigma;
    return std_norm_cdf(z);
}

// General normal PDF: φ(x; μ, σ)
fn norm_pdf(x: f32, mu: f32, sigma: f32) -> f32 {
    let z = (x - mu) / sigma;
