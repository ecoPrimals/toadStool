...
// Q(a,x) = 1 - P(a,x) is the upper regularized gamma.
//
// Uses series expansion when x < a+1 (rapid convergence); Lentz continued fraction
// when x >= a+1 (avoids cancellation in the series).
//
// Input: pairs [a₀, x₀, a₁, x₁, ...] interleaved (as vec2<u32> for f64)
// Output: [P(a₀,x₀), P(a₁,x₁), ...] at binding(1), [Q(a₀,x₀), ...] at binding(3)
//
// Applications: Chi-squared distributions, Poisson CDF, gamma CDF, exponential
// integral, survival analysis, queuing theory.
// Reference: Abramowitz & Stegun §6.5; Press et al. "Numerical Recipes" Ch. 6
//
...
@group(0) @binding(3) var<storage, read_write> output_q: array<vec2<u32>>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    output[idx] = bitcast<vec2<u32>>(p);
    output_q[idx] = bitcast<vec2<u32>>(q);
}
