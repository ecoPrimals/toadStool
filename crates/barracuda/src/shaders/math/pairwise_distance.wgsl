// pairwise_distance.wgsl - Pairwise Distance
//
...
struct Params {
    num_pairs: u32,
    dim: u32,
    p: f32,         // p-norm (1.0 = L1, 2.0 = L2, etc.)
    epsilon: f32,   // For numerical stability
}

@group(0) @binding(0) var<storage, read> input1: array<f32>;       // [num_pairs, dim]
@group(0) @binding(1) var<storage, read> input2: array<f32>;       // [num_pairs, dim]
@group(0) @binding(2) var<storage, read_write> output: array<f32>; // [num_pairs]
