// Global Pooling - Graph-level representation aggregation
...
@group(0) @binding(1) var<storage, read> node_features: array<f32>;  // [num_nodes, num_features]
@group(0) @binding(2) var<storage, read_write> output: array<f32>;   // [num_features]

// Global pooling kernel
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (params.aggregation_type == 0u) {
        ...
        for (var n = 0u; n < params.num_nodes; n++) {
            sum += node_features[n * params.num_features + feature_idx];
        }
        ...
        output[feature_idx] = sum / f32(params.num_nodes);
