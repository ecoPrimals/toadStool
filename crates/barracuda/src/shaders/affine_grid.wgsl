// affine_grid.wgsl - Affine Grid Generator
//
...
@group(0) @binding(0) var<storage, read> theta: array<f32>;        // [B, 2, 3] affine matrices
@group(0) @binding(1) var<storage, read_write> grid: array<f32>;   // [B, H, W, 2] output grid
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (params.align_corners == 1u) {
        ...
        x = -1.0 + 2.0 * f32(w) / f32(params.width - 1u);
        y = -1.0 + 2.0 * f32(h) / f32(params.height - 1u);
    } else {
        // AlignCorners=false: pixel centers
        x = -1.0 + 2.0 * (f32(w) + 0.5) / f32(params.width);
        y = -1.0 + 2.0 * (f32(h) + 0.5) / f32(params.height);
    }
    
    // Load affine transformation matrix for this batch
    // theta: [B, 2, 3] where each 2x3 matrix is:
    // [a, b, tx]
    // [c, d, ty]
