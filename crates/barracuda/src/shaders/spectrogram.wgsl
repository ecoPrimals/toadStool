...
@group(0) @binding(0) var<storage, read> input: array<f32>;        // [real, imag, real, imag, ...]
@group(0) @binding(1) var<storage, read_write> output: array<f32>; // [magnitude^power]
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Apply power
    output[idx] = pow(magnitude, params.power);
}
