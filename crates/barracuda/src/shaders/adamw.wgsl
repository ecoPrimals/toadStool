...
//         ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^   ^^^^^^^^^^^^^^^^^^^
//         Adam update (gradient-based)                    Weight decay (decoupled)
//
// Used in: BERT, GPT, modern transformers (superior to Adam for large models)

@group(0) @binding(0) var<storage, read> gradients: array<f32>;      // Input gradients
@group(0) @binding(1) var<storage, read_write> params: array<f32>;   // Model parameters to update
@group(0) @binding(2) var<storage, read_write> m: array<f32>;        // First moment (momentum)
@group(0) @binding(3) var<storage, read_write> v: array<f32>;        // Second moment (RMSprop)

struct AdamWParams {
    num_params: u32,
    learning_rate: f32,
    beta1: f32,          // Exponential decay rate for first moment (typically 0.9)
    beta2: f32,          // Exponential decay rate for second moment (typically 0.999)
    epsilon: f32,        // Small constant for numerical stability (typically 1e-8)
    weight_decay: f32,   // Decoupled weight decay (typically 0.01)
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let m_hat = m_val / (1.0 - pow(adamw_params.beta1, f32(adamw_params.step)));
    
    // Compute bias-corrected second moment estimate
    let v_hat = v_val / (1.0 - pow(adamw_params.beta2, f32(adamw_params.step)));
    
    // Adam update (gradient-based)
    let adam_update = adamw_params.learning_rate * m_hat / (sqrt(v_hat) + adamw_params.epsilon);
    
    // Decoupled weight decay (KEY DIFFERENCE from Adam!)
    // Applied AFTER gradient update, making it independent of adaptive LR
