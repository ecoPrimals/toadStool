    ...
    pub fn generate_markdown(&self) -> String {
        ...
        if total_ops > 0 {
            ...
            let avg_parity = if !ops_with_cuda.is_empty() {
                ops_with_cuda.iter().map(|r| r.parity_percent).sum::<f64>() / ops_with_cuda.len() as f64
            } else {
                0.0
            };
            ...
            report.push_str(&format!("| Operations with CUDA comparison | {} |\n", ops_with_cuda.len()));
            report.push_str(&format!("| ≥90% parity | {} ({:.1}%) |\n", ops_at_90, ops_at_90 as f64 / ops_with_cuda.len().max(1) as f64 * 100.0));
            report.push_str(&format!("| ≥95% parity | {} ({:.1}%) |\n", ops_at_95, ops_at_95 as f64 / ops_with_cuda.len().max(1) as f64 * 100.0));
            report.push_str(&format!("| ≥100% parity (faster) | {} ({:.1}%) |\n", ops_at_100, ops_at_100 as f64 / ops_with_cuda.len().max(1) as f64 * 100.0));
