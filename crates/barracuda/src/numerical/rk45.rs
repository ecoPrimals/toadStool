...
mod tests {
    ...
    fn test_lotka_volterra() {
        // Classic predator-prey equations
        // dx/dt = αx - βxy
        // dy/dt = δxy - γy
        let alpha = 1.5;
        let beta = 1.0;
        ...
        // Should remain positive and not explode
    ...
    fn test_step_rejection() {
        // A stiff-ish problem that may require step rejections
        let f = |_t: f64, y: &[f64]| vec![-50.0 * y[0]];
        let config = Rk45Config::new(1e-8, 1e-10).with_h_init(1.0); // Start with large h

        let result = rk45_solve(&f, 0.0, 0.1, &[1.0], &config).unwrap();

        // Should have some rejected steps due to initial large h
        // (might be 0 if initial h gets reduced immediately)
