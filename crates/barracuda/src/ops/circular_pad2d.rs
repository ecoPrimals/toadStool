        ...
        for c in 0..channels {
            for oh in 0..out_h {
                for ow in 0..out_w {
                    ...
                    // Wrap around
                    let ih =
                        ((ih_raw % height as isize + height as isize) % height as isize) as usize;
...
mod tests {
    ...
    async fn test_circular_pad2d_basic() {
        ...
        let output = circular_pad2d(&dev.device, &dev.queue, &input, 1, 1, 2, 2, 1, 1, 1, 1)
            .await
            .unwrap();
    ...
    async fn test_circular_pad2d_edge_cases() {
        ...
        // No padding (no-op)
        let input = vec![1.0, 2.0, 3.0, 4.0];
        let output = circular_pad2d(&dev.device, &dev.queue, &input, 1, 1, 2, 2, 0, 0, 0, 0)
            .await
            .unwrap();
        ...
        // Single pixel input
        let input = vec![5.0];
        let output = circular_pad2d(&dev.device, &dev.queue, &input, 1, 1, 1, 1, 1, 1, 1, 1)
            .await
            .unwrap();
    ...
    async fn test_circular_pad2d_boundary() {
        ...
        let output = circular_pad2d(&dev.device, &dev.queue, &input, 1, 1, 3, 3, 1, 0, 0, 0)
            .await
            .unwrap();
        assert_eq!(output.len(), 1 * 1 * 4 * 3);

        // Asymmetric padding
        let output = circular_pad2d(&dev.device, &dev.queue, &input, 1, 1, 3, 3, 1, 2, 1, 2)
            .await
            .unwrap();
    ...
    async fn test_circular_pad2d_precision() {
        ...
        let output = circular_pad2d(&dev.device, &dev.queue, &input, 1, 1, 2, 2, 1, 0, 1, 0)
            .await
            .unwrap();
