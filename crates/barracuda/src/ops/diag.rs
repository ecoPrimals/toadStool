...
mod tests {
    ...
    async fn test_diag_edge_cases() {
        ...
        let diag_1x1 = diag(&dev.device, &dev.queue, &matrix_1x1, DiagMode::Extract, 1).await.unwrap();
        assert_eq!(diag_1x1, vec![42.0]);
        
        // Construct 1x1
        let vec_1 = vec![99.0];
        let matrix_1 = diag(&dev.device, &dev.queue, &vec_1, DiagMode::Construct, 1).await.unwrap();
        assert_eq!(matrix_1, vec![99.0]);
        
        // Extract and reconstruct should be inverse
        let original_diag = vec![1.0, 2.0, 3.0];
        let matrix = diag(&dev.device, &dev.queue, &original_diag, DiagMode::Construct, 3).await.unwrap();
        let extracted = diag(&dev.device, &dev.queue, &matrix, DiagMode::Extract, 3).await.unwrap();
    ...
    async fn test_diag_boundary() {
        ...
        let matrix = diag(&dev.device, &dev.queue, &diag_vals, DiagMode::Construct, size).await.unwrap();
        ...
        // Extract back
        let extracted = diag(&dev.device, &dev.queue, &matrix, DiagMode::Extract, size).await.unwrap();
    ...
    async fn test_diag_precision() {
        ...
        let matrix = diag(&dev.device, &dev.queue, &precise_diag, DiagMode::Construct, 5).await.unwrap();
        
        assert_eq!(matrix.len(), 25);
        
        // Check precision of diagonal
        for (i, &val) in precise_diag.iter().enumerate() {
            assert!((matrix[i * 5 + i] - val).abs() < 1e-6, 
                   "Diagonal value {} mismatch", i);
        }
        
        // Extract and verify precision maintained
        let extracted = diag(&dev.device, &dev.queue, &matrix, DiagMode::Extract, 5).await.unwrap();
        for (i, (&orig, &ext)) in precise_diag.iter().zip(extracted.iter()).enumerate() {
            assert!((orig - ext).abs() < 1e-6,
                   "Precision loss at index {}: {} vs {}", i, orig, ext);
        }
    }
}
