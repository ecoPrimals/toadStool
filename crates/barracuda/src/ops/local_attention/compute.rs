//! GPU compute operations for Local Attention
//!
//! This module contains the 3-pass GPU execution:
//! 1. Pass 1: Compute QK^T scores (matmul)
//! 2. Pass 2: Apply softmax with local window mask
//! 3. Pass 3: Apply weights to values

use super::{LocalAttention, LocalAttentionParams};
...
impl LocalAttention {
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        ComputeDispatch::new(device, "local_attn_matmul")
        ...
        ComputeDispatch::new(device, "local_attn_softmax")
            .shader(Self::shader_local_softmax(), "main")
        ...
        ComputeDispatch::new(device, "local_attn_apply")
