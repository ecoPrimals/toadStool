...
mod tests {
    ...
    async fn test_adaptive_max_pool1d_edge_cases() {
        ...
        let output = adaptive_max_pool1d(&dev.device, &dev.queue, &input, 1, 1, 4, 1).await.unwrap();
        ...
        let output = adaptive_max_pool1d(&dev.device, &dev.queue, &input, 1, 1, 4, 4).await.unwrap();
    ...
    async fn test_adaptive_max_pool1d_boundary() {
        ...
        let output = adaptive_max_pool1d(&dev.device, &dev.queue, &input, 1, 1, 32, 4).await.unwrap();
        assert_eq!(output.len(), 4);
        assert!(output.iter().all(|&x| x.is_finite()));
        ...
        let output = adaptive_max_pool1d(&dev.device, &dev.queue, &input, 1, 1, 4, 2).await.unwrap();
    ...
    async fn test_adaptive_max_pool1d_large_batch() {
        ...
        let input: Vec<f32> = (0..batch_size * channels * length)
            ...
            .collect();
        let output = adaptive_max_pool1d(&dev.device, &dev.queue, &input, batch_size, channels, length, output_length).await.unwrap();
    ...
    async fn test_adaptive_max_pool1d_precision() {
        ...
        let output = adaptive_max_pool1d(&dev.device, &dev.queue, &input, 1, 2, 4, 2).await.unwrap();
