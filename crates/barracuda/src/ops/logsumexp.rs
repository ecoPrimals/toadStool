...
mod tests {
    ...
    async fn test_logsumexp_basic() {
        ...
        let output = logsumexp(&dev.device, &dev.queue, &input, 0, &[4])
            .await
            .unwrap();
    ...
    async fn test_logsumexp_edge_cases() {
        ...
        // Single element
        let input = vec![5.0];
        let output = logsumexp(&dev.device, &dev.queue, &input, 0, &[1])
            .await
            .unwrap();
        assert_eq!(output.len(), 1);
        ...
        let output = logsumexp(&dev.device, &dev.queue, &input, 0, &[3])
            .await
            .unwrap();
        assert!(output[0].is_finite());
    }
    ...
    async fn test_logsumexp_large_batch() {
        ...
        let output = logsumexp(&dev.device, &dev.queue, &input, 0, &[1000])
            .await
            .unwrap();
        assert_eq!(output.len(), 1);
        assert!(output[0].is_finite());
    ...
    async fn test_logsumexp_precision() {
        ...
        let output = logsumexp(&dev.device, &dev.queue, &input, 1, &[2, 3])
            .await
            .unwrap();
