...
impl CircularPad {
    ...
    fn wgsl_shader() -> &'static str {
        {
            static S: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
                crate::shaders::precision::downcast_f64_to_f32_with_transcendentals(include_str!(
                    "../shaders/tensor/circular_pad_f64.wgsl"
                ))
            });
            &S
        }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        let bind_group_layout =
            device
                ...
                .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                    label: Some("CircularPad Bind Group Layout"),
                    entries: &[
        ...
        let bind_group = device.device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("CircularPad Bind Group"),
        ...
        let pipeline_layout =
            device
                ...
                .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    label: Some("CircularPad Pipeline Layout"),
        ...
        let compute_pipeline =
            device
                ...
                .create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                    label: Some("CircularPad Pipeline"),
        ...
        let mut encoder = device
            ...
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("CircularPad Encoder"),
            });

        {
            let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("CircularPad Pass"),
...
impl Tensor {
    /// Apply circular padding (NCHW format)
    ///
    /// # Arguments
    ///
...
mod tests {
    ...
    async fn test_circular_pad_wrapping() {
        ...
        assert_eq!(output.shape(), &[1, 1, 3, 4]);
        assert_eq!(output.to_vec().unwrap().len(), 12);

        // First column should be wrapped from the last column
        let result = output.to_vec().unwrap();
        // Row 0: [3, 1, 2, 3]
        assert_eq!(result[0], 3.0); // Wrapped from right
