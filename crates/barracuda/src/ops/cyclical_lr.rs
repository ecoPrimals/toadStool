...
mod tests {
    ...
    async fn test_cyclical_lr_basic() {
        ...
        let lr_at_2000 = cyclical_lr(&dev.device, &dev.queue, 0.001, 0.006, 2000, 2000, CyclicalPolicy::Triangular).await.unwrap();
        assert!(lr_at_2000 >= 0.001 && lr_at_2000 <= 0.006);
        
        // At base (0)
        let lr_base = cyclical_lr(&dev.device, &dev.queue, 0.001, 0.006, 2000, 0, CyclicalPolicy::Triangular).await.unwrap();
        assert!(lr_base >= 0.001 && lr_base <= 0.006, "LR should be in valid range");
    }
    ...
    async fn test_cyclical_lr_edge_cases() {
        ...
        // Zero step
        let lr = cyclical_lr(&dev.device, &dev.queue, 0.0001, 0.001, 1000, 0, CyclicalPolicy::Triangular).await.unwrap();
        assert!(lr >= 0.0001 && lr <= 0.001);
        
        // Very small learning rates
        let lr_small = cyclical_lr(&dev.device, &dev.queue, 1e-6, 1e-5, 100, 50, CyclicalPolicy::Triangular).await.unwrap();
        assert!(lr_small >= 1e-6 && lr_small <= 1e-5);
        
        // Step at step_size
        let lr_at_size = cyclical_lr(&dev.device, &dev.queue, 0.01, 0.1, 500, 500, CyclicalPolicy::Triangular).await.unwrap();
    ...
    async fn test_cyclical_lr_boundary() {
        ...
        let lr_tri = cyclical_lr(&dev.device, &dev.queue, 0.001, 0.01, 1000, 1000, CyclicalPolicy::Triangular).await.unwrap();
        assert!(lr_tri >= 0.001 && lr_tri <= 0.01);
        
        let lr_tri2 = cyclical_lr(&dev.device, &dev.queue, 0.001, 0.01, 1000, 3000, CyclicalPolicy::Triangular2).await.unwrap();
        assert!(lr_tri2 >= 0.001 && lr_tri2 <= 0.01);
        
        // Test ExpRange policy
        let lr_exp = cyclical_lr(&dev.device, &dev.queue, 0.001, 0.01, 1000, 500, CyclicalPolicy::ExpRange(0.99)).await.unwrap();
    ...
    async fn test_cyclical_lr_large_steps() {
        ...
        for &step in &steps {
            let lr = cyclical_lr(&dev.device, &dev.queue, base, max, step_size, step, CyclicalPolicy::Triangular).await.unwrap();
            assert!(lr >= base && lr <= max, "LR at step {} out of bounds: {}", step, lr);
        }
        
        // Verify periodicity: step 0 and step 20000 should have same LR
        let lr0 = cyclical_lr(&dev.device, &dev.queue, base, max, step_size, 0, CyclicalPolicy::Triangular).await.unwrap();
        let lr_cycle = cyclical_lr(&dev.device, &dev.queue, base, max, step_size, 2 * 2 * step_size, CyclicalPolicy::Triangular).await.unwrap();
    ...
    async fn test_cyclical_lr_precision() {
        ...
        let lr_tri = cyclical_lr(&dev.device, &dev.queue, base, max, step_size, test_step, CyclicalPolicy::Triangular).await.unwrap();
        assert!((lr_tri - 0.055).abs() < 1e-3, "Triangular should be midpoint");
        
        // Triangular2: same as Triangular for first cycle
        let lr_tri2 = cyclical_lr(&dev.device, &dev.queue, base, max, step_size, test_step, CyclicalPolicy::Triangular2).await.unwrap();
        assert!((lr_tri - lr_tri2).abs() < 1e-5, "Triangular2 first cycle same as Triangular");
        
        // ExpRange: decaying over time
        let lr_exp = cyclical_lr(&dev.device, &dev.queue, base, max, step_size, test_step, CyclicalPolicy::ExpRange(0.995)).await.unwrap();
