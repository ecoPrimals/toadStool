...
mod tests {
    ...
    async fn test_put_basic() {
        ...
        let output = put(&dev.device, &dev.queue, 5, &indices, &values).await.unwrap();
    ...
    async fn test_put_boundary() {
        ...
        let output = put(&dev.device, &dev.queue, 3, &indices, &values).await.unwrap();
    ...
    async fn test_put_large_batch() {
        ...
        let output = put(&dev.device, &dev.queue, 1000, &indices, &values).await.unwrap();
    ...
    async fn test_put_precision() {
        ...
        let output = put(&dev.device, &dev.queue, 7, &indices, &values).await.unwrap();
