...
mod tests {
    ...
    async fn test_narrow_edge_cases() {
        ...
        let output = narrow(&dev.device, &dev.queue, &input, 0, 1, 1, &[3]).await.unwrap();
        assert_eq!(output, vec![2.0]);

        // Full range (start=0, length=all)
        let input: Vec<f32> = (0..5).map(|i| i as f32).collect();
        let output = narrow(&dev.device, &dev.queue, &input, 0, 0, 5, &[5]).await.unwrap();
    ...
    async fn test_narrow_boundary() {
        ...
        // Start at end
        let input: Vec<f32> = (0..10).map(|i| i as f32).collect();
        let output = narrow(&dev.device, &dev.queue, &input, 0, 8, 2, &[10]).await.unwrap();
        assert_eq!(output, vec![8.0, 9.0]);

        // Multi-dimensional along different dim
        let input: Vec<f32> = (0..24).map(|i| i as f32).collect();
        let output = narrow(&dev.device, &dev.queue, &input, 1, 1, 2, &[2, 3, 4]).await.unwrap();
    ...
    async fn test_narrow_large_batch() {
        ...
        let output = narrow(&dev.device, &dev.queue, &input, 0, 100, 500, &[1000]).await.unwrap();
    ...
    async fn test_narrow_precision() {
        ...
        let output = narrow(&dev.device, &dev.queue, &input, 0, 1, 3, &[5]).await.unwrap();
