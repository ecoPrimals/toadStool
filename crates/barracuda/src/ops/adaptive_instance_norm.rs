...
mod tests {
    ...
    async fn test_adaptive_instance_norm_edge_cases() {
        ...
        let output = adaptive_instance_norm(&dev.device, &dev.queue, &content, &style_mean, &style_std, 1, 1, 2, 2).await.unwrap();
        assert!(output.iter().all(|&x| x.is_finite()));
        ...
        let output = adaptive_instance_norm(&dev.device, &dev.queue, &content, &style_mean, &style_std, 1, 1, 1, 1).await.unwrap();
    ...
    async fn test_adaptive_instance_norm_boundary() {
        ...
        let output1 = adaptive_instance_norm(&dev.device, &dev.queue, &content, &vec![0.0], &vec![1.0], 1, 1, 2, 2).await.unwrap();
        
        // Style 2: mean=10, std=5
        let output2 = adaptive_instance_norm(&dev.device, &dev.queue, &content, &vec![10.0], &vec![5.0], 1, 1, 2, 2).await.unwrap();
    ...
    async fn test_adaptive_instance_norm_large_batch() {
        ...
        let output = adaptive_instance_norm(&dev.device, &dev.queue, &content, &style_mean, &style_std, batch_size, channels, height, width).await.unwrap();
