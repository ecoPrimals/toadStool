...
mod tests {
    ...
    async fn test_embedding_basic() {
        ...
        let embeddings = Tensor::from_vec_on(embed_data.clone(), vec![3, 4], device)
            .await
            .unwrap();
    ...
    async fn test_embedding_edge_cases() {
        ...
        let embeddings = Tensor::from_vec_on(embed_data.clone(), vec![2, 3], device.clone())
            .await
            .unwrap();
        let indices = vec![0];
        let result = embeddings.embedding(indices.clone()).unwrap();
    ...
    async fn test_embedding_boundary() {
        ...
        let embeddings = Tensor::from_vec_on(embed_data.clone(), vec![1, 1], device.clone())
            .await
            .unwrap();
        let indices = vec![0];
        let result = embeddings.embedding(indices.clone()).unwrap();
    ...
    async fn test_embedding_large_tensor() {
        ...
        let embed_data: Vec<f32> = (0..vocab_size * embed_dim)
            ...
            .collect();
        let embeddings =
            Tensor::from_vec_on(embed_data.clone(), vec![vocab_size, embed_dim], device)
                .await
                .unwrap();
    ...
    async fn test_embedding_precision() {
        ...
        let embed_data = vec![
            0.123, -0.456, 0.789, -0.234, 0.567, -0.890, 0.345, -0.678, 0.901, -0.123, 0.456,
            -0.789,
        ];
