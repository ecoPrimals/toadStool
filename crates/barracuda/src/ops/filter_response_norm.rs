...
mod tests {
    ...
    async fn test_filter_response_norm_edge_cases() {
        ...
        let gamma = vec![1.0; 2];
        let beta = vec![0.0; 2];
        let output = filter_response_norm(&dev.device, &dev.queue, &zeros, &gamma, &beta, 1, 2, 3, 3, 1e-5).await.unwrap();
        ...
        let gamma = vec![1.0; 2];
        let beta = vec![0.5; 2];
        let output = filter_response_norm(&dev.device, &dev.queue, &input, &gamma, &beta, 1, 2, 4, 4, 1e-5).await.unwrap();
    ...
    async fn test_filter_response_norm_boundary() {
        ...
        let gamma = vec![1.5];
        let beta = vec![0.1];
        let output = filter_response_norm(&dev.device, &dev.queue, &input, &gamma, &beta, 1, 1, 8, 8, 1e-5).await.unwrap();
        ...
        let gamma = vec![1.0; 3];
        let beta = vec![0.0; 3];
        let output = filter_response_norm(&dev.device, &dev.queue, &input, &gamma, &beta, 2, 3, 4, 4, 1e-5).await.unwrap();
    ...
    async fn test_filter_response_norm_large_tensor() {
        ...
        let output = filter_response_norm(&dev.device, &dev.queue, &input, &gamma, &beta, batch, channels, size, size, 1e-5).await.unwrap();
