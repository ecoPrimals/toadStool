...
impl SinkhornDistance {
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        // Compile shader
        let shader_module = device.compile_shader(Self::wgsl_shader(), Some("SinkhornDistance Shader"));
...
mod tests {
    ...
    async fn test_sinkhorn_distance_basic() {
        ...
        let cost_matrix = Tensor::from_vec_on(
            vec![1.0; size * size],
            vec![size, size],
            device.clone(),
        )
