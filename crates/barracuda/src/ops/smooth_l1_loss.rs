...
mod tests {
    ...
    async fn test_smooth_l1_loss_boundary() {
        ...
        let loss_beta1 = smooth_l1_loss(&dev.device, &dev.queue, &preds, &targets, 1.0).await.unwrap();
        let loss_beta2 = smooth_l1_loss(&dev.device, &dev.queue, &preds, &targets, 2.0).await.unwrap();
    ...
    async fn test_smooth_l1_loss_precision() {
        ...
        let loss = smooth_l1_loss(&dev.device, &dev.queue, &preds, &targets, beta).await.unwrap();
        assert!(loss > 0.0 && loss.is_finite());
        
        // Verify loss is smooth (no discontinuities)
        let loss_small = smooth_l1_loss(&dev.device, &dev.queue, &vec![0.0], &vec![0.99], beta).await.unwrap();
        let loss_large = smooth_l1_loss(&dev.device, &dev.queue, &vec![0.0], &vec![1.01], beta).await.unwrap();
