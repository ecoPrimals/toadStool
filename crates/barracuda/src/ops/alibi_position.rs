...
mod tests {
    ...
    async fn test_alibi_position_edge_cases() {
        ...
        let scores = vec![5.0];
        let output = alibi_position(&dev.device, &dev.queue, &scores, 1, 1, 1).await.unwrap();
        ...
        let output = alibi_position(&dev.device, &dev.queue, &scores, 1, 1, 4).await.unwrap();
        assert!(output.iter().all(|&x| x.is_finite()));
    }
    ...
    async fn test_alibi_position_boundary() {
        ...
        let output1 = alibi_position(&dev.device, &dev.queue, &scores1, 1, 1, 4).await.unwrap();
        
        let scores2 = vec![0.0; 1 * 4 * 4 * 4];
        let output2 = alibi_position(&dev.device, &dev.queue, &scores2, 1, 4, 4).await.unwrap();
    ...
    async fn test_alibi_position_large_batch() {
        ...
        let output = alibi_position(&dev.device, &dev.queue, &scores, batch_size, num_heads, seq_len).await.unwrap();
    ...
    async fn test_alibi_position_precision() {
        ...
        let output = alibi_position(&dev.device, &dev.queue, &scores, 1, 1, 3).await.unwrap();
