...
mod tests {
    ...
    async fn test_replication_pad2d_edge_cases() {
        ...
        let output = replication_pad2d(&dev.device, &dev.queue, &input, 1, 1, 4, 4, 0, 0, 0, 0).await.unwrap();
        assert_eq!(output, input);

        // Single pixel
        let input = vec![5.0];
        let output = replication_pad2d(&dev.device, &dev.queue, &input, 1, 1, 1, 1, 2, 2, 2, 2).await.unwrap();
    ...
    async fn test_replication_pad2d_boundary() {
        ...
        let output = replication_pad2d(&dev.device, &dev.queue, &input, 1, 3, 4, 4, 3, 3, 3, 3).await.unwrap();
        assert_eq!(output.len(), 1 * 3 * 10 * 10);

        // Asymmetric padding
        let input = vec![1.0; 1 * 1 * 8 * 8];
        let output = replication_pad2d(&dev.device, &dev.queue, &input, 1, 1, 8, 8, 2, 3, 1, 4).await.unwrap();
    ...
    async fn test_replication_pad2d_large_batch() {
        ...
        let output = replication_pad2d(&dev.device, &dev.queue, &input, batch_size, channels, 16, 16, 2, 2, 2, 2).await.unwrap();
