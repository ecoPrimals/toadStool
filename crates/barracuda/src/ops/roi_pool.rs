...
mod tests {
    ...
    async fn test_roi_pool_edge_cases() {
        ...
        let output = roi_pool(&dev.device, &dev.queue, &features, &rois, 1, 2, 4, 4, 2, 2).await.unwrap();
        assert_eq!(output.len(), 1 * 2 * 2 * 2);
        
        // Small RoI
        let rois = vec![RoI { batch_idx: 0, x1: 0.0, y1: 0.0, x2: 0.25, y2: 0.25 }];
        let output = roi_pool(&dev.device, &dev.queue, &features, &rois, 1, 2, 4, 4, 2, 2).await.unwrap();
        assert!(output.iter().all(|&x| x.is_finite()));
    }
    ...
    async fn test_roi_pool_boundary() {
        ...
        let output = roi_pool(&dev.device, &dev.queue, &features, &rois, 1, 1, 8, 8, 2, 2).await.unwrap();
        assert_eq!(output.len(), 1 * 1 * 2 * 2);
        
        // Different output sizes
        let rois = vec![RoI { batch_idx: 0, x1: 0.0, y1: 0.0, x2: 0.5, y2: 0.5 }];
        let output = roi_pool(&dev.device, &dev.queue, &features, &rois, 1, 1, 8, 8, 4, 4).await.unwrap();
    ...
    async fn test_roi_pool_large_batch() {
        ...
        let output = roi_pool(&dev.device, &dev.queue, &features, &rois, 2, 3, 16, 16, 7, 7).await.unwrap();
    ...
    async fn test_roi_pool_precision() {
        ...
        let rois = vec![RoI { batch_idx: 0, x1: 0.0, y1: 0.0, x2: 1.0, y2: 1.0 }];
        let output = roi_pool(&dev.device, &dev.queue, &features, &rois, 1, 1, 4, 4, 2, 2).await.unwrap();
