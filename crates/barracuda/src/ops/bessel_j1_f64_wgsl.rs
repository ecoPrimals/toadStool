...
impl BesselJ1F64 {
    ...
    fn j1_scalar(x: f64) -> f64 {
        ...
        if ax >= 8.0 {
            ...
            let p1 = 1.0 + 1.83105e-3 * z2 - 3.516396496e-4 * z4
                + 2.457520174e-5 * z6 - 2.40337019e-6 * z8;

            let q1 = 4.687499995e-2 * z - 2.002690873e-4 * z * z2
                + 8.449199096e-6 * z * z4 - 8.8228987e-7 * z * z6
                + 1.057874120e-7 * z * z8;
            ...
            let p = 72362614232.0 - 7895059235.0 * z + 242396853.1 * z2
                - 2972611.439 * z3 + 15704.48260 * z4 - 30.16036606 * z5;

            let q = 144725228442.0 + 2300535178.0 * z + 18583304.74 * z2
                + 99447.43394 * z3 + 376.9991397 * z4 + z5;
    ...
    fn j1_gpu(&self, x: &[f64]) -> Result<Vec<f64>> {
        ...
        let shader_module = self.device.compile_shader(Self::wgsl_shader(), Some("Bessel J1 f64"));
        ...
        let mut encoder = self.device.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Bessel J1 f64 Encoder") });
        {
...
mod tests {
    ...
    fn test_j1_at_zero() -> Result<()> {
        ...
        assert!((result[0]).abs() < 1e-10, "J₁(0) = {}, expected 0", result[0]);
        Ok(())
    }
    ...
    fn test_j1_known_values() -> Result<()> {
        ...
        for (i, &val) in result.iter().enumerate() {
            assert!((val - expected[i]).abs() < 1e-6, "J₁({}) = {}, expected {}", x[i], val, expected[i]);
        }
        Ok(())
    }
