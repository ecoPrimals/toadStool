    ...
    pub fn new(input: Tensor, slice_range: SliceRange, values: Tensor) -> Result<Self> {
        ...
        if slice_range.start >= slice_range.end {
            return Err(BarracudaError::invalid_op(
                "SliceAssign",
                format!("Start {} must be less than end {}", slice_range.start, slice_range.end),
            ));
        }
        ...
        let slice_size = (slice_range.end - slice_range.start + slice_range.stride - 1) / slice_range.stride;
        if values_size != slice_size {
            return Err(BarracudaError::invalid_op(
                "SliceAssign",
                format!("Values size {} doesn't match slice size {}", values_size, slice_size),
            ));
        }
