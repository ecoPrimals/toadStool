...
mod tests {
    ...
    async fn test_soft_nms_boundary() {
        ...
        let mut boxes = vec![
            BoundingBox { x1: 0.0, y1: 0.0, x2: 10.0, y2: 10.0, score: 0.9 },
            BoundingBox { x1: 0.5, y1: 0.5, x2: 10.5, y2: 10.5, score: 0.85 },
        ];
        let keep = soft_nms(&dev.device, &dev.queue, &mut boxes, 0.5, 0.5).await.unwrap();
        assert!(keep.len() >= 1);
        ...
        let keep = soft_nms(&dev.device, &dev.queue, &mut boxes, 0.5, 0.3).await.unwrap();
        assert!(keep.len() >= 1);
    }
    ...
    async fn test_soft_nms_precision() {
        ...
        let mut boxes = vec![
            BoundingBox { x1: 0.0, y1: 0.0, x2: 10.0, y2: 10.0, score: 0.9 },
            BoundingBox { x1: 2.0, y1: 2.0, x2: 12.0, y2: 12.0, score: 0.8 },
        ];
