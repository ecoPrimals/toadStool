...
mod tests {
    ...
    async fn test_index_select_edge_cases() {
        ...
        let indices = vec![1];
        let output = index_select(&dev.device, &dev.queue, &input, &indices, 0, &[3, 4]).await.unwrap();
    ...
    async fn test_index_select_boundary() {
        ...
        let indices = vec![0, 2];
        let output = index_select(&dev.device, &dev.queue, &input, &indices, 1, &[2, 3, 4]).await.unwrap();
        assert!(output.len() > 0);
    ...
    async fn test_index_select_large_batch() {
        ...
        let output = index_select(&dev.device, &dev.queue, &input, &indices, 0, &[100, 10]).await.unwrap();
    ...
    async fn test_index_select_precision() {
        ...
        let output = index_select(&dev.device, &dev.queue, &input, &indices, 0, &[2, 4]).await.unwrap();
