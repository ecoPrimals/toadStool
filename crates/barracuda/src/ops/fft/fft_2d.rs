...
impl Fft2D {
    pub fn new(input: Tensor, rows: u32, cols: u32) -> Result<Self> {
        ...
        if shape[0] != rows as usize || shape[1] != cols as usize || shape[2] != 2 {
            return Err(BarracudaError::Device(
                format!("FFT 2D shape mismatch: expected [{}, {}, 2], got {:?}", rows, cols, shape),
            ));
        }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        for row_idx in 0..self.rows {
            ...
            let row_tensor = Tensor::from_data(&row_data, vec![self.cols as usize, 2], device.clone())?;
            let fft = Fft1D::new(row_tensor, self.cols)?;
            let row_fft = fft.execute()?;
            row_results.extend(row_fft.to_vec()?);
        }
        
        let rows_transformed = Tensor::from_data(&row_results, vec![self.rows as usize, self.cols as usize, 2], device.clone())?;
        ...
        for col_idx in 0..self.cols {
            ...
            let col_tensor = Tensor::from_data(&col_data, vec![self.rows as usize, 2], device.clone())?;
        ...
        Ok(Tensor::from_data(&final_data, vec![self.rows as usize, self.cols as usize, 2], device.clone())?)
    }
