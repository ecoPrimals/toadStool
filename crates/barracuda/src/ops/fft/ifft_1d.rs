...
impl Ifft1D {
    ...
    pub fn new(input: Tensor, degree: u32) -> Result<Self> {
        ...
        if degree & (degree - 1) != 0 {
            return Err(BarracudaError::Device(
                format!("IFFT degree {} must be power of 2", degree),
            ));
        }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        let normalize_params_buffer = device.device.create_buffer_init(
            &wgpu::util::BufferInitDescriptor {
        ...
        let mut normalize_encoder = device
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
        ...
        {
            let mut compute_pass = normalize_encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
        ...
        device.queue.submit(std::iter::once(normalize_encoder.finish()));
