...
mod tests {
    ...
    async fn test_repeat_interleave_edge_cases() {
        ...
        // Single element
        let input = vec![5.0];
        let output = repeat_interleave(&dev.device, &dev.queue, &input, 3).await.unwrap();
        assert_eq!(output, vec![5.0, 5.0, 5.0]);

        // Repeat once (identity)
        let input = vec![1.0, 2.0];
        let output = repeat_interleave(&dev.device, &dev.queue, &input, 1).await.unwrap();
    ...
    async fn test_repeat_interleave_boundary() {
        ...
        // Large repeat count
        let input = vec![1.0];
        let output = repeat_interleave(&dev.device, &dev.queue, &input, 100).await.unwrap();
    ...
    async fn test_repeat_interleave_large_batch() {
        ...
        let output = repeat_interleave(&dev.device, &dev.queue, &input, 5).await.unwrap();
