...
mod tests {
    ...
    async fn test_center_loss_edge_cases() {
        ...
        let loss = center_loss(&dev.device, &dev.queue, &features, &labels, &centers, 4, 10, 2).await.unwrap();
        ...
        let loss = center_loss(&dev.device, &dev.queue, &features, &labels, &centers, 1, 8, 1).await.unwrap();
        assert!(loss > 0.0);
    }
    ...
    async fn test_center_loss_boundary() {
        ...
        let loss = center_loss(&dev.device, &dev.queue, &features, &labels, &centers, 8, 5, 4).await.unwrap();
        assert!(loss > 0.0);
        assert!(loss.is_finite());
    }
    ...
    async fn test_center_loss_large_batch() {
        ...
        let loss = center_loss(&dev.device, &dev.queue, &features, &labels, &centers, batch_size, feature_dim, num_classes).await.unwrap();
        assert!(loss > 0.0);
        assert!(loss.is_finite());
    }
    ...
    async fn test_center_loss_precision() {
        ...
        let loss = center_loss(&dev.device, &dev.queue, &features, &labels, &centers, 2, 2, 2).await.unwrap();
