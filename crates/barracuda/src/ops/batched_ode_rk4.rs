...
impl BatchedOdeRK4F64 {
    ...
    pub fn integrate(&self, initial_states: &[f64], batch_params: &[f64]) -> Result<Vec<f64>> {
        ...
        let cfg_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let init_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label:    Some("BatchedOdeRK4 InitStates"),
            contents: bytemuck::cast_slice(initial_states),
            usage:    wgpu::BufferUsages::STORAGE,
        });

        let param_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let mut encoder = dev.device.create_command_encoder(
            &wgpu::CommandEncoderDescriptor { label: Some("BatchedOdeRK4 Encoder") },
        );
        {
