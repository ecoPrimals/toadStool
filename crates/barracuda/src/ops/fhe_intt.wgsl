// FHE Inverse Number Theoretic Transform (INTT) - U64 Emulation Version
//
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Load twiddle factor
    // For INTT: twiddle_idx = local_idx * (N / (2 * stride))
...
/// Scale by N^(-1) kernel (for INTT final step)
@compute @workgroup_size(256)
fn scale_by_n(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let idx = global_id.x;
    ...
    // Get modulus and inverse N
    let q = u64_from_parts(params.modulus_lo, params.modulus_hi);
    let inv_n = u64_from_parts(params.root_of_unity_lo, params.root_of_unity_hi); // Reuse field for inv_n
    let mu = u64_from_parts(params.barrett_mu_lo, params.barrett_mu_hi);
    
    // Multiply by inv_n mod q
    let scaled = mod_mul_u64(value, inv_n, q);
    
    // Store result
    store_u64_to_output(idx, scaled);
}
