    ...
    for b in 0..batch_size {
        for oh in 0..out_height {
            for ow in 0..out_width {
                ...
                for c in 0..channels {
                    let idx00 = b * channels * in_height * in_width
                        + c * in_height * in_width
                        + py0 * in_width
                        + px0;
                    let idx01 = b * channels * in_height * in_width
                        + c * in_height * in_width
                        + py0 * in_width
                        + px1;
                    let idx10 = b * channels * in_height * in_width
                        + c * in_height * in_width
                        + py1 * in_width
                        + px0;
                    let idx11 = b * channels * in_height * in_width
                        + c * in_height * in_width
                        + py1 * in_width
                        + px1;

                    let v00 = if px0 < in_width && py0 < in_height {
                        input[idx00]
                    } else {
                        0.0
                    };
                    let v01 = if px1 < in_width && py0 < in_height {
                        input[idx01]
                    } else {
                        0.0
                    };
                    let v10 = if px0 < in_width && py1 < in_height {
                        input[idx10]
                    } else {
                        0.0
                    };
                    let v11 = if px1 < in_width && py1 < in_height {
                        input[idx11]
                    } else {
                        0.0
                    };
...
mod tests {
    ...
    async fn test_grid_sample_basic() {
        ...
        let output = grid_sample(&dev.device, &dev.queue, &input, &grid, 1, 3, 4, 4, 2, 2)
            .await
            .unwrap();
    ...
    async fn test_grid_sample_edge_cases() {
        ...
        let output = grid_sample(&dev.device, &dev.queue, &input, &grid, 1, 1, 4, 4, 2, 2)
            .await
            .unwrap();
        ...
        let output = grid_sample(&dev.device, &dev.queue, &input, &grid, 1, 3, 4, 4, 1, 1)
            .await
            .unwrap();
    ...
    async fn test_grid_sample_boundary() {
        ...
        let output = grid_sample(&dev.device, &dev.queue, &input, &grid, 1, 3, 4, 4, 8, 8)
            .await
            .unwrap();
    ...
    async fn test_grid_sample_precision() {
        ...
        let output = grid_sample(&dev.device, &dev.queue, &input, &grid, 1, 1, 2, 2, 1, 1)
            .await
            .unwrap();
