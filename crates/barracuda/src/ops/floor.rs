...
pub struct Floor { input: Tensor }

impl Floor {
    pub fn new(input: Tensor) -> Self { Self { input } }
    fn wgsl_shader() -> &'static str { include_str!("../shaders/floor.wgsl") }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        let mut encoder = device.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Floor Encoder") });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Floor Pass"), timestamp_writes: None });
...
impl Tensor {
    pub fn floor(self) -> Result<Self> { Floor::new(self).execute() }
}
