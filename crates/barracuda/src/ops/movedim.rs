...
mod tests {
    ...
    async fn test_movedim_edge_cases() {
        ...
        let input = vec![1.0; 2 * 3 * 4];
        let output = movedim(&dev.device, &dev.queue, &input, 1, 1, &[2, 3, 4]).await.unwrap();
        assert_eq!(output, input);

        // Single dimension
        let input = vec![1.0; 10];
        let output = movedim(&dev.device, &dev.queue, &input, 0, 0, &[10]).await.unwrap();
    ...
    async fn test_movedim_boundary() {
        ...
        let output = movedim(&dev.device, &dev.queue, &input, 0, 3, &[2, 3, 4, 5]).await.unwrap();
        assert_eq!(output.len(), input.len());

        // Move last to first
        let output = movedim(&dev.device, &dev.queue, &input, 3, 0, &[2, 3, 4, 5]).await.unwrap();
        assert_eq!(output.len(), input.len());
    }
    ...
    async fn test_movedim_large_tensor() {
        ...
        let output = movedim(&dev.device, &dev.queue, &input, 1, 3, &[2, 3, 4, 5, 6]).await.unwrap();
        assert_eq!(output.len(), input.len());
    }
    ...
    async fn test_movedim_precision() {
        ...
        let output = movedim(&dev.device, &dev.queue, &input, 0, 1, &[2, 3, 4]).await.unwrap();
