...
impl Default for GillespieConfig {
    fn default() -> Self {
        Self { t_max: 100.0, max_steps: 100_000 }
    }
}
...
impl GillespieGpu {
    ...
    pub fn simulate(
        ...
        assert_eq!(initial_states.len(), n_t * n_s, "initial_states must be [T×S]");
        assert_eq!(prng_seeds.len(),     n_t * 4,   "prng_seeds must be [T×4]");

        // ── Upload read-only buffers ───────────────────────────────────────────
        let rate_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Gillespie rates"),
            contents: bytemuck::cast_slice(rate_k),
            usage: wgpu::BufferUsages::STORAGE,
        });
        let sreact_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Gillespie stoich_react"),
            contents: bytemuck::cast_slice(stoich_react),
            usage: wgpu::BufferUsages::STORAGE,
        });
        let snet_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let states_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("Gillespie states"),
            contents: bytemuck::cast_slice(initial_states),
            usage: wgpu::BufferUsages::STORAGE | wgpu::BufferUsages::COPY_SRC,
        });
        let prng_buf = dev.device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let bg = {
            ...
            dev.device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry { binding: 1, resource: rate_buf.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 2, resource: sreact_buf.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 3, resource: snet_buf.as_entire_binding() },
        ...
        let mut encoder = dev.device.create_command_encoder(
            &wgpu::CommandEncoderDescriptor { label: Some("GillespieSSA") });
        {
        ...
        Ok(GillespieResult { states, times, n_trajectories: n_t, n_species: n_s })
    }
}
...
mod tests {
    ...
    async fn test_irreversible_decay_mean() {
        ...
        let result = ssa.simulate(
            &rate_k, &stoich_react, &stoich_net,
            &initial, &seeds, n_traj,
            &GillespieConfig { t_max: 1.0, max_steps: 100_000 },
        ).unwrap();
