...
mod tests {
    ...
    async fn test_masked_fill_edge_cases() {
        ...
        let output = masked_fill(&dev.device, &dev.queue, &input, &mask, 0.0).await.unwrap();
        assert_eq!(output, vec![0.0, 0.0, 0.0]);

        // None masked
        let input = vec![1.0, 2.0, 3.0];
        let mask = vec![false, false, false];
        let output = masked_fill(&dev.device, &dev.queue, &input, &mask, 999.0).await.unwrap();
    ...
    async fn test_masked_fill_boundary() {
        ...
        let output = masked_fill(&dev.device, &dev.queue, &input, &mask, f32::NEG_INFINITY).await.unwrap();
    ...
    async fn test_masked_fill_large_batch() {
        ...
        let output = masked_fill(&dev.device, &dev.queue, &input, &mask, -1.0).await.unwrap();
    ...
    async fn test_masked_fill_precision() {
        ...
        let output = masked_fill(&dev.device, &dev.queue, &input, &mask, -7.5).await.unwrap();
