...
mod tests {
    ...
    async fn test_reflection_pad2d_edge_cases() {
        ...
        let output = reflection_pad2d(&dev.device, &dev.queue, &input, 1, 1, 4, 4, 0, 0, 0, 0).await.unwrap();
        assert_eq!(output, input);

        // Single pixel
        let input = vec![5.0]; // 1x1x1x1
        let output = reflection_pad2d(&dev.device, &dev.queue, &input, 1, 1, 1, 1, 1, 1, 1, 1).await.unwrap();
    ...
    async fn test_reflection_pad2d_boundary() {
        ...
        let output = reflection_pad2d(&dev.device, &dev.queue, &input, 1, 3, 4, 4, 2, 2, 2, 2).await.unwrap();
        assert_eq!(output.len(), 1 * 3 * 8 * 8);

        // Asymmetric padding
        let input = vec![1.0; 1 * 1 * 8 * 8];
        let output = reflection_pad2d(&dev.device, &dev.queue, &input, 1, 1, 8, 8, 3, 1, 2, 4).await.unwrap();
    ...
    async fn test_reflection_pad2d_large_batch() {
        ...
        let output = reflection_pad2d(&dev.device, &dev.queue, &input, batch_size, channels, 16, 16, 2, 2, 2, 2).await.unwrap();
