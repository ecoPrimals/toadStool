...
mod tests {
    ...
    async fn test_kl_divergence_large_batch() {
        ...
        let q: Vec<f32> = (0..1000).map(|i| ((i + 500) as f32 % 1000.0 + 1.0) / 1000.0).collect();
        let kl = kl_divergence(&dev.device, &dev.queue, &p, &q).await.unwrap();
        assert!(kl >= 0.0);
        assert!(kl.is_finite());
    }
    ...
    async fn test_kl_divergence_precision() {
        ...
        let p = vec![0.7, 0.3];
        let q = vec![0.3, 0.7];
        let kl_pq = kl_divergence(&dev.device, &dev.queue, &p, &q).await.unwrap();
        let kl_qp = kl_divergence(&dev.device, &dev.queue, &q, &p).await.unwrap();
