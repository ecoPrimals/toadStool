...
pub struct Gt { lhs: Tensor, rhs: Tensor }

impl Gt {
    pub fn new(lhs: Tensor, rhs: Tensor) -> Self { Self { lhs, rhs } }
    fn wgsl_shader() -> &'static str { include_str!("../shaders/gt.wgsl") }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        let mut encoder = device.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Gt Encoder") });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Gt Pass"), timestamp_writes: None });
...
impl Tensor {
    pub fn gt(self, other: &Self) -> Result<Self> { Gt::new(self, other.clone()).execute() }
}
