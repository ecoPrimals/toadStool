    ...
    fn wgsl_shader() -> &'static str {
        {
            static SHADER: std::sync::LazyLock<String> =
                std::sync::LazyLock::new(|| crate::shaders::precision::downcast_f64_to_f32_with_transcendentals(include_str!("../shaders/gnn/gcn_conv_f64.wgsl")));
            SHADER.as_str()
        }
    }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        let bind_group = device.device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                ...
                wgpu::BindGroupEntry {
                    binding: 4,
                    resource: self.degrees.buffer().as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 5,
...

        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("GCNConv Transform Pass"),
        ...
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("GCNConv Aggregate Pass"),
                timestamp_writes: None,
            });
        ...
        if self.add_self_loops {
            ...
            {
                let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                    label: Some("GCNConv Self Loops Pass"),
                    timestamp_writes: None,
                });
