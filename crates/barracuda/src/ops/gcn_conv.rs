...
mod tests {
    ...
    async fn test_gcn_conv_edge_cases() {
        ...
        let output = gcn_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, 1, 4, 8).await.unwrap();
        ...
        let output = gcn_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, 2, 4, 8).await.unwrap();
    ...
    async fn test_gcn_conv_boundary() {
        ...
        let weights = vec![0.1; 4 * 8];
        let output = gcn_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, num_nodes, 4, 8).await.unwrap();
    ...
    async fn test_gcn_conv_large_batch() {
        ...
        let output = gcn_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, num_nodes, in_feat, out_feat).await.unwrap();
    ...
    async fn test_gcn_conv_precision() {
        ...
        let output = gcn_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, 3, 2, 4).await.unwrap();
