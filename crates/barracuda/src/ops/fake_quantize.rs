...
mod tests {
    ...
    async fn test_fake_quantize_basic() {
        ...
        let output = fake_quantize(&dev.device, &dev.queue, &input, 0.1, 0)
            .await
            .unwrap();
    ...
    async fn test_fake_quantize_boundary() {
        ...
        let output = fake_quantize(&dev.device, &dev.queue, &input, scale, 0)
            .await
            .unwrap();
        ...
        let output = fake_quantize(&dev.device, &dev.queue, &input, scale, 50)
            .await
            .unwrap();
        assert!(output.iter().all(|&x| x.is_finite()));
    }
    ...
    async fn test_fake_quantize_precision() {
        ...
        let output1 = fake_quantize(&dev.device, &dev.queue, &input, scale, 0)
            .await
            .unwrap();
        let output2 = fake_quantize(&dev.device, &dev.queue, &input, scale, 0)
            .await
            .unwrap();
        ...
        for (i, &out_val) in output1.iter().enumerate() {
            ...
            assert!(
                diff < 1.0,
                "Quantized value {} differs too much from input {}",
                out_val,
                input[i]
            );
        }
        ...
        let output_large_scale = fake_quantize(&dev.device, &dev.queue, &input, 1.0, 0)
            .await
            .unwrap();
