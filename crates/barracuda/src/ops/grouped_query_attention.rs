        ...
        for qh in 0..num_q_heads {
            ...
            for i in 0..seq_len {
                ...
                for j in 0..seq_len {
                    let mut score = 0.0;
                    for d in 0..head_dim {
                        let q_idx = b * num_q_heads * seq_len * head_dim + qh * seq_len * head_dim + i * head_dim + d;
                        let k_idx = b * num_kv_heads * seq_len * head_dim + kv_head * seq_len * head_dim + j * head_dim + d;
                ...
                for d in 0..head_dim {
                    ...
                    for j in 0..seq_len {
                        let v_idx = b * num_kv_heads * seq_len * head_dim + kv_head * seq_len * head_dim + j * head_dim + d;
                        weighted_sum += scores[j] * value[v_idx];
                    }
                    let out_idx = b * num_q_heads * seq_len * head_dim + qh * seq_len * head_dim + i * head_dim + d;
...
mod tests {
    ...
    async fn test_grouped_query_attention() {
        ...
        let v = k.clone();
        let output = grouped_query_attention(&dev.device, &dev.queue, &q, &k, &v, 1, 8, 2, 4, 4).await.unwrap();
