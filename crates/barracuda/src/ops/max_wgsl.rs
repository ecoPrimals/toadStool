    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        match self.dim {
            None => {
                ...
                // Compile shader
                let shader_module = device.compile_shader(Self::wgsl_shader_reduce(), Some("Max Reduce Shader"));
                ...
                let global_max = partial_results.iter().fold(f32::NEG_INFINITY, |a: f32, &b| a.max(b));
            ...
            Some(dim) => {
                ...
                // Compile shader
                let shader_module = device.compile_shader(Self::wgsl_shader_dim(), Some("Max Dim Shader"));
