...
mod tests {
    ...
    async fn test_mfcc_edge_cases() {
        ...
        let mfcc_features = mfcc(&dev.device, &dev.queue, &mel_spec, 1, 80, 13).await.unwrap();
        assert_eq!(mfcc_features.len(), 13);

        // Maximum MFCC coefficients (n_mfcc = n_mels)
        let mel_spec = vec![1.0; 10 * 20];
        let mfcc_features = mfcc(&dev.device, &dev.queue, &mel_spec, 10, 20, 20).await.unwrap();
    ...
    async fn test_mfcc_boundary() {
        ...
        let mfcc_features = mfcc(&dev.device, &dev.queue, &mel_spec, 100, 80, 5).await.unwrap();
        assert_eq!(mfcc_features.len(), 100 * 5);

        // Many mel bands
        let mel_spec = vec![1.0; 50 * 128];
        let mfcc_features = mfcc(&dev.device, &dev.queue, &mel_spec, 50, 128, 20).await.unwrap();
    ...
    async fn test_mfcc_large_batch() {
        ...
        let mfcc_features = mfcc(&dev.device, &dev.queue, &mel_spec, 1000, 80, 13).await.unwrap();
    ...
    async fn test_mfcc_precision() {
        ...
        let mfcc_features = mfcc(&dev.device, &dev.queue, &mel_spec, 10, 80, 13).await.unwrap();
