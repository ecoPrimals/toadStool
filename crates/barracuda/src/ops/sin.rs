...
pub struct Sin { input: Tensor }

impl Sin {
    pub fn new(input: Tensor) -> Self { Self { input } }
    fn wgsl_shader() -> &'static str { include_str!("../shaders/sin.wgsl") }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        let mut encoder = device.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("Sin Encoder") });
        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor { label: Some("Sin Pass"), timestamp_writes: None });
...
impl Tensor {
    pub fn sin(self) -> Result<Self> { Sin::new(self).execute() }
}
