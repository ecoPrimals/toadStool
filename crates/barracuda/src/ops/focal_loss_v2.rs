...
mod tests {
    ...
    async fn test_focal_loss_v2_boundary() {
        ...
        let loss_gamma1 = focal_loss_v2(&dev.device, &dev.queue, &preds, &targets, 0.25, 1.0).await.unwrap();
        let loss_gamma3 = focal_loss_v2(&dev.device, &dev.queue, &preds, &targets, 0.25, 3.0).await.unwrap();
    ...
    async fn test_focal_loss_v2_large_batch() {
        ...
        let targets: Vec<f32> = (0..size).map(|i| if i % 2 == 0 { 1.0 } else { 0.0 }).collect();
    ...
    async fn test_focal_loss_v2_precision() {
        ...
        let loss_alpha_low = focal_loss_v2(&dev.device, &dev.queue, &preds, &targets, 0.1, 2.0).await.unwrap();
        let loss_alpha_high = focal_loss_v2(&dev.device, &dev.queue, &preds, &targets, 0.9, 2.0).await.unwrap();
