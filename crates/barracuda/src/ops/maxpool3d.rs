...
mod tests {
    ...
    async fn test_maxpool3d_basic() {
        ...
        let output = maxpool3d(&dev.device, &dev.queue, &input, 1, 2, 4, 4, 4, 2, 2)
            .await
            .unwrap();
    ...
    async fn test_maxpool3d_edge_cases() {
        ...
        let output = maxpool3d(&dev.device, &dev.queue, &input, 1, 1, 2, 2, 2, 2, 2)
            .await
            .unwrap();
        ...
        let output = maxpool3d(&dev.device, &dev.queue, &input, 1, 1, 4, 4, 4, 2, 2)
            .await
            .unwrap();
    ...
    async fn test_maxpool3d_boundary() {
        ...
        let output = maxpool3d(&dev.device, &dev.queue, &input, 1, 3, 8, 8, 8, 2, 4)
            .await
            .unwrap();
        assert!(output.len() > 0);
        ...
        let output = maxpool3d(&dev.device, &dev.queue, &input, 1, 16, 6, 6, 6, 2, 2)
            .await
            .unwrap();
