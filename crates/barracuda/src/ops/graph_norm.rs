...
pub async fn graph_norm(
    ...
    for node in 0..num_nodes {
        ...
        for f in 0..num_features {
            graph_mean[graph_id * num_features + f] += 
                node_features[node * num_features + f];
        }
    }
    ...
    for node in 0..num_nodes {
        let graph_id = batch_assignment[node];
        for f in 0..num_features {
            let diff = node_features[node * num_features + f] 
                     - graph_mean[graph_id * num_features + f];
    ...
    for node in 0..num_nodes {
        ...
        for f in 0..num_features {
            ...
            output[node * num_features + f] = 
                (node_features[node * num_features + f] - mean) / std;
        }
    }
...
mod tests {
    ...
    async fn test_graph_norm() {
        ...
        let output = graph_norm(&dev.device, &dev.queue, &node_features, &batch, 8, 8, 2, 1e-5).await.unwrap();
