...
impl SpatialDropout {
    pub fn new(input: Tensor, mask: Tensor, drop_prob: f32, training: bool) -> Result<Self> {
        ...
        if mask_shape.len() != 2 || mask_shape[0] != input_shape[0] || mask_shape[1] != input_shape[1] {
            return Err(BarracudaError::invalid_op(
                "spatial_dropout",
    ...
    fn wgsl_shader() -> &'static str {
        include_str!("../shaders/spatial_dropout.wgsl")
    }
...
mod tests {
    ...
    async fn test_spatial_dropout_basic() {
        ...
        let input = Tensor::from_vec_on(vec![1.0; 1 * 3 * 4 * 4], vec![1, 3, 4, 4], device.clone())
            .await
            .unwrap();
        let mask = Tensor::from_vec_on(vec![1.0; 1 * 3], vec![1, 3], device.clone())
            .await
            .unwrap();
