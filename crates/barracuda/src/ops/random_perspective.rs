...
pub async fn random_perspective(
    ...
    let dst_corners = [
        (src_corners[0].0 + rand() * width as f32, src_corners[0].1 + rand() * height as f32),
        (src_corners[1].0 + rand() * width as f32, src_corners[1].1 + rand() * height as f32),
        (src_corners[2].0 + rand() * width as f32, src_corners[2].1 + rand() * height as f32),
        (src_corners[3].0 + rand() * width as f32, src_corners[3].1 + rand() * height as f32),
    ];
    ...
    for c in 0..channels {
        for i in 0..height {
            for j in 0..width {
                ...
                let top = (dst_corners[0].0 * (1.0 - u) + dst_corners[1].0 * u,
                          dst_corners[0].1 * (1.0 - u) + dst_corners[1].1 * u);
                let bottom = (dst_corners[3].0 * (1.0 - u) + dst_corners[2].0 * u,
...
mod tests {
    ...
    async fn test_random_perspective() {
        ...
        let transformed = random_perspective(&dev.device, &dev.queue, &image, 3, 100, 100, 0.2, 33333).await.unwrap();
