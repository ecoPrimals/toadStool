...
impl IndexAdd {
    ...
    pub fn new(input: Tensor, dim: usize, indices: Vec<u32>, values: Tensor) -> Result<Self> {
        ...
        if dim >= input_shape.len() {
            return Err(BarracudaError::invalid_op(
                "IndexAdd",
                format!("Dimension {} out of bounds for rank {}", dim, input_shape.len()),
            ));
        }
        ...
        if values_size != expected_values_size {
            return Err(BarracudaError::invalid_op(
                "IndexAdd",
                format!("Values size {} doesn't match expected size {}", values_size, expected_values_size),
            ));
        }
        ...
        for &idx in &indices {
            if idx as usize >= dim_size {
                return Err(BarracudaError::invalid_op(
                    "IndexAdd",
                    format!("Index {} out of bounds for dimension size {}", idx, dim_size),
                ));
            }
        }
    ...
    fn wgsl_shader() -> &'static str {
        include_str!("../shaders/index_add.wgsl")
    }
