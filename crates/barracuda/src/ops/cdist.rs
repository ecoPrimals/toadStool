...
mod tests {
    ...
    async fn test_cdist_edge_cases() {
        ...
        let x1 = vec![1.0, 2.0];
        let x2 = vec![3.0, 4.0];
        let distances = cdist(&dev.device, &dev.queue, &x1, &x2, 1, 1, 2).await.unwrap();
        ...
        let distances = cdist(&dev.device, &dev.queue, &x1, &x2, 1, 1, 3).await.unwrap();
    ...
    async fn test_cdist_boundary() {
        ...
        let distances = cdist(&dev.device, &dev.queue, &x1, &x2, 1, 3, 2).await.unwrap();
    ...
    async fn test_cdist_large_batch() {
        ...
        let distances = cdist(&dev.device, &dev.queue, &x1, &x2, n1, n2, dim).await.unwrap();
    ...
    async fn test_cdist_precision() {
        ...
        let distances = cdist(&dev.device, &dev.queue, &x1, &x2, 2, 1, 2).await.unwrap();
