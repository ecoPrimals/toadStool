...
mod tests {
    ...
    async fn test_expand_basic() {
        ...
        let input = vec![5.0];
        let output = expand(&dev.device, &dev.queue, &input, &[1], &[6]).await.unwrap();
    ...
    async fn test_expand_edge_cases() {
        ...
        let input = vec![7.0];
        let output = expand(&dev.device, &dev.queue, &input, &[1], &[10]).await.unwrap();
        assert_eq!(output.len(), 10);
        assert!(output.iter().all(|&x| (x - 7.0).abs() < 1e-5));

        // No expansion (already correct size)
        let input = vec![1.0, 2.0, 3.0];
        let output = expand(&dev.device, &dev.queue, &input, &[3], &[3]).await.unwrap();
    ...
    async fn test_expand_boundary() {
        ...
        let input = vec![3.14];
        let output = expand(&dev.device, &dev.queue, &input, &[1], &[1000]).await.unwrap();
        assert_eq!(output.len(), 1000);
        assert!(output.iter().all(|&x| (x - 3.14).abs() < 1e-5));

        // Smaller expansion
        let input = vec![99.0];
        let output = expand(&dev.device, &dev.queue, &input, &[1], &[5]).await.unwrap();
    ...
    async fn test_expand_large_batch() {
        ...
        let input = vec![42.0];
        let output = expand(&dev.device, &dev.queue, &input, &[1], &[10000]).await.unwrap();
    ...
    async fn test_expand_precision() {
        ...
        let output = expand(&dev.device, &dev.queue, &input, &[1], &[100]).await.unwrap();
