...
impl ComplexPow {
    pub fn new(input: Tensor, exponent: f32) -> Result<Self> {
        ...
        Ok(Self { input, exponent, pipeline, bind_group_layout })
    }
    ...
    pub fn execute(self) -> Result<Tensor> {
        ...
        #[repr(C)] #[derive(Copy, Clone, bytemuck::Pod, bytemuck::Zeroable)] struct Params { num_complex: u32, exponent: f32, }
        let params = Params { num_complex: (n / 2) as u32, exponent: self.exponent };
        let params_buffer = device.device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("P"), contents: bytemuck::bytes_of(&params), usage: wgpu::BufferUsages::UNIFORM });
