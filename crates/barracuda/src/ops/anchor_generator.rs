...
mod tests {
    ...
    async fn test_anchor_generator_edge_cases() {
        ...
        let anchors = anchor_generator(&dev.device, &dev.queue, 1, 1, 8, &[16.0], &[1.0]).await.unwrap();
        assert_eq!(anchors.len(), 1 * 1 * 1 * 1 * 4); // 4 coordinates
        assert!(anchors.iter().all(|&x| x.is_finite()));
        
        // Test with single aspect ratio
        let anchors = anchor_generator(&dev.device, &dev.queue, 2, 2, 16, &[32.0], &[1.0]).await.unwrap();
    ...
    async fn test_anchor_generator_boundary() {
        ...
        let anchors1 = anchor_generator(&dev.device, &dev.queue, 3, 3, 8, &[16.0], &[1.0]).await.unwrap();
        let anchors2 = anchor_generator(&dev.device, &dev.queue, 3, 3, 16, &[16.0], &[1.0]).await.unwrap();
    ...
    async fn test_anchor_generator_large_batch() {
        ...
        let anchors = anchor_generator(&dev.device, &dev.queue, feature_h, feature_w, 16, &sizes, &ratios).await.unwrap();
        
        assert_eq!(anchors.len(), feature_h * feature_w * sizes.len() * ratios.len() * 4);
        assert!(anchors.iter().all(|&x| x.is_finite()));
    }
    ...
    async fn test_anchor_generator_precision() {
        ...
        let anchors = anchor_generator(&dev.device, &dev.queue, 1, 1, 16, &[32.0], &[1.0]).await.unwrap();
