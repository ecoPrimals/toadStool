...
mod tests {
    ...
    async fn test_normalize_edge_cases() {
        ...
        let output = normalize(&dev.device, &dev.queue, &input, 1, &[1, 2], 1e-8).await.unwrap();
        assert!(output.iter().all(|&x| x.is_finite()));

        // Single element
        let input = vec![5.0];
        let output = normalize(&dev.device, &dev.queue, &input, 0, &[1], 1e-8).await.unwrap();
    ...
    async fn test_normalize_large_batch() {
        ...
        let output = normalize(&dev.device, &dev.queue, &input, 1, &[100, 10], 1e-8).await.unwrap();
    ...
    async fn test_normalize_precision() {
        ...
        let output = normalize(&dev.device, &dev.queue, &input, 1, &[1, 3], 1e-8).await.unwrap();
