...
mod tests {
    ...
    async fn test_gat_conv_edge_cases() {
        ...
        let edges = vec![];
        let output = gat_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, &attention, 1, 4, 8).await.unwrap();
        ...
        let edges = vec![(0, 1)];
        let output = gat_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, &attention, 2, 4, 8).await.unwrap();
    ...
    async fn test_gat_conv_boundary() {
        ...
        let output = gat_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, &attention, num_nodes, 8, 16).await.unwrap();
    ...
    async fn test_gat_conv_large_batch() {
        ...
        let output = gat_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, &attention, num_nodes, in_feat, out_feat).await.unwrap();
    ...
    async fn test_gat_conv_precision() {
        ...
        let output = gat_conv(&dev.device, &dev.queue, &node_features, &edges, &weights, &attention, 3, 4, 8).await.unwrap();
