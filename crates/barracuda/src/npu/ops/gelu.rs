...
mod tests {
    ...
    fn test_gelu_basic() {
        ...
        // GELU(0) = 0
        assert!((output[2] - 0.0).abs() < 1e-5, "GELU(0) should be 0, got {}", output[2]);
        
        // GELU is monotonic for positive values
        assert!(output[3] < output[4], "GELU should be monotonic for positive values");
        
        // GELU(x) ≈ x for large positive x
        assert!((output[4] - 2.0).abs() < 0.1, "GELU(2) should be ≈ 2, got {}", output[4]);
    ...
    fn test_gelu_vs_relu_sparsity() {
        ...
        assert!(relu_sp >= gelu_sp, "ReLU should create more sparsity than GELU");
