...
impl NpuMlBackend {
    ...
    pub fn with_threshold(threshold: f32) -> Result<Self> {
        ...
        let manager = akida_driver::DeviceManager::discover()
            .map_err(|e| crate::error::BarracudaError::device_not_found(
                format!("No NPU devices found: {}", e)
            ))?;
        ...
        // Open first device
        let device = manager.open(0)
            .map_err(|e| crate::error::BarracudaError::device_not_found(
                format!("Failed to open NPU device: {}", e)
            ))?;
        
        // Query capabilities at runtime
        let info = manager.device(0)
            .map_err(|e| crate::error::BarracudaError::device_not_found(
                format!("Failed to query NPU info: {}", e)
            ))?;
    ...
    pub fn execute_mlp_layer(
        ...
        let result = executor.infer(&events, &mut self.device)
            .map_err(|e| crate::error::BarracudaError::execution_failed(
                format!("NPU inference failed: {}", e)
            ))?;
    ...
    fn test_npu_backend_creation() {
        ...
        match NpuMlBackend::new() {
            Ok(backend) => {
                ...
                println!("  Memory: {} MB", backend.capabilities().memory_bytes / (1024 * 1024));
            }
            Err(e) => {
