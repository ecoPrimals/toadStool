...
impl AutoContext {
    ...
    pub fn randn(&self, shape: Vec<usize>) -> Result<Tensor> {
        ...
        let device = self.devices.values().next()
    ...
    pub fn matmul(&self, a: &Tensor, b: &Tensor) -> Result<Tensor> {
        ...
        println!("  🎯 MatMul {}×{} → {} (score: {:.3})",
                 a.shape()[0], a.shape()[1],
                 executor.name(),
                 executor.score_operation(&op, &[TensorDescriptor::new(a.shape().to_vec(), DType::F32),
    ...
    pub fn relu(&self, input: &Tensor) -> Result<Tensor> {
        ...
        let executor = self.scheduler.select_executor(&op, &[desc.clone()]);
        let hw_type = executor.hardware_type();
        
        println!("  🎯 ReLU [{}] → {} (score: {:.3})",
    ...
    pub fn conv2d(&self, input: &Tensor, kernel: &Tensor) -> Result<Tensor> {
        ...
        let executor = self.scheduler.select_executor(&op, &[desc_input.clone(), desc_kernel.clone()]);
        let hw_type = executor.hardware_type();
        
        println!("  🎯 Conv2D {:?} * {:?} → {} (score: {:.3})",
                 input.shape(),
                 kernel.shape(),
                 executor.name(),
    ...
    pub fn add(&self, a: &Tensor, b: &Tensor) -> Result<Tensor> {
        ...
        let executor = self.scheduler.select_executor(&op, &[desc_a.clone(), desc_b]);
        let hw_type = executor.hardware_type();
