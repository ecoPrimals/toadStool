...
impl AkidaExecutor {
    ...
    pub fn new() -> Result<Self> {
        ...
        log::info!("Akida executor initialized with {} boards", caps.boards.len());
    ...
    pub async fn spike_encode_akida(
        &self,
        input: &[f32],
        time_steps: u32,
    ) -> Result<Vec<u32>> {
    ...
    pub async fn stdp_learning_akida(
        ...
        log::debug!(
            "Executing STDP learning on Akida board {}",
            board.index
        );
        ...
        log::debug!(
            "Akida STDP complete: {} weights updated",
            result.len()
        );
    ...
    fn select_board(&self) -> &AkidaBoard {
        let index = self.current_board.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
...
impl NeuromorphicComparison {
    ...
    pub fn print(&self) {
        ...
        println!("  Speedup:         {:.1}x {}", 
                self.speedup,
                if self.speedup > 1.0 { "🚀 FASTER" } else { "" });
        
        println!("\n⚡ Power Consumption:");
        println!("  GPU (RTX 3090):  {:.0}W", self.gpu_power_w);
        println!("  Akida NPU:       {:.1}W", self.akida_power_w);
        println!("  Reduction:       {:.0}x 🌱 GREENER", self.gpu_power_w / self.akida_power_w);
        ...
        println!("  Efficiency:      {:.0}x ⚡ BETTER", self.energy_efficiency);
...
mod tests {
    ...
    async fn test_akida_executor_creation() {
        ...
        match result {
            Ok(executor) => {
                println!("✅ Akida executor created: {} boards, {} NPUs",
    ...
    async fn test_spike_encode_akida() {
        ...
        let spikes = executor.spike_encode_akida(&input, time_steps).await.unwrap();
    ...
    async fn test_lif_neuron_akida() {
        ...
        let output = executor.lif_neuron_akida(
            &input_spikes,
            &weights,
            threshold,
            leak,
