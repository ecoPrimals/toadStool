...
async fn main() -> Result<()> {
    ...
    // Create tensor
    let poly_tensor = Tensor::from_data(&poly_u32, vec![degree_small as usize * 2], _device.clone())?;
    ...
    println!("   Estimated GPU target: ~{}μs (56x speedup)", cpu_time_large.as_micros() / 56);
    ...
    // Inverse NTT
    let intt_op_large = FheIntt::new(ntt_result_large, degree_large as u32, modulus_large, inv_root_large)?;
