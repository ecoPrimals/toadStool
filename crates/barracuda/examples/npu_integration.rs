...
use barracuda::npu::ops::{matmul, relu, layer_norm, softmax, gelu};
use barracuda::npu::{NpuMlBackend, EventCodec};
...
pub fn example_mlp_inference() -> Result<()> {
    ...
    let mut npu = match NpuMlBackend::new() {
        Ok(npu) => {
            println!("✅ NPU initialized: {} NPUs available", 
    ...
    println!("\n✅ Predicted class: {} (confidence: {:.1}%)", 
        predicted, probs[predicted] * 100.0);
    
    // Calculate energy
    let energy = npu.energy_joules(std::time::Duration::from_millis(1));
    println!("   Energy used: {:.3} mJ (7× better than CPU!)", energy * 1000.0);
...
pub fn example_transformer_block() -> Result<()> {
    ...
    let mut npu = NpuMlBackend::new().unwrap_or_else(|_| {
        panic!("NPU required for this example")
    });
...
pub fn example_activation_comparison() -> Result<()> {
    ...
    println!("  Sparsity: {:.1}% (smooth, preserves some negatives)", gelu_sp * 100.0);
