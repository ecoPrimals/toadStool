...
async fn e2e_fft_3d_workflow() {
    ...
    println!("   ✅ DC component: {:.4} (expected ~{:.4})", dc_real, expected_dc);
...
async fn e2e_complex_exp_to_fft() {
    ...
    let total_energy: f32 = spectrum_data.iter()
        .step_by(2)
        .map(|&x| x * x)
        .sum();
...
async fn e2e_full_molecular_dynamics_simulation() {
    ...
    let charge_grid = Tensor::from_data(&charges, vec![grid_size, grid_size, grid_size, 2], device.clone()).unwrap();
    
    // Step 2: FFT to reciprocal space
    let fft_3d = Fft3D::new(charge_grid, grid_size as u32, grid_size as u32, grid_size as u32).unwrap();
    let reciprocal = fft_3d.execute().unwrap();
    println!("   ✅ Step 2: FFT to reciprocal space");
    ...
    assert_eq!(potential_data.len(), total * 2, "Potential grid size correct");
