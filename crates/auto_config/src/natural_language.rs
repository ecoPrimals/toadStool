...
impl NaturalLanguageProcessor {
    ...
    fn detect_performance_profile(&self, request: &str) -> PerformanceProfile {
        let performance_keywords = [
            "fast", "quick", "speed", "performance", "optimize", "efficient"
        ];
        
        let ml_keywords = [
            "machine learning", "ml", "ai", "neural", "training", "model"
        ];
        
        let power_saver_keywords = [
            "battery", "power", "laptop", "mobile", "energy"
        ];
        
        if ml_keywords.iter().any(|&keyword| request.contains(keyword)) {
            PerformanceProfile::MaxPerformance
        } else if performance_keywords.iter().any(|&keyword| request.contains(keyword)) {
            PerformanceProfile::Performance
        } else if power_saver_keywords.iter().any(|&keyword| request.contains(keyword)) {
    ...
    fn detect_runtime_preferences(&self, request: &str) -> Vec<String> {
        ...
        if request.contains("container") || request.contains("docker") || request.contains("isolated") {
            preferences.push("container".to_string());
        }
        
        if request.contains("gpu") || request.contains("machine learning") || request.contains("cuda") {
            preferences.push("gpu".to_string());
        }
        
        if request.contains("wasm") || request.contains("webassembly") || request.contains("portable") {
    ...
    fn detect_use_case(&self, request: &str) -> UseCase {
        ...
        } else if request.contains("machine learning") || request.contains("ml") || request.contains("ai") {
            UseCase::MachineLearning
        } else if request.contains("web") || request.contains("server") || request.contains("api") {
            UseCase::WebDevelopment
        } else if request.contains("data") || request.contains("analysis") || request.contains("processing") {
    ...
    fn detect_user_experience(&self, request: &str) -> UserExperience {
        let beginner_keywords = [
            "don't know", "new to", "beginner", "simple", "easy", 
            "just work", "automatic", "grandma", "help"
        ];
        
        let expert_keywords = [
            "optimize", "configure", "advanced", "custom", "specific",
            "performance", "tuning", "cluster", "distributed"
        ];
        
        if beginner_keywords.iter().any(|&keyword| request.contains(keyword)) {
            UserExperience::Beginner
        } else if expert_keywords.iter().any(|&keyword| request.contains(keyword)) {
    ...
    async fn generate_config_from_intent(&self, intent: &ConfigurationIntent) -> ToadStoolResult<GeneratedConfig> {
    ...
    async fn generate_explanation(&self, intent: &ConfigurationIntent, config: &GeneratedConfig) -> ToadStoolResult<String> {
        let mut explanation = String::new();
        
        // Security explanation
        match config.security_profile {
            SecurityProfile::Maximum => {
                explanation.push_str("🛡️ I've set up maximum security to keep your computer completely safe. ");
            },
            ...
            _ => {
                explanation.push_str("✅ I've set up standard security that balances safety and performance. ");
            }
        }
        ...
        match config.performance_profile {
            PerformanceProfile::MaxPerformance => {
                explanation.push_str("🚀 Everything is optimized for maximum speed and performance. ");
            },
            PerformanceProfile::Performance => {
        ...
        match intent.use_case {
            UseCase::MachineLearning => {
                explanation.push_str("🤖 I've enabled GPU acceleration for your machine learning work. ");
            },
            UseCase::BusinessApplication => {
                explanation.push_str("💼 I've configured enterprise-grade reliability and monitoring. ");
            },
            UseCase::ScriptExecution => {
                explanation.push_str("📝 I've set up a simple environment perfect for running scripts. ");
            },
            _ => {}
        }
        ...
        match intent.user_experience {
            UserExperience::Beginner => {
                explanation.push_str("😊 Everything is set to 'just work' - no technical knowledge needed!");
            },
            UserExperience::Expert => {
    ...
    async fn generate_suggestions(&self, intent: &ConfigurationIntent) -> ToadStoolResult<Vec<String>> {
...
mod tests {
    ...
    async fn test_natural_language_processing() {
        ...
        let response = processor.configure_from_natural_language(
            "I want to run Python scripts safely for my business"
        ).await.unwrap();
        ...
        assert!(matches!(response.config.security_profile, 
            SecurityProfile::Minimal | SecurityProfile::Standard | SecurityProfile::High | SecurityProfile::Maximum));
    }
    ...
    async fn test_intent_parsing() {
        ...
        let intent = processor.parse_intent("make it fast for machine learning").await.unwrap();
        
        assert_eq!(intent.use_case, UseCase::MachineLearning);
        assert!(matches!(intent.performance_profile, PerformanceProfile::MaxPerformance));
    }
} 