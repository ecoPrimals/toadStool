...
impl HybridEsn {
    ...
    pub fn from_weights(
        ...
        if input_dim * reservoir_dim != w_in.len() {
            return Err(AkidaError::capability_query_failed(format!(
                "w_in length {} not divisible by reservoir_dim {}",
                w_in.len(),
                reservoir_dim
            )));
        }
        ...
        if output_dim * reservoir_dim != w_out.len() {
            return Err(AkidaError::capability_query_failed(format!(
                "w_out length {} not divisible by reservoir_dim {}",
                w_out.len(),
                reservoir_dim
            )));
        }
        ...
        info!(
            "HybridEsn: {}→{}→{} (leak={:.2}), mode=PureSoftware",
...
mod tests {
    ...
    fn approach_b_scale_trick_non_degenerate() {
        ...
        for _ in 0..30 {
            outputs2.push(hw_exec2.step(&input).unwrap()[0]);
        }
    ...
    fn hardware_native_has_bounded_relu_saturation() {
        ...
        let weights =
            EsnWeights::new(w_in.clone(), w_res.clone(), w_out.clone(), is, rs, os, 0.3).unwrap();
        let scale = ScaleTrickConfig::from_weights(&w_in, &w_res);
        let eps = scale.epsilon;
        let mut hw_exec = HardwareEsnExecutor {
            reservoir_dim: rs,
    ...
    fn esn_weights_spectral_radius() {
        ...
        let w_res: Vec<f32> = (0..rs * rs)
            ...
            .collect();
        let w = EsnWeights::new(vec![0.1; rs * 4], w_res, vec![0.1; rs], 4, rs, 1, 0.3).unwrap();
