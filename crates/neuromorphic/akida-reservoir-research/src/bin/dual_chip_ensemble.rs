...
async fn main() -> Result<()> {
    ...
    println!("   Expected state size per chip: {}", config.state_size_per_chip);
    ...
    if !model1_exists || !model2_exists {
        ...
        println!("      cargo run --bin generate-reservoir -- --seed 42 --out reservoir_seed42.fbz");
        println!("      cargo run --bin generate-reservoir -- --seed 123 --out reservoir_seed123.fbz");
    ...
    match DualChipEnsemble::discover_and_create(config.clone()) {
        Ok(mut ensemble) => {
            ...
            println!("      Latency: {:?} ({:.2} µs)", elapsed, elapsed.as_micros() as f64);
            ...
            println!("      Actual measured:   {:.2}µs", elapsed.as_micros() as f64);
