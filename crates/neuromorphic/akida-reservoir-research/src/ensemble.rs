...
use akida_driver::{DeviceManager, AkidaDevice};
...
impl DualChipEnsemble {
    ...
    pub fn discover_and_create(config: EnsembleConfig) -> Result<Self> {
        ...
        let manager = DeviceManager::discover()
            .context("Failed to discover Akida devices")?;
        ...
        let device1 = manager.open(0)
            .context("Failed to open device 0")?;
        let device2 = manager.open(1)
            .context("Failed to open device 1")?;
    ...
    pub fn load_reservoirs(&mut self) -> Result<()> {
        ...
        self.model1.load_to_device(&mut self.device1)
            .context("Failed to load to device 1")?;
        info!("  ✅ Chip 1 loaded");
        
        // Load to chip 2
        debug!("Loading to chip 2...");
        self.model2.load_to_device(&mut self.device2)
    ...
    pub fn get_ensemble_state(&mut self, input: &[u8]) -> Result<Array1<f32>> {
        ...
        let result1 = self.model1.infer(input, &mut self.device1)
            .context("Chip 1 inference failed")?;
        let state1 = inference_to_state(&result1);
        
        // Chip 2 inference
        debug!("Chip 2 inference...");
        let result2 = self.model2.infer(input, &mut self.device2)
            .context("Chip 2 inference failed")?;
        let state2 = inference_to_state(&result2);
        
        // Concatenate states
        debug!("Concatenating states ({} + {} = {})",
               state1.len(), state2.len(), state1.len() + state2.len());
        
        let ensemble_state = concatenate_states(&[state1, state2])
            .context("Failed to concatenate states")?;
    ...
    pub fn collect_training_states(
        &mut self,
        inputs: &[Vec<u8>],
