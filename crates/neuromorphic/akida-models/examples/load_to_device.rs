...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Bytes transferred: {} bytes ({:.2} KB)",
    ...
    // Device utilization
    let memory_used_pct = (metrics.bytes_transferred as f64 / (caps.memory_mb as f64 * 1024.0 * 1024.0)) * 100.0;
    println!("💾 Device Memory Used: {:.2}%\n", memory_used_pct);
    
    println!("🎉 Demo complete! Model is now loaded on device {}", device.index());
