...
use crate::{Model, Result, AkidaModelError};
...
impl Model {
    ...
    /// - Transfer fails
    /// - Validation fails
    pub fn load_to_device(&self, device: &mut akida_driver::AkidaDevice) -> Result<akida_driver::LoadMetrics> {
        ...
        tracing::debug!("Device caps: {} NPUs, {} MB", caps.npu_count, caps.memory_mb);
    ...
    fn to_program(&self) -> Result<akida_driver::ModelProgram> {
        ...
        if data.is_empty() {
            return Err(AkidaModelError::loading_failed("Model has no program data".to_string()));
        }
