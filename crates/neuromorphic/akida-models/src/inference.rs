...
impl Model {
    ...
    pub fn infer(&self, input: &[u8], device: &mut akida_driver::AkidaDevice) 
        -> Result<akida_driver::InferenceResult> 
    {
        ...
        tracing::debug!("Inference config: input={} bytes, output={} bytes",
                       config.input_size_bytes(), config.output_size_bytes());
