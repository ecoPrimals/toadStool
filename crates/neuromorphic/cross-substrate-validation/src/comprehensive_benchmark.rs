...
use toadstool_runtime_universal::{UniversalRuntime, WorkloadBuilder, OperationType, ComputeUnitType};
...
pub async fn run_comprehensive_benchmark(runtime: &UniversalRuntime) -> Vec<BenchmarkResult> {
    ...
    println!("   Testing {} workloads across all substrates\n", suite.len());
    ...
    for (i, spec) in suite.iter().enumerate() {
        ...
        // Benchmark CPU
        let cpu_time = benchmark_substrate(runtime, ComputeUnitType::Cpu, spec.operation, &input).await;
        ...
        for (idx, _unit) in gpu_units.iter().enumerate() {
            if let Some(time) = benchmark_substrate_by_index(runtime, idx, spec.operation, &input).await {
        ...
        let (winner, best_time) = times.iter()
        ...
        let speedup = if best_time > 0.0 { baseline / best_time } else { 1.0 };
...
pub fn print_results_summary(results: &[BenchmarkResult]) {
    ...
    let nvidia_wins = results.iter().filter(|r| r.winner.contains("NVIDIA")).count();
    let npu_wins = results.iter().filter(|r| r.winner.contains("NPU")).count();
    
    println!("   Winner Distribution:");
    println!("     CPU:         {} wins ({:.1}%)", cpu_wins, cpu_wins as f64 / results.len() as f64 * 100.0);
    println!("     GPU AMD:     {} wins ({:.1}%)", amd_wins, amd_wins as f64 / results.len() as f64 * 100.0);
    println!("     GPU NVIDIA:  {} wins ({:.1}%)", nvidia_wins, nvidia_wins as f64 / results.len() as f64 * 100.0);
    println!("     NPU:         {} wins ({:.1}%)", npu_wins, npu_wins as f64 / results.len() as f64 * 100.0);
