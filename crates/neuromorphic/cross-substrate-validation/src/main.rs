...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if stats.num_gpu > 0 {
            ...
            for (i, _unit) in gpu_units.iter().enumerate() {
                if let Ok(gpu_result) = benchmark_gpu(&universal, i, &input).await {
                    ...
                    println!("   │ {:22} │ {:8.1} │ {:10.2}M/s │ {:7.2}x │ {:8} │",
                             format!("GPU {}", i),
                             gpu_result.time_us,
    ...
    if stats.num_cpu > 0 { println!("   ✅ CPU ({} cores)", stats.num_cpu); }
    if stats.num_gpu > 0 { println!("   ✅ GPU ({} devices)", stats.num_gpu); }
    if akida_manager.device_count() > 0 { 
        println!("   ✅ Neuromorphic ({} Akida devices)", 
...
async fn benchmark_cpu(runtime: &UniversalRuntime, input: &[f32]) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    ...
    let output = runtime.execute_on_type(ComputeUnitType::Cpu, workload).await?;
...
async fn benchmark_gpu(runtime: &UniversalRuntime, index: usize, input: &[f32]) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    let start = Instant::now();
...
async fn benchmark_neuromorphic(manager: &DeviceManager, _input_size: usize) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
