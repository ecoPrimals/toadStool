...
fn test_lz4_simple_data() {
    ...
    let (region, stats) = decompress_isolated(&compressed, CompressionAlgorithm::Lz4, None).unwrap();
...
fn test_lz4_utf8_data() {
    ...
    assert_eq!(String::from_utf8(region.as_slice().to_vec()).unwrap(), original);
}
...
fn test_zstd_simple_data() {
    ...
    let (region, stats) = decompress_isolated(&compressed, CompressionAlgorithm::Zstd, None).unwrap();
    assert_eq!(region.as_slice(), &original[..]);
    assert_eq!(stats.decompressed_size, original.len());
}
...
fn test_compression_stats_lz4() {
    ...
    let (_region, stats) = decompress_isolated(&compressed, CompressionAlgorithm::Lz4, None).unwrap();
...
fn test_compression_stats_zstd() {
    ...
    let (_region, stats) = decompress_isolated(&compressed, CompressionAlgorithm::Zstd, None).unwrap();
...
fn test_json_like_data() {
    ...
    let json_like = r#"{"user":"test","action":"execute","timestamp":1234567890,"data":{"key":"value"}}"#.repeat(100);
    let original = json_like.as_bytes();
    
    let lz4_compressed = compress_lz4(original);
    let (lz4_region, _) = decompress_isolated(&lz4_compressed, CompressionAlgorithm::Lz4, None).unwrap();
    assert_eq!(lz4_region.as_slice(), original);
    
    let zstd_compressed = compress_zstd(original);
    let (zstd_region, _) = decompress_isolated(&zstd_compressed, CompressionAlgorithm::Zstd, None).unwrap();
...
fn test_log_data() {
    ...
    let lz4_compressed = compress_lz4(original);
    let (_lz4_region, lz4_stats) = decompress_isolated(&lz4_compressed, CompressionAlgorithm::Lz4, None).unwrap();
    
    let zstd_compressed = compress_zstd(original);
    let (_zstd_region, zstd_stats) = decompress_isolated(&zstd_compressed, CompressionAlgorithm::Zstd, None).unwrap();
    ...
    println!("Log compression - LZ4: {:.2}x, Zstd: {:.2}x", lz4_ratio, zstd_ratio);
}
