...
fn test_e2e_decompress_and_process() {
    ...
    println!("Compressed: {} bytes → {} bytes ({:.1}% ratio)",
        original_data.len(), compressed.len(),
...
fn test_e2e_with_audit_trail() {
    ...
    let result = runtime.process_isolated(input, |data| {
        // Simulate analysis
        Ok(data.len() * 2)
    }).unwrap();
...
fn test_e2e_compress_encrypt_process() {
    ...
    println!("NestGate compression: {} → {} bytes", sensitive.len(), compressed.len());
    ...
    let result = runtime.process_isolated(memory.as_slice(), |plaintext| {
...
fn test_e2e_performance_monitoring() {
    ...
    println!("Compressed: {} bytes ({:.2}% of original)",
        compressed.len(),
    ...
    println!("Decompression: {:?} ({:.2} MB/s)",
        decomp_time, decomp_stats.throughput_mbps);
    
    // Process
    let start = std::time::Instant::now();
    let result = runtime.process_isolated(memory.as_slice(), |data| {
