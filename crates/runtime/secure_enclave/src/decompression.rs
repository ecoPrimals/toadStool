...
pub fn decompress_isolated(
    ...
    let stats = DecompressionStats::new(
        compressed.len(),
        decompressed.len(),
        duration_micros,
    );
...
mod tests {
    ...
    fn test_zstd_decompression() {
        ...
        assert!(compressed.len() < original.len(), 
            "Data should be compressed: {} vs {}", compressed.len(), original.len());
    }
    ...
    fn test_decompression_stats() {
        ...
        let (_memory, stats) = decompress_isolated(
            &compressed,
            CompressionAlgorithm::Zstd,
            None,
        ).unwrap();
    ...
    fn test_memory_isolation() {
        ...
        let (mut memory, _stats) = decompress_isolated(
            &compressed,
            CompressionAlgorithm::Zstd,
            None,
        ).unwrap();
