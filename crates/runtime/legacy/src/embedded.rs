...
use crate::{
    ...
    LegacyRuntimeConfig, LegacySystemType, LegacyArchitecture, SystemInfo, 
...
impl LegacyAdapter for Microcontroller8BitAdapter {
    ...
    async fn submit_job(&self, job: LegacyJob) -> ToadStoolResult<Uuid> {
        ...
        let embedded_job = EmbeddedJob {
            ...
            source_files: vec![],
            memory_layout: self.config.as_ref().unwrap().memory_layout.clone(),
            programming_interface: self.config.as_ref().unwrap().programming_interface.clone(),
...
impl LegacyAdapter for System16BitAdapter {
    ...
    async fn initialize(&mut self, config: &LegacyRuntimeConfig) -> ToadStoolResult<()> {
        ...
        // Initialize DOS interface if needed
        if self.config.as_ref().unwrap().architecture == LegacyArchitecture::Intel8086 {
