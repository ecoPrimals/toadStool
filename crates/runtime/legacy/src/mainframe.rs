...
use crate::{
    ...
    LegacyAdapter, LegacyJob, LegacyRuntimeConfig, LegacySystemType, MainframeConfig, 
...
impl LegacyAdapter for IBMMainframeAdapter {
    ...
    async fn initialize(&mut self, config: &LegacyRuntimeConfig) -> ToadStoolResult<()> {
        ...
        self.jcl_generator.initialize(&self.config.as_ref().unwrap().jcl_settings).await?;
        self.cobol_compiler.initialize(&self.config.as_ref().unwrap().cobol_settings).await?;
        self.dataset_manager.initialize(&self.config.as_ref().unwrap().datasets).await?;
