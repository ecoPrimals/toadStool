...
pub struct CrossCompilationToolchain {
    #[expect(
        dead_code,
        reason = "stored from init; will be used for sysroot + target triple resolution"
    )]
...
impl CrossCompilationToolchain {
    ...
    pub async fn cross_compile(
        ...
        // Generate cache key
        let source_hash = format!("{:x}", Sha256::digest(code));
        ...
        let compiled_binary = self
            .compile_with_toolchain(code, &target, &toolchain)
            .await?;
    ...
    async fn get_compilation_target(
        &self,
        platform: &EdgePlatform,
        ...
        match platform {
            ...
            EdgePlatform::RaspberryPi { model, .. } => {
                let architecture = match model {
                    ...
                    crate::platforms::PiModel::PiZero | crate::platforms::PiModel::PiZero2W => {
                        "armv6"
                    }
                    crate::platforms::PiModel::PiPico | crate::platforms::PiModel::PiPicoW => {
                        "armv6m"
                    }
                    _ => "armv7",
                };
    ...
    async fn get_toolchain(&self, name: &str) -> ToadStoolResult<ToolchainInfo> {
        ...
        toolchains
            .get(name)
            .cloned()
            .ok_or_else(|| ToadStoolError::not_found(format!("Toolchain '{}' not found", name)))
    }
    ...
    async fn compile_with_toolchain(
        ...
        let output_file_str = output_file.to_str().ok_or_else(|| {
            ToadStoolError::runtime(format!("Invalid output file path: {:?}", output_file))
        })?;
        ...
        let output = cmd
            .output()
            .map_err(|e| ToadStoolError::execution(format!("Failed to execute compiler: {}", e)))?;
