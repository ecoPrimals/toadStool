...
pub trait PeripheralInterface: Send + Sync + std::fmt::Debug {
    ...
    fn read(&self, address: u32) -> impl Future<Output = ToadStoolResult<u32>> + Send + '_;
    ...
    /// Reset peripheral
    fn reset(&mut self) -> impl Future<Output = ToadStoolResult<()>> + Send + '_;

    /// Get peripheral status
    fn get_status(&self) -> impl Future<Output = ToadStoolResult<PeripheralStatus>> + Send + '_;
}
