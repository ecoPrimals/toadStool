...
use crate::{
    BiomeManifest, BiomeMetadata, PrimalConfig, ServiceConfig, WorkloadSource,
    BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage,
    ServiceResources, ServicePort, HealthCheck, DatasetConfig,
};
...
impl TemplateGenerator {
    ...
    pub fn list_templates() -> Vec<(String, String)> {
        vec![
            ("basic".to_string(), "Essential services for general computing".to_string()),
            ("science".to_string(), "Scientific computing with data analysis tools".to_string()),
            ("ai-research".to_string(), "AI/ML training and inference environment".to_string()),
            ("quantum".to_string(), "Quantum computing research platform".to_string()),
            ("genomics".to_string(), "Bioinformatics and genomics analysis".to_string()),
            ("vision".to_string(), "Computer vision and imaging processing".to_string()),
            ("distributed".to_string(), "Multi-node distributed computing cluster".to_string()),
            ("sovereign".to_string(), "Maximum security sovereign computing".to_string()),
            ("development".to_string(), "Development and testing environment".to_string()),
        ]
    }
    ...
    fn create_basic_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        ...
        services.insert("compute".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
                registry: "registry.ecosystem.sovereignscience.org".to_string(),
        ...
        (name, description, primals, services, resources, security, networking, storage)
    }
    
    fn create_science_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "science-biome".to_string();
        let description = "Scientific computing environment with data analysis and visualization".to_string();
        
        let (_, _, primals, mut services, mut resources, security, networking, mut storage) = self.create_basic_template();
        
        // Add scientific computing services
        services.insert("jupyter".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["curl".to_string(), "-f".to_string(), "http://localhost:8888".to_string()],
                interval: 30,
                timeout: 10,
                retries: 3,
                start_period: 60,
        ...
        services.insert("postgresql".to_string(), ServiceConfig {
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["pg_isready".to_string(), "-U".to_string(), "scientist".to_string()],
                interval: 30,
                timeout: 10,
                retries: 3,
                start_period: 30,
    ...
    fn create_ai_research_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "ai-research-biome".to_string();
        let description = "AI/ML research environment with GPU acceleration and model training".to_string();
        
        let (_, _, primals, mut services, mut resources, security, networking, mut storage) = self.create_science_template();
        
        // Add AI/ML services
        services.insert("pytorch".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
                registry: "docker.io".to_string(),
            ...
            environment: vec![
                ("CUDA_VISIBLE_DEVICES".to_string(), "all".to_string()),
            ].into_iter().collect(),
            ports: vec![ServicePort {
                container_port: 8000,
                host_port: Some(8000),
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["python".to_string(), "-c".to_string(), "import torch; print(torch.cuda.is_available())".to_string()],
                interval: 60,
                timeout: 30,
                retries: 3,
                start_period: 120,
        ...
        services.insert("tensorboard".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
                registry: "docker.io".to_string(),
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["curl".to_string(), "-f".to_string(), "http://localhost:6006".to_string()],
                interval: 30,
                timeout: 10,
                retries: 3,
                start_period: 60,
    ...
    fn create_quantum_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "quantum-biome".to_string();
        let description = "Quantum computing research environment with simulators and hardware access".to_string();
        
        let (_, _, primals, mut services, mut resources, security, networking, storage) = self.create_basic_template();
        
        // Quantum computing services
        services.insert("qiskit".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
                registry: "registry.ecosystem.sovereignscience.org".to_string(),
            ...
            environment: vec![
                ("QISKIT_BACKEND".to_string(), "aer_simulator".to_string()),
            ].into_iter().collect(),
            ports: vec![ServicePort {
                container_port: 8888,
                host_port: Some(8888),
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["python".to_string(), "-c".to_string(), "import qiskit; print('Qiskit ready')".to_string()],
                interval: 60,
                timeout: 30,
                retries: 3,
                start_period: 120,
    ...
    fn create_genomics_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "genomics-biome".to_string();
        let description = "Bioinformatics and genomics analysis environment with secure data handling".to_string();
        
        let (_, _, primals, mut services, resources, mut security, networking, mut storage) = self.create_science_template();
        
        // Genomics services
        services.insert("bioconductor".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
                registry: "docker.io".to_string(),
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["curl".to_string(), "-f".to_string(), "http://localhost:8787".to_string()],
                interval: 30,
                timeout: 10,
                retries: 3,
                start_period: 120,
        ...
        security.forbidden_syscalls.extend(vec!["ptrace".to_string(), "process_vm_readv".to_string()]);
    ...
    fn create_vision_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "vision-biome".to_string();
        let description = "Computer vision and imaging processing environment".to_string();
        
        let (_, _, primals, mut services, resources, security, networking, mut storage) = self.create_ai_research_template();
        
        // Computer vision services
        services.insert("opencv".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
                registry: "registry.ecosystem.sovereignscience.org".to_string(),
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["python".to_string(), "-c".to_string(), "import cv2; print('OpenCV ready')".to_string()],
                interval: 60,
                timeout: 30,
                retries: 3,
                start_period: 120,
    ...
    fn create_distributed_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "distributed-biome".to_string();
        let description = "Multi-node distributed computing cluster with Songbird coordination".to_string();
        
        let (_, _, mut primals, services, mut resources, security, mut networking, mut storage) = self.create_basic_template();
    ...
    fn create_sovereign_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "sovereign-biome".to_string();
        let description = "Maximum security sovereign computing environment with zero external dependencies".to_string();
        
        let (_, _, primals, services, resources, mut security, mut networking, mut storage) = self.create_basic_template();
    ...
    fn create_development_template(&self) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        let name = "dev-biome".to_string();
        let description = "Development and testing environment with debugging tools".to_string();
        
        let (_, _, primals, mut services, resources, mut security, networking, storage) = self.create_basic_template();
        
        // Development tools
        services.insert("vscode-server".to_string(), ServiceConfig {
            version: "latest".to_string(),
            source: WorkloadSource::Container {
                registry: "docker.io".to_string(),
            ...
            dependencies: vec!["beardog".to_string()],
            health_check: Some(HealthCheck {
                command: vec!["curl".to_string(), "-f".to_string(), "http://localhost:8080".to_string()],
                interval: 30,
                timeout: 10,
                retries: 3,
                start_period: 60,
    ...
    fn create_custom_template(&self, spec: &CustomTemplateSpec) -> (String, String, HashMap<String, PrimalConfig>, HashMap<String, ServiceConfig>, BiomeResources, BiomeSecurity, BiomeNetworking, BiomeStorage) {
        ...
        for service_spec in &spec.services {
            services.insert(service_spec.name.clone(), ServiceConfig {
                version: "latest".to_string(),
                source: WorkloadSource::Container {
                    registry: "docker.io".to_string(),
                ...
                ports: service_spec.ports.iter().map(|&port| ServicePort {
                    container_port: port,
                    host_port: Some(port),
                    protocol: "tcp".to_string(),
                }).collect(),
    ...
    fn get_template_tags(&self, template: &BiomeTemplate) -> Vec<String> {
        match template {
            ...
            BiomeTemplate::Science => vec!["science".to_string(), "research".to_string(), "data-analysis".to_string()],
            BiomeTemplate::AiResearch => vec!["ai".to_string(), "ml".to_string(), "gpu".to_string(), "training".to_string()],
            BiomeTemplate::Quantum => vec!["quantum".to_string(), "simulation".to_string(), "research".to_string()],
            BiomeTemplate::Genomics => vec!["genomics".to_string(), "bioinformatics".to_string(), "secure".to_string()],
            BiomeTemplate::Vision => vec!["vision".to_string(), "imaging".to_string(), "opencv".to_string(), "gpu".to_string()],
            BiomeTemplate::Distributed => vec!["distributed".to_string(), "cluster".to_string(), "songbird".to_string()],
            BiomeTemplate::Sovereign => vec!["sovereign".to_string(), "security".to_string(), "air-gapped".to_string()],
            BiomeTemplate::Development => vec!["development".to_string(), "testing".to_string(), "tools".to_string()],
    ...
    fn manifest_to_yaml(&self, manifest: &BiomeManifest) -> Result<String> {
        ...
        yaml.push_str(&format!("# {}\n", manifest.metadata.description.as_ref().unwrap_or(&"ToadStool Biome Manifest".to_string())));
