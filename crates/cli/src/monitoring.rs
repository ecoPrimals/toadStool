...
impl MonitoringSystem {
    ...
    pub async fn start_monitoring(
        ...
        info!("📈 Starting monitoring session: {} for {:?}", session_id, target);
        ...
        self.spawn_collection_task(session_id, target, metrics, session_interval).await;
...
impl MetricsStore {
    ...
    async fn store_batch(&mut self, batch: MetricBatch) {
        for metric in batch.metrics {
            ...
            let series = self.series.entry(metric.name.clone()).or_insert_with(|| {
                TimeSeries {
    ...
    fn update_stats(&mut self, metric_name: &str, value: f64) {
        let stats = self.stats.entry(metric_name.to_string()).or_insert_with(|| {
            MetricStats {
                count: 0,
                min: f64::INFINITY,
...
impl MetricsCollector for SystemMetricsCollector {
    ...
    fn capabilities(&self) -> Vec<String> {
        vec!["cpu".to_string(), "memory".to_string(), "storage".to_string()]
    }
}
