...
async fn test_e2e_executor_full_lifecycle_standalone() {
    ...
    let list_result = executor.list_biomes(false, "json".to_string(), false, None).await;
    ...
    // 3. List again
    let list_result_2 = executor.list_biomes(false, "yaml".to_string(), true, None).await;
...
async fn test_e2e_error_propagation_clean() {
    ...
    let errors = vec![
        executor.down_biome("nonexistent1".to_string(), false, 30, false).await,
        executor.down_biome("nonexistent2".to_string(), true, 60, false).await,
        executor.show_logs("nonexistent3".to_string(), false, 50, false, None, None).await,
    ];
    
    // All should error, but cleanly
    for error in errors {
        assert!(error.is_err(), "Operation should fail for nonexistent biome");
...
async fn test_e2e_multi_format_output_standalone() {
    ...
    for format in formats {
        let exec = executor.clone();
        handles.push(tokio::spawn(async move {
            exec.list_biomes(false, format.to_string(), false, None).await
        }));
    }
    ...
    for handle in handles {
        ...
        assert!(result.is_ok(), "All output formats should work without registry");
    }
}
...
async fn test_e2e_stress_many_operations_standalone() {
    ...
    for i in 0..20 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            // Mix of operations
            let _ = exec.list_biomes(i % 2 == 0, "json".to_string(), false, None).await;
            let _ = exec.down_biome(format!("stress-{}", i), false, 30, false).await;
            let _ = exec.show_logs(format!("stress-{}", i), false, 50, false, None, None).await;
