...
pub(crate) fn run(ctx: &mut PipelineContext<'_>) -> Option<HandoffResult> {
    ...
    if ctx.config.skip_preflight {
        tracing::warn!(
            "skip_preflight=true — skipping module stuck, IOMMU, and kernel health checks"
        );
    } else {
    ...
    if ctx.is_catalyst {
        ...
        let pmc_check =
            crate::vfio::device::MappedBar::from_sysfs_rw(&ctx.config.bdf, 4096).map(|bar| {
                let pmc_read = crate::nv::register_read::RegisterRead::from_result(
                    bar.read_u32(pmc::ENABLE as usize),
                );
        ...
        match pmc_check {
            Ok((pmc, popcount)) => {
                ...
                ctx.steps.push(HandoffStep {
                    name: "anchor_release_guard".into(),
                    ok: true,
                    detail: Some(format!(
                        "PMC_ENABLE=0x{pmc:08x} (popcount={popcount}){}",
                        if popcount < ctx.hw.pmc_warm_threshold {
                            " — cold start, catalyst will warm"
                        } else {
                            " — GPU warm"
                        }
                    )),
