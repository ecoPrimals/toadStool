...
pub(crate) fn run(ctx: &mut PipelineContext<'_>) -> Option<HandoffResult> {
    ...
    if ctx.is_catalyst {
        ...
        pmc::quench_interrupts(&ctx.config.bdf, &ctx.hw.interrupt_profile, "post-settle");
    ...
    if ctx.is_catalyst {
        ...
        match crate::vfio::device::MappedBar::from_sysfs_rw(&ctx.config.bdf, bar0_size) {
            Ok(catalyst_bar0) => {
                ...
                let sovereign_snap =
                    crate::vfio::sovereign_stages::SovereignSnapshot::capture(&catalyst_bar0);
                ...
                evidence.record(
                    "fecs_cpuctl",
                    format!("{:#010x}", sovereign_snap.fecs_cpuctl),
                );
                evidence.record("fecs_pc", format!("{:#010x}", sovereign_snap.fecs_pc));
                evidence.record(
                    "gpccs_cpuctl",
                    format!("{:#010x}", sovereign_snap.gpccs_cpuctl),
                );
                ...
                evidence.record(
                    "pgraph_status",
                    format!("{:#010x}", sovereign_snap.pgraph_status),
                );
                // Probe TPC status across GPCs (generation-aware topology)
                for gpc in 0..ctx.hw.gpc_count {
                    let addr =
                        ctx.hw.tpc_base as usize + gpc as usize * ctx.hw.tpc_gpc_stride as usize;
                ...
                let imem_nonzero = imem_probe
                    .iter()
                    .filter(|&&w| w != 0 && w != 0xDEAD_DEAD)
                    .count();
                let imem_faulted = imem_probe
                    .iter()
                    .filter(|&&w| w & 0xBADF_0000 == 0xBADF_0000)
                    .count();
                ...
                if imem_nonzero > 0 && imem_faulted == 0 {
                    ...
                    for (name, base) in [("fecs", 0x40_9000usize), ("gpccs", 0x41_a000usize)] {
                        ...
                        let fw_bytes: Vec<u8> =
                            fw_data.iter().flat_map(|w| w.to_le_bytes()).collect();
