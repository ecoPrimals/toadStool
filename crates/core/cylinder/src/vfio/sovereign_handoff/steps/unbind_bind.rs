...
pub(crate) fn run(ctx: &mut PipelineContext<'_>) -> Option<HandoffResult> {
    ...
    let sibling_summary: Vec<String> = ctx
        .sibling_state
        .iter()
        .map(|(s, d)| format!("{s}: {} → unbound", d.as_deref().unwrap_or("none")))
        .collect();
    let mut detail_msg = prev_driver
        .map(|d| format!("was: {d}"))
    ...
    let siblings_clean = ctx
        .sibling_state
        .iter()
        .all(|(s, _)| guarded_sysfs::read_current_driver(s).is_none());
        ...
        && let Some(ref pr) = ctx.patch_result
    {
        ...
        let enable_path = crate::linux_paths::sysfs_pci_device_file(&ctx.config.bdf, "enable");
        ...
        let override_path =
            crate::linux_paths::sysfs_pci_device_file(&ctx.config.bdf, "driver_override");
        if let Err(e) = guarded_sysfs::sysfs_write_guarded(
            &override_path,
            &ctx.config.module_name,
            guarded_sysfs::UNBIND_TIMEOUT,
        ...
        match guarded_sysfs::insmod_guarded(&patched_path, guarded_sysfs::INSMOD_TIMEOUT) {
            Ok(()) => {
                ...
                let probe_path = crate::linux_paths::sysfs_pci_driver_bind(&ctx.config.module_name);
            ...
            Err(e) => {
                let poisoned = matches!(e, guarded_sysfs::GuardedSysfsError::KmodTimeout { .. });
