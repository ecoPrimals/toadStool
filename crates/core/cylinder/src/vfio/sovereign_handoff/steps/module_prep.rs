        ...
        match &ctx.config.module_source {
            ModuleSourceConfig::Patched { stock_module, patch_set } => {
                if kmod::is_module_loaded(&ctx.config.module_name) {
                    tracing::info!(module = ctx.config.module_name.as_str(),
                                   "module already loaded — guarded unload before patched load");
                    if let Err(e) = guarded_sysfs::rmmod_guarded(
                        &ctx.config.module_name, guarded_sysfs::RMMOD_TIMEOUT,
                    ) {
                        ctx.steps.push(HandoffStep {
                            name: "module_prep".into(), ok: false,
                            detail: Some(format!("cannot unload existing {}: {e}", ctx.config.module_name)),
                            duration_ms: t.elapsed().as_millis() as u64,
                        });

                        return Some(halt_result(&ctx.config.bdf, "module_prep", std::mem::take(&mut ctx.steps), None, false, false, ctx.overall, &[], &ctx.config.module_name, false));
                    }
                }
                ...
                match module_patch::patch_module_with_rename(&stock_path, &ps, rename_pair) {
                    Ok(pr) => {
                        ...
                        if let Err(e) = guarded_sysfs::insmod_guarded(
                            ...
                            return Some(halt_result(&ctx.config.bdf, "module_prep", std::mem::take(&mut ctx.steps), ctx.patch_result.take(), false, false, ctx.overall, &[], &ctx.config.module_name, false));
                        }
                ...
                let patch_detail = ctx.patch_result.as_ref()
            ...
            ModuleSourceConfig::System => {
                match kmod::ensure_module_loaded(&ctx.config.module_name) {
                    Ok(freshly_loaded) => {
