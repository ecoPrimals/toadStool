...
impl SymbolResolver for NmResolver {
    fn resolve(&self, ko_path: &Path) -> Result<HashMap<String, u64>, PatchError> {
        let syms =
            crate::vfio::kmod::nm_text_symbols(ko_path).map_err(|e| PatchError::NmFailed {
...
pub(crate) fn resolve_symbol_file_offsets(elf: &[u8]) -> HashMap<String, u64> {
    ...
    if e_shentsize == 0 || e_shoff == 0 || e_shnum == 0 {
        return result;
    }
    ...
    for i in 0..e_shnum {
        ...
        let sym_offset =
            u64::from_le_bytes(elf[sh + 24..sh + 32].try_into().unwrap_or([0; 8])) as usize;
        let sym_size =
            u64::from_le_bytes(elf[sh + 32..sh + 40].try_into().unwrap_or([0; 8])) as usize;
        let sym_link =
            u32::from_le_bytes(elf[sh + 40..sh + 44].try_into().unwrap_or([0; 4])) as usize;
        ...
        if strtab_sh + 32 > elf.len() {
            continue;
        }
        let strtab_off = u64::from_le_bytes(
            elf[strtab_sh + 24..strtab_sh + 32]
                .try_into()
                .unwrap_or([0; 8]),
        ) as usize;
        ...
        for j in 0..num_syms {
            ...
            if sym + entry_size > elf.len() {
                break;
            }

            let st_name =
                u32::from_le_bytes(elf[sym..sym + 4].try_into().unwrap_or([0; 4])) as usize;
            let st_info = elf[sym + 4];
            let st_shndx =
                u16::from_le_bytes(elf[sym + 6..sym + 8].try_into().unwrap_or([0; 2])) as usize;
            ...
            if st_shndx == 0 || st_shndx >= section_offsets.len() {
                continue;
            }
