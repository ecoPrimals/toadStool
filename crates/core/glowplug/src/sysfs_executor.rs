...
impl SysfsSwapExecutor {
    pub(crate) fn bdf_from_id(device: &DeviceId) -> Result<&str, SysfsSwapError> {
        match device {
    ...
    pub(crate) fn is_warm_preserving_swap(from: &str, to: &str) -> bool {
    ...
    pub(crate) fn driver_name_for_target(target: &str) -> &str {
        match target {
