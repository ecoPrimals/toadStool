...
impl Default for RuntimeConfig {
    fn default() -> Self {
        Self {
            ...
            preferences: vec!["wasm".to_string(), "container".to_string(), "native".to_string()],
            engines: HashMap::new(),
...
impl ConfigBuilder {
    ...
    pub fn add_file<P: AsRef<Path>>(mut self, path: P) -> Self {
        ...
        if path.exists() {
            ...
            warn!("Configuration file not found (optional): {}", path.display());
        }
    ...
    pub fn add_directory<P: AsRef<Path>>(mut self, dir: P) -> ConfigResult<Self> {
        ...
        for entry in entries {
            ...
            if let Some(extension) = path.extension() {
                if matches!(extension.to_str(), Some("toml") | Some("yaml") | Some("yml") | Some("json")) {
    ...
    pub fn build(self) -> ConfigResult<ToadStoolConfig> {
        ...
        let config: ToadStoolConfig = self.config.try_deserialize()?;
...
pub fn load_config() -> ConfigResult<ToadStoolConfig> {
    ...
    for config_path in get_config_paths() {
        ...
        for filename in &["toadstool.toml", "toadstool.yaml", "toadstool.yml", "toadstool.json"] {
    ...
    fn default() -> Self {
        Self {
            ...
            allowed_port_ranges: vec![
                PortRangeConfig { start: 8000, end: 8999 },
                PortRangeConfig { start: 3000, end: 3999 },
            ],
...
mod tests {
    ...
    #[ignore] // TODO: Fix config test structure
    #[test]
