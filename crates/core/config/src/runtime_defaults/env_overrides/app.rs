...
pub(super) fn apply(config: &mut ToadStoolConfig) -> ConfigResult<()> {
    ...
    if let Ok(debug) = std::env::var(socket_env::TOADSTOOL_DEBUG) {
        config.features.enable_debug = parse::parse_bool(&debug);
    }

    if let Ok(verbose) = std::env::var(socket_env::TOADSTOOL_VERBOSE) {
    ...
    if let Ok(data_dir) = std::env::var(socket_env::TOADSTOOL_DATA_DIR) {
        config.app.data_dir = data_dir;
    }

    if let Ok(cache_dir) = std::env::var(socket_env::TOADSTOOL_CACHE_DIR) {
        config.app.cache_dir = cache_dir;
    }

    if let Ok(worker_threads) = std::env::var(socket_env::TOADSTOOL_WORKER_THREADS) {
