...
pub(super) fn apply(config: &mut ToadStoolConfig) -> ConfigResult<()> {
    if let Ok(jwt_secret) = std::env::var(socket_env::TOADSTOOL_JWT_SECRET) {
        config.security.auth.jwt_secret = Some(jwt_secret);
    }

    if let Ok(session_timeout) = std::env::var(socket_env::TOADSTOOL_SESSION_TIMEOUT) {
    ...
    if let Ok(lockout_duration) = std::env::var(socket_env::TOADSTOOL_LOCKOUT_DURATION) {
    ...
    if let Ok(enabled) = std::env::var(socket_env::TOADSTOOL_ENCRYPTION_ENABLED) {
        config.security.encryption.enabled = parse::parse_bool(&enabled);
    }

    if let Ok(algorithm) = std::env::var(socket_env::TOADSTOOL_ENCRYPTION_ALGORITHM) {
        config.security.encryption.algorithm = algorithm;
    }

    if let Ok(key_length) = std::env::var(socket_env::TOADSTOOL_ENCRYPTION_KEY_LENGTH) {
        config.security.encryption.key_length =
            parse::parse_usize(&key_length, "encryption key length")?;
    }

    if let Ok(enabled) = std::env::var(socket_env::TOADSTOOL_AUDIT_ENABLED) {
    ...
    if let Ok(log_level) = std::env::var(socket_env::TOADSTOOL_AUDIT_LOG_LEVEL) {
        config.security.audit.log_level = log_level;
    }

    if let Ok(sandbox_type) = std::env::var(socket_env::TOADSTOOL_SANDBOX_TYPE) {
        config.security.sandbox.sandbox_type = sandbox_type;
    }

    if let Ok(allow_network) = std::env::var(socket_env::TOADSTOOL_SANDBOX_ALLOW_NETWORK) {
        config.security.sandbox.allow_network = parse::parse_bool(&allow_network);
    }

    if let Ok(allow_file_access) = std::env::var(socket_env::TOADSTOOL_SANDBOX_ALLOW_FILE_ACCESS) {
