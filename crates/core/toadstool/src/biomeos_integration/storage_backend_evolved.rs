...
impl StorageBackend {
    ...
    async fn get_provider(&self) -> Result<CapabilityProvider> {
        ...
        if provider_lock.is_none() {
            ...
            let discovered = CapabilityProvider::discover(capability)
                .await
                .map_err(|e| match e {
                    CapabilityError::NoProviderFound(_) => StorageBackendError::NoStorageProvider,
    ...
    pub async fn provision_volume(&self, request: VolumeRequest) -> Result<VolumeInfo> {
        ...
        let response = provider.call("storage.provision_volume", params)
            .await
    ...
    pub async fn mount_volume(&self, volume_id: &str, mount_path: PathBuf) -> Result<VolumeInfo> {
        ...
        let response = provider.call("storage.mount_volume", params)
            .await
    ...
    pub async fn unmount_volume(&self, volume_id: &str) -> Result<VolumeInfo> {
        ...
        let response = provider.call("storage.unmount_volume", params)
            .await
    ...
    pub async fn delete_volume(&self, volume_id: &str) -> Result<()> {
        ...
        provider.call("storage.delete_volume", params)
            .await
    ...
    pub async fn get_volume_status(&self, volume_id: &str) -> Result<VolumeInfo> {
        ...
        let response = provider.call("storage.get_volume_status", params)
            .await
    ...
    pub async fn list_volumes(&self) -> Result<Vec<VolumeInfo>> {
        ...
        let response = provider.call("storage.list_volumes", json!({}))
            .await
            .map_err(|e| StorageBackendError::Capability(CapabilityError::RpcFailed(e.to_string())))?;
        
        let volumes = response["volumes"]
            .as_array()
            .ok_or_else(|| StorageBackendError::Capability(
                CapabilityError::InvalidResponse("No volumes array in response".into())
            ))?;
