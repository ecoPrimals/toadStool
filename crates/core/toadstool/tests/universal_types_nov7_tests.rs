...
fn test_system_resources_creation() {
    let resources = SystemResources {
        ...
        storage_bytes: 1000 * 1024 * 1024 * 1024, // 1000 GB
    ...
    assert_eq!(resources.memory_bytes, 64 * 1024 * 1024 * 1024);
    assert_eq!(resources.storage_bytes, 1000 * 1024 * 1024 * 1024);
    assert_eq!(resources.gpu_units, 2);
    assert_eq!(resources.network_bandwidth, 10 * 1024 * 1024 * 1024);
}
...
fn test_system_resources_minimal() {
    ...
    assert_eq!(resources.storage_bytes, 10 * 1024 * 1024 * 1024);
...
fn test_system_resources_high_spec() {
    let resources = SystemResources {
        cpu_cores: 128.0,
        memory_bytes: 512 * 1024 * 1024 * 1024, // 512 GB
        storage_bytes: 10000 * 1024 * 1024 * 1024, // 10 TB
        network_bandwidth: 100 * 1024 * 1024 * 1024, // 100 Gbps
        gpu_units: 8,
        special_hardware: HashMap::new(),
    };
    
    assert!(resources.cpu_cores >= 64.0);
    assert!(resources.memory_bytes >= 256 * 1024 * 1024 * 1024);
    assert!(resources.gpu_units >= 4);
}

#[test]
fn test_system_resources_clone() {
    let mut special_hw = HashMap::new();
    special_hw.insert("tpu".to_string(), 4);
    ...
    let resources = SystemResources {
        ...
        gpu_units: 1,
        special_hardware: special_hw,
    };
    
    let cloned = resources.clone();
    assert_eq!(resources.cpu_cores, cloned.cpu_cores);
    assert_eq!(resources.memory_bytes, cloned.memory_bytes);
    assert_eq!(resources.gpu_units, cloned.gpu_units);
    assert_eq!(resources.special_hardware.len(), cloned.special_hardware.len());
}
