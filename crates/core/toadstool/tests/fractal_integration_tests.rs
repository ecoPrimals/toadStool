...
use toadstool::fractal_integration::{FractalRuntime, FractalServiceAdvertiser, BarracudaIntegration};
use toadstool::layer_adaptation::{LayerCapabilityAdapter, GpuAccess, StorageType, NetworkAccess};
...
async fn test_fractal_runtime_full_initialization() {
    ...
    assert!(result.is_ok(), "FractalRuntime initialization should succeed");
    ...
    assert!(!caps.to_capability_list().is_empty(), "Should have capabilities");
    
    // Should have valid identity
    let identity = runtime.identity();
    let identity_read = identity.read().await;
    assert_eq!(identity_read.primal_type, "toadstool");
    assert!(!identity_read.capabilities.is_empty(), "Identity should have capabilities");
}
...
async fn test_capability_adaptation_validity() {
    ...
    assert!(
        matches!(caps.compute.gpu_access, 
    ...
    assert!(
        matches!(caps.storage.storage_type, 
            StorageType::DirectBlock | StorageType::HostFilesystem | StorageType::CloudObject | StorageType::PersistentVolume),
        "Storage type should be valid"
    );
    
    // All layers should have some network access
    assert!(
        matches!(caps.network.network_access,
            NetworkAccess::Direct | NetworkAccess::HostNamespace | NetworkAccess::CloudVPC),
        "Network access should be valid"
    );
    
    // Metadata should be populated
    assert!(!caps.metadata.layer.is_empty(), "Layer metadata should exist");
}
...
async fn test_multiple_runtime_instances() {
    ...
    assert_ne!(id1_read.instance_id, id2_read.instance_id, 
...
async fn test_capability_list_comprehensive() {
    ...
    match layer {
        DeploymentLayer::BareMetalOS => {
            ...
            assert!(cap_list.iter().any(|c| c.contains("direct") || c.contains("cpu")));
        }
        DeploymentLayer::ContainerLayer { .. } => {
            // Container should have host or container capabilities
            assert!(cap_list.iter().any(|c| c.contains("host") || c.contains("cpu")));
        }
        DeploymentLayer::CloudLayer { .. } => {
            // Cloud should have cloud capabilities
            assert!(cap_list.iter().any(|c| c.contains("cloud") || c.contains("cpu")));
        }
        _ => {
...
async fn test_runtime_identity_capability_sync() {
    ...
    for runtime_cap in runtime_cap_list {
        let found = identity_read.capabilities.iter()
            .any(|id_cap| id_cap.name == runtime_cap);
        
        assert!(found, "Identity should have runtime capability: {}", runtime_cap);
    }
}
...
async fn test_all_layer_adaptations() {
    ...
    // Test middleware
    let adapter = LayerCapabilityAdapter::new(
...
async fn test_runtime_init_performance() {
    ...
    assert!(duration.as_secs() < 10, "Init should complete within 10 seconds, took {:?}", duration);
}
...
async fn test_layer_detection_caching() {
    ...
    println!("First detection: {:?}, Second detection: {:?}", duration1, duration2);
}
