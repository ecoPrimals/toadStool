...
use toadstool_common::runtime_ports::{discover_available_port, discover_port_with_preference, RuntimePortDiscovery};
...
fn test_runtime_port_discovery_with_preference() {
    ...
    assert!(port.is_ok(), "Should discover port (preferred or alternative)");
...
fn test_preferred_port_fallback() {
    ...
    assert!(port.is_ok(), "Should discover alternative when preferred unavailable");
