...
mod tests {
    ...
    // ── Additional coverage: error branches, edge cases (20% uncovered paths) ──
    ...
    // ── Additional coverage: error branches, edge cases (20% uncovered) ──
    ...
    async fn test_create_fallback_service_unknown_protocol_uses_http() {
        ...
        let services = engine.discover_by_capability(&cap).await.unwrap();
        assert_eq!(services.len(), 1);
        assert!(services[0].endpoints[0].protocol.len() > 0);
    }
