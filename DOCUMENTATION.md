...
- **8,521 lib tests**, 0 failures — this is `--lib` only, and is **39% of the repo's test functions**; ~13,102 more live in `tests/` directories that CI neither builds nor runs (S382, D-TEST-COMPILE). 0 fmt diffs; 192 lib clippy warnings after the 1.97.1 toolchain bump (D-TOOLCHAIN-DRIFT). Full workspace concurrent test suite.
