...
- **8,521 lib tests**, 0 failures — this is `--lib` only, and is **39% of the repo's test functions**; ~13,102 more live in `tests/` directories. As of S383 every test target **compiles** (`cargo test --workspace --no-run` exits 0, now a CI gate); whether CI **runs** them is still open (D-TEST-COMPILE). 0 fmt diffs; 192 lib clippy warnings after the 1.97.1 toolchain bump (D-TOOLCHAIN-DRIFT). Full workspace concurrent test suite.
