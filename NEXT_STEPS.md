...
**Updated**: Jun 2026 — S306. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete.
...
**Latest**: S306 — Deep debt XIII: `bar_cartography.rs` split (types/scan/display/helpers), `amd/ioctl.rs` split (types/ops), `ServiceMeshType` vendor variants removed, deprecated audit. S305 — sync ctor migration + `#[allow]` → `#[expect]`. S304 — Category A elimination (-300L).
