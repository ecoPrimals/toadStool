...
**~85%+ estimated line coverage** (lib-only). **8,521 lib tests** (0 failures). Target 90%.
...
| Item | Status |
|------|--------|
| Coverage push 85%→90% | **Active sprint** — S294–S336 expanded to 8,521 lib; remaining gap in VFIO/DRM/GPU hardware paths |
...
| E2E sovereign pipeline test | **WIRING VALIDATED, NOT E2E** (S263) — warm handoff → VFIO open → channel → dispatch → readback all execute, but **no shader has ever run on this path**. The readback returns without a GR result, so "E2E" names the plumbing, not the compute. Blocked behind runlist + FECS PENDING_CTX_RELOAD. No sovereign numeric result exists on any NVIDIA GPU as of Aug 16. |
...
- [ ] **Test coverage target 90%** -- 8,521 lib tests; ~85%+ line; S294–S336 expanding coverage; remaining gap in hardware-dependent paths (VFIO, DRM, V4L2, akida); push to 90% ongoing
