...
**Updated**: Jul 30, 2026 — S347 (Windows cross-compile fix, Wave 155j P1). **9,193+ lib tests, 0 failures.** `toadstool.exe` compiles (Windows depot unblocked). **S346** — security fail-closed, unsafe containment, 75 doc warnings fixed. **S344** — deny.toml 19+ bans, crypto best-effort, MMIO consolidated.
...
**Latest**: S347 — Windows cross-compile fix (Wave 155j P1). `toadstool-runtime-gpu` moved from Linux-only to cross-platform optional dep tied to `gpu-discovery`. `RuntimeEngineDispatch::Gpu` cfg-gated to `all(linux, gpu-discovery)`. `cargo check --target x86_64-pc-windows-msvc` passes. Unblocks `toadstool.exe` in depot. S346 — security fail-closed, unsafe containment, 75 rustdoc warnings fixed. S344 — deny.toml 19+ bans, crypto best-effort, MMIO consolidated.
