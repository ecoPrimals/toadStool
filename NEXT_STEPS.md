...
**Updated**: Jun 2026 — S311. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete. **PRIMAL-SOCKET-CLEANUP done** (Wave 107). **TOADSTOOL-AUTO-REGISTER done** (Wave 111). **riboCipher SHIPPED** (Wave 111 — server detect + client signal).
...
**Latest**: S311 — riboCipher transport signal convergence: server-side detection on all JSON-RPC accept loops (Unix + TCP), client-side `[0xEC, 0x01]` signal on all outbound IPC (register, discover, announce, UnixJsonRpcClient, ConnectedJsonRpcClient). Legacy connections WARN per Wave 111 deprecation timeline. Tier 2/3 (mito/nuclear) stubs present.
