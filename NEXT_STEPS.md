...
**Updated**: Jul 16, 2026 — S336+. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete. **PRIMAL-SOCKET-CLEANUP done** (Wave 107). **TOADSTOOL-AUTO-REGISTER done** (Wave 111). **riboCipher REJECT** (Wave 113 — unsignalled connections rejected with error response). **Cross-architecture** — `cargo check --target x86_64-pc-windows-gnu` passes (S329). **Phase 2 Silicon Atheism** — cross-platform GPU backends: `WgpuGpuDiscovery`, `PortableSwapExecutor`, `PortableResourceHandle` (S332). **S336** — security_impl migrated to crypto_integration (#![expect(deprecated)] removed), dead channels feature removed (12 cfg blocks), test extraction wave 6 (2 files, -173L). **S335** — doc-comment primal name sweep (22 replacements, 9 files), test extraction waves 4-5 (10 files, −1,103L), dead code elimination (DispatchJob.id removed).
**Status**: Production-grade | Rust edition **2024** (MSRV 1.85) | **AGPL-3.0-or-later** | **All quality gates green** | tests verified (23,000+ workspace, 0 failures; **9,232+ lib-only**) | **112 JSON-RPC methods** (direct) | Wire Standard L3 (partial) | **Zero `libc`** (ecoBin v3.0 — rustix for all hardware I/O) | **Zero production panics/expects/unwraps** | **Zero production TODO/FIXME/HACK** | IPC-first | workspace `unsafe_code = "deny"`, **41 crates `forbid`** | **44 unsafe blocks** (all SAFETY-documented) | **rustix 1.x workspace-wide** | **100% env centralized** (zero production raw env string literals, S321; 410+ reads via socket_env constants) | **capability-based primal references** | **`async-trait` banned in `deny.toml`** | **Phase D dispatch live** | **E2E sovereign dispatch VALIDATED on Titan V** | **Zero production files >750L** | **Zero clippy warnings** (S335) | **Phase 2 GPU backends** — `WgpuGpuDiscovery`, `PortableSwapExecutor`, `PortableResourceHandle` (S332) | **Borrowed deserialization** (S331) | **Cross-arch cfg-gating** (S329: 134 files)
**Latest**: S336 — security_impl migrated to crypto_integration (#![expect(deprecated)] removed), dead channels feature removed (12 cfg blocks), test extraction wave 6 (2 files, -173L). S335 — doc-comment primal name sweep (22 replacements, 9 files), test extraction waves 4-5 (10 files, −1,103L), dead code elimination (DispatchJob.id removed). S334 — test extraction waves 2–3: 11 files extracted (−1,888 lines), 750L gate cleared, dispatch clone polish. S333 — structural debt: 7 large files refactored via test extraction (−2,188 production lines: core.rs 744→325, load_balancer.rs 517→141, dispatch.rs 582→193, paths.rs 592→284, kernel_sentinel.rs 556→327, catalyst_watchdog.rs 691→474, ember.rs 502→308), hardcoded "BearDog" → "crypto provider" in BTSP relay. S332 — Phase 2 Silicon Atheism: `WgpuGpuDiscovery`, `PortableSwapExecutor`, `PortableResourceHandle` (+26 tests). S331 — borrowed deserialization sweep (6 files, `Deserialize::deserialize(&Value)` replacing `from_value(v.clone())`), test race fix. S330 — deep debt: clone elimination, +31 tests, zero clippy. S329 — cross-architecture adoption (134 files `cfg`-gated). S328 — DH-1 `/tmp` fix (3-tier runtime dir). S327 — hot-path clone elimination (6 files: `Deserialize(v)` replacing `from_value(v.clone())` in fan_out/pipeline/sovereign/init/profile, dedup request.id clone, decrypt_result reorder), timeout invariant tests (+9), socket env invariant tests (+7), submit_params test expansion (+11), router decomposition (776→441L + router_tests.rs 230L). 9,171+ lib tests. S326 — router contract tests (7 tests, routing table safety net), graph node coverage (+11 tests, duration serde + builder paths), glowplug PCI discovery split (glowplug_client 642→467L + glowplug_discovery 192L). 9,145+ lib tests. S325 — kernel sentinel coverage (28 tests), biomeos path consolidation (3 sites → `get_biomeos_dir()`), hot-path clone elimination (submit_params + dispatcher), discovery fallback production gate (`TOADSTOOL_ENV=production` disables localhost fallback). S324 — test graduation (6 un-ignored), catalyst watchdog coverage (15 tests), dispatcher mock E2E (7 tests), MMIO split (613→275+179+194L). S323 — test extraction (method_gate 644→279, job 652→277, shader_dispatch 590→471, 63 tests moved), submit param split (642→487+174L), cpu_resource resilience extraction (675→536+154L), flaky background test fix (un-ignored), edge communication coverage (+9 tests). 9,145+ lib tests. S322 — client riboCipher mock fix (12 tests unblocked), composition tests graduated from quarantine (+11), ipc_watch coverage (9 tests), mmio/trials test extraction, telemetry consumer constants. S321 — env literal purge, Duration dedup, dep unification, reagent refactor. S320 — Wave 114 mito-beacon acceptance. S319 — gRPC + OpenCL deleted. S318 — Router split + lint hygiene. S317 — 6 deprecated ctors deleted. S316 — File splits. S315 — Wave 113 compliance.
...
### Jun 14, 2026 — S315 Wave 113 Compliance

| Item | Status |
|------|--------|
| Bare `"health"` JSON-RPC method → `{status, primal, version}` (guideStone mandate) | **DONE** |
| Early-health responder accepts riboCipher `[0xEC, 0x01]` prefix | **DONE** |
| Wave 113 REJECT: unsignalled connections on Unix/TCP/BTSP return `-32600` error | **DONE** |
| MITO/NUCLEAR tiers send error response instead of silent close | **DONE** |
| `"health"` added to `DIRECT_JSONRPC_METHODS`, method_gate, wire_l3 | **DONE** |
| Connection tests updated to send riboCipher signal (11 tests + 2 new) | **DONE** |

### Jun 14, 2026 — S314 Deprecated Symbol Evolution

| Item | Status |
|------|--------|
| `node_type::{BEARDOG,SONGBIRD,NESTGATE}` — 3 legacy constants deleted (0 production callers) | **DONE** |
| `FeatureFlags::enable_grpc` — dead field removed (populated but never read) | **DONE** |
| `DISTRIBUTED_URL` + `get_distributed_storage_url()` — dead API bundle removed | **DONE** |
| `TOADSTOOL_ENABLE_GRPC` env constant — deprecated | **DONE** |
| Tests updated (discovery client, config overrides, types comprehensive) | **DONE** |
...
| Item | Status |
|------|--------|
| Coverage push 85%→90% | **Active sprint** — S294–S330 expanded to 9,206+ lib; remaining gap in VFIO/DRM/GPU hardware paths |
...
- [ ] **Test coverage target 90%** -- 23,000+ tests (9,206+ lib-only); ~85%+ line; S294–S330 expanding coverage; remaining gap in hardware-dependent paths (VFIO, DRM, V4L2, akida); push to 90% ongoing
...
Session history for S43–S328+ lives in [CHANGELOG.md](CHANGELOG.md). Fossil record for S87–S240 archived to `ecoPrimals/infra/wateringHole/fossilRecord/toadstool/`.
