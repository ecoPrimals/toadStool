...
**Updated**: Jun 20, 2026 — S321+. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete. **PRIMAL-SOCKET-CLEANUP done** (Wave 107). **TOADSTOOL-AUTO-REGISTER done** (Wave 111). **riboCipher REJECT** (Wave 113 — unsignalled connections rejected with error response).
...
**Latest**: S321 — env literal purge (zero raw env strings), Duration dedup, dep unification (bytes/ruzstd/serialport/ndarray), reagent/mod.rs smart refactor (704L→3 files). S320 — Wave 114 mito-beacon acceptance. S319 — gRPC + OpenCL deleted. S318 — Router split + lint hygiene. S317 — 6 deprecated ctors deleted. S316 — File splits. S315 — Wave 113 compliance.
