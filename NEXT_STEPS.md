...
**Updated**: Jun 2026 — S314. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete. **PRIMAL-SOCKET-CLEANUP done** (Wave 107). **TOADSTOOL-AUTO-REGISTER done** (Wave 111). **riboCipher COMPLIANT** (Wave 112 — server detect + client signal, ERROR on unsignalled).
...
**Latest**: S314 — Deprecated symbol evolution: 3 legacy node_type constants deleted, `enable_grpc` dead field removed, `DISTRIBUTED_URL` dead API removed. S313 — 3 `unreachable!()` → typed errors, `unix.rs` split. S312 — riboCipher WARN→ERROR.
