...
**Updated**: Jun 2026 — S315. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete. **PRIMAL-SOCKET-CLEANUP done** (Wave 107). **TOADSTOOL-AUTO-REGISTER done** (Wave 111). **riboCipher REJECT** (Wave 113 — unsignalled connections rejected with error response).
...
**Latest**: S315 — Wave 113 compliance: bare `health` method, early-health riboCipher support, unsignalled connections REJECTED. S314 — Deprecated symbol evolution. S313 — `unreachable!()` → typed errors, `unix.rs` split.
