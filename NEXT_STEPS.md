...
**Updated**: Jul 29, 2026 — S346 (strandGate deep evolution sprint). **9,193+ lib tests, 0 failures.** Security fail-closed. Unsafe containment. 75 doc warnings fixed. 27 crates version-aligned. **S344** — deny.toml 19+ bans, crypto best-effort, MMIO consolidated. **S343** — wgpu system queries + dispatch.
...
**Latest**: S346 — Deep debt evolution sprint. Security fail-closed (sandbox PlatformNotSupported, PKI requires TOADSTOOL_STANDALONE). Entropy hardened (getrandom). Silent stubs evolved (BLE deploy, migration verify). Unsafe containment (madvise→hw-safe, DRM ioctl→hw-safe, systemd fds→hw-safe, SPIR-V→runtime/gpu). 75 rustdoc warnings fixed. 27 crates → version.workspace. Magic numbers extracted. Hardcoded /run/toadstool eliminated. S344 — deny.toml 19+ bans, crypto best-effort, MMIO consolidated. S343 — wgpu system queries + dispatch.
