...
**Updated**: Jun 2026 — S307. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete.
...
**Latest**: S307 — Deep debt XIV: `registers.rs` split (pri/cg/pclock), `pm4.rs` test extract, `swap.rs` test extract, stale test cleanup (25 tests removed), unfulfilled lint hygiene. S306 — `bar_cartography.rs` + `amd/ioctl.rs` splits, `ServiceMeshType` removed. S305 — sync ctor migration + `#[allow]` → `#[expect]`.
