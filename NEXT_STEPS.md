...
**Updated**: Jul 15, 2026 — S330+. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete. **PRIMAL-SOCKET-CLEANUP done** (Wave 107). **TOADSTOOL-AUTO-REGISTER done** (Wave 111). **riboCipher REJECT** (Wave 113 — unsignalled connections rejected with error response). **Cross-architecture** — `cargo check --target x86_64-pc-windows-gnu` passes (S329).
**Status**: Production-grade | Rust edition **2024** (MSRV 1.85) | **AGPL-3.0-or-later** | **All quality gates green** | tests verified (23,000+ workspace, 0 failures; **9,206+ lib-only**) | **112 JSON-RPC methods** (direct) | Wire Standard L3 (partial) | **Zero `libc`** (ecoBin v3.0 — rustix for all hardware I/O) | **Zero production panics/expects/unwraps** | **Zero production TODO/FIXME/HACK** | IPC-first | workspace `unsafe_code = "deny"`, **41 crates `forbid`** | **44 unsafe blocks** (all SAFETY-documented; −2 from kernel_sentinel AsFd evolution) | **rustix 1.x workspace-wide** | **100% env centralized** (zero production raw env string literals, S321; 410+ reads via socket_env constants; 22 LEGACY reads emit deprecation tracing) | **capability-based primal references** (`PRIMAL_NAME`/`PRIMAL_BINARY_NAME` constants) | **`async-trait` banned in `deny.toml`** | **Phase D dispatch live** | **E2E sovereign dispatch VALIDATED on Titan V** | **Telemetry wire contract v1.1** (barraCuda/biomeOS L5) | **`--headless` mode** for port-free VPS deployment | **`--socket` wired** for launcher-injected UDS paths | **Zero production `#[allow]`** (Wave 78 compliant) | **`capability_registry.toml`** (17 capability groups, 112 methods) | **Zero `/tmp/` hardcoding** — `BIOMEOS_SOCKET_DIR` > `XDG_RUNTIME_DIR` > `temp_dir` | **`TRANSPORT_ENDPOINT` accepted** (sourDough standard, Wave 100) | **BYOB default bind `127.0.0.1`** | **Zero production files >750L** | **~25 deprecated symbols removed** | **Zero sync-ctor fallbacks** (auth/agents fully async) | **Auto-register hardware** — PCI sysfs GPU/NPU inventory sent in `ipc.register` + `primal.announce` | **`CoordinationTransport::GRPC` DELETED** (S319 — no gRPC in ecoPrimals) | **riboCipher REJECT** — Wave 113 enforced on all accept loops | **GuideStone `health` method** — `{status, primal, version}` on bare `"health"` | **Zero unfulfilled `#[expect]`** | **Workspace deps unified** (bytes/ruzstd/serialport/ndarray, S321) | **Duration dedup** (CPU_USAGE_SAMPLE_WINDOW + 8 named constants, S321)
**Latest**: S330 — deep debt: `decrypt_result` clone elimination (by-value ownership), +31 tests (unix connection 12, discovery engine 11, execution config 8), zero clippy warnings workspace-wide. 9,206+ lib tests. S329 — cross-architecture adoption: `cargo check --target x86_64-pc-windows-gnu` passes (134 files `cfg`-gated; hw-safe/display/cylinder/nvpmu/akida/gpu behind `#[cfg(target_os = "linux")]`, Unix IPC behind `#[cfg(unix)]`). S328 — DH-1 `/tmp` fix (3-tier runtime dir: XDG > `/run/membrane/<user>` systemd > temp_dir). S327 — hot-path clone elimination (6 files: `Deserialize(v)` replacing `from_value(v.clone())` in fan_out/pipeline/sovereign/init/profile, dedup request.id clone, decrypt_result reorder), timeout invariant tests (+9), socket env invariant tests (+7), submit_params test expansion (+11), router decomposition (776→441L + router_tests.rs 230L). 9,171+ lib tests. S326 — router contract tests (7 tests, routing table safety net), graph node coverage (+11 tests, duration serde + builder paths), glowplug PCI discovery split (glowplug_client 642→467L + glowplug_discovery 192L). 9,145+ lib tests. S325 — kernel sentinel coverage (28 tests), biomeos path consolidation (3 sites → `get_biomeos_dir()`), hot-path clone elimination (submit_params + dispatcher), discovery fallback production gate (`TOADSTOOL_ENV=production` disables localhost fallback). S324 — test graduation (6 un-ignored), catalyst watchdog coverage (15 tests), dispatcher mock E2E (7 tests), MMIO split (613→275+179+194L). S323 — test extraction (method_gate 644→279, job 652→277, shader_dispatch 590→471, 63 tests moved), submit param split (642→487+174L), cpu_resource resilience extraction (675→536+154L), flaky background test fix (un-ignored), edge communication coverage (+9 tests). 9,145+ lib tests. S322 — client riboCipher mock fix (12 tests unblocked), composition tests graduated from quarantine (+11), ipc_watch coverage (9 tests), mmio/trials test extraction, telemetry consumer constants. S321 — env literal purge, Duration dedup, dep unification, reagent refactor. S320 — Wave 114 mito-beacon acceptance. S319 — gRPC + OpenCL deleted. S318 — Router split + lint hygiene. S317 — 6 deprecated ctors deleted. S316 — File splits. S315 — Wave 113 compliance.
...
**~85%+ estimated line coverage** (lib-only). **9,206+ lib tests** (0 failures). Target 90%.
