...
**Updated**: Jun 2026 — S316. **VPS-ready** — musl-static binary built with `--headless` support. All P0 blockers resolved. Transport evolution Phase 2 complete. **PRIMAL-SOCKET-CLEANUP done** (Wave 107). **TOADSTOOL-AUTO-REGISTER done** (Wave 111). **riboCipher REJECT** (Wave 113 — unsignalled connections rejected with error response).
...
**Latest**: S316 — Deep Debt XVII: file splits (`cpu_resource.rs` 749→673, `glowplug_client.rs` 729→635) + `TOADSTOOL_ENABLE_GRPC` deleted. S315 — Wave 113 compliance: bare `health` method, riboCipher REJECT. S314 — Deprecated symbol evolution.
