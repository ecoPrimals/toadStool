...
### Recently Completed
- **S229 (May 8, 2026)**: **JH-0 — MethodGate Adoption** — Pre-dispatch capability gate per primalSpring `METHOD_GATE_STANDARD.md`. All ~65 methods classified as Public (health, identity, capabilities, provenance, auth) or Protected (dispatch, workloads, hardware, transport). Ships `Permissive` (all calls allowed). 3 new `auth.*` methods: `auth.check`, `auth.mode`, `auth.peer_info`. Error codes: `-32005 AUTH_REQUIRED`, `-32006 PERMISSION_DENIED`. JH-2 (resource envelope enforcement) tracked, blocked on BearDog JH-1. +21 tests.
