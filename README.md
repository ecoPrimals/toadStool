...
**Last Updated**: May 2026 — S238. **22,843+** workspace tests, 0 failures (7,896+ lib-only). ~83.6% lib-only line coverage (target 90%). **65 JSON-RPC methods** (direct) + semantic registry with **Wire Standard L3** (cost_estimates + operation_dependencies). AGPL-3.0-or-later. Zero C FFI deps (ecoBin v3.0). **46 unsafe blocks** (all in hw-safe/GPU/VFIO/display/plugin containment crates); all SAFETY-documented; workspace `unsafe_code = "deny"`, **41 crates `forbid`** + 5 hw crates with narrow `#[allow(unsafe_code, reason)]`. **Zero production panics/expects**. IPC-first JSON-RPC (dual-socket: `compute.sock` + `compute-tarpc.sock`). Rust 1.85+ (edition 2024, MSRV). **Wave 8 Phase A: coral-ember absorbed** (S237). **JH-2 FULLY RESOLVED** (all 3 envelope dimensions enforced — S238). **20+ magic numbers extracted** to named constants (S238). **MethodGate JH-0** (pre-dispatch capability gate, S229). **`TOADSTOOL_AUTH_MODE`** env var for enforcing mode (DF-2, S233). **Capability-based discovery compliant** per `CAPABILITY_BASED_DISCOVERY_STANDARD.md` v1.2.
