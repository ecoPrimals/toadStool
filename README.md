...
### Recently Completed
- **S236 (May 11, 2026)**: **Deep Debt — Magic Numbers, Match Safety, Test Refactor** — Extracted magic numbers in `DiscoveryDefaults` and `DiscoveryConfig` to named constants (`DISCOVERY_TIMEOUT_SECS`, `DISCOVERY_REFRESH_SECS`, `DISCOVERY_CACHE_TTL_SECS`, `DISCOVERY_MAX_RETRIES`, `DISCOVERY_RETRY_DELAY_SECS`, `DEFAULT_DISCOVERY_INTERVAL_SECS`, `DEFAULT_SERVICE_TIMEOUT_SECS`, `DEFAULT_MAX_SERVICES`, `EcosystemDiscoverer::DEFAULT_TIMEOUT_SECS`). Eliminated `unreachable!()` in `nvpmu/dma.rs` via exhaustive match with early return. Smart-refactored `dispatch/tests.rs` (1020 LOC monolith) into `tests/` directory with 4 responsibility-scoped submodules: `core_dispatch.rs`, `shader.rs`, `envelope.rs`, `trio_contract.rs`. 72 dispatch tests, clippy clean workspace-wide.
- **S235 (May 11, 2026)**: **Wave 8 Compute Trio Foundation** — BrainChip vendor ID fixed (`0x1E7C` canonical across all crates). `compute.dispatch.submit` trio-standard IPC contract: accepts `binary_b64` (base64, preferred over legacy u8 array), `shader_info`, `dispatch_dims`, buffer `data_b64`; responses include `timing { dispatch_ms, readback_ms }`. `dispatch_capabilities` returns `gpu_count`, `architectures` (e.g. `["sm75", "rdna3"]`), `vfio_status` for Gate 2. Per-GPU `architecture` field on VFIO/DRM GPU entries. Diesel engine absorption roadmap documented (Phases A-D: ember, glowplug, cylinder, local dispatch). +9 tests.
...
**Last Updated**: May 2026 — S234 (IPC env var expansion contract documented). **22,843+** workspace tests, 0 failures (7,896+ lib-only). ~83.6% lib-only line coverage (target 90%). **65 JSON-RPC methods** (direct) + semantic registry with **Wire Standard L3** (cost_estimates + operation_dependencies). AGPL-3.0-or-later. Zero C FFI deps (ecoBin v3.0). **46 unsafe blocks** (all in hw-safe/GPU/VFIO/display/plugin containment crates); all SAFETY-documented; workspace `unsafe_code = "deny"`, **41 crates `forbid`** + 5 hw crates with narrow `#[allow(unsafe_code, reason)]`. **Zero production panics/expects**. IPC-first JSON-RPC (dual-socket: `compute.sock` + `compute-tarpc.sock`). Rust 1.85+ (edition 2024, MSRV). **BTSP Phase 3 encrypted channel** (ChaCha20-Poly1305, S215; transport switch verified S218). **PG-46 resolved** (socket reuse, S214). **Self-registration** with Songbird via `DISCOVERY_SOCKET` (S207). **Capability-based discovery compliant** per `CAPABILITY_BASED_DISCOVERY_STANDARD.md` v1.2. **MethodGate JH-0** (pre-dispatch capability gate, S229). **JH-2 full envelope enforcement** (mem, cpu, timeout, method allowlist — S232). **`TOADSTOOL_AUTH_MODE`** env var for enforcing mode (DF-2, S233).
