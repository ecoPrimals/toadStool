...
### Recently Completed
- **S231 (May 8, 2026)**: **JH-2 — Resource Envelope Enforcement at Dispatch** — `ResourceEnvelope` type (`mem_mb`, `cpu_cores`, `method_allowlist`), `CallerContext` (identity + envelope), `enforce_envelope()` wired into `compute.dispatch.submit`. Gate `check_with_context()` enforces method allowlists from tokens. `auth.peer_info` now reflects caller context. Permissive-by-default (anonymous callers pass). Full enforcement activates when BearDog JH-1 ships ionic tokens. +17 tests (767 total in server).
- **S230 (May 8, 2026)**: **Error Code Alignment** — `UNAUTHORIZED` → `-32000`, `PERMISSION_DENIED` → `-32001` per ecosystem METHOD_GATE_STANDARD v1.0. Workload codes relocated (`-32010`/`-32011`/`-32012`).
- **S229 (May 8, 2026)**: **JH-0 — MethodGate Adoption** — Pre-dispatch capability gate per primalSpring `METHOD_GATE_STANDARD.md`. All ~65 methods classified as Public/Protected. Ships `Permissive`. 3 new `auth.*` methods. +21 tests.
