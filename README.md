...
**Sovereign Compute Hardware** | Pure Rust | ecoBin | Aug 17, 2026 | S382 | v0.2.0

---

## What Is This?

**ToadStool** is the hardware infrastructure primal — the **WHERE** in the Compute Trio (barraCuda = WHAT, toadStool = WHERE, coralReef = HOW). It discovers GPUs, NPUs, CPUs at runtime via sysfs/PCIe — natively, with no vendor tooling: identity, bound driver, driver version, and liveness all come from the kernel, so accelerators are found whatever driver is bound, or none. JSON-RPC 2.0 + tarpc IPC over Unix sockets. GPU job queue with cross-gate routing. **All-silicon pipeline**: discovers every functional unit on the GPU die (shader cores, tensor cores, RT cores, TMUs, ROPs, rasterizer, depth buffer, tessellator, video encoder) and routes work to the cheapest unit that meets the requested tolerance.
...
| `cargo test --workspace` | **8,521 lib tests, 0 failures** (lib-only default); **~20** ignored (hardware-gated); full workspace ~8m on Dual EPYC |
...
| JSON-RPC surface | **128** JSON-RPC methods (direct) + semantic registry |
...
### JSON-RPC Methods (126 direct + semantic registry; S286+ adds `dispatch.verify_trust`, `dispatch.telemetry.schema`)
...
|   |   +-- edge/                  (excluded S378 — orphaned crate, zero workspace dependents)
...
10. **100% unsafe documentation** -- every `unsafe` block has `// SAFETY:` comments (160 blocks, all justified; all in designated containment crates: hw-safe, cylinder, nvpmu, display, runtime/gpu, akida-driver)
...
| Doc warnings | 0 |
| Build warnings | 0 |
| Workspace tests | **8,521 lib**, 0 failures |
...
### Active / Next
- **Test coverage** -- pushing toward 90% target; 8,521 lib tests; ~85%+ lib-only line (185K lines instrumented); remaining gap: hardware-dependent paths (VFIO, DRM, V4L2), specialty runtimes
...
- **S373 (Aug 9, 2026)**: **Deep Debt — Large File Refactoring, Hardcoding Removal, Doc Completeness** — Smart decomposition: `platform_backends.rs` (962→797L, process isolation extracted), `capabilities.rs` (922→813L, PCIe config extracted), `vfio/mod.rs` (877→738L, bind helpers extracted). Hardcoding evolved: huge_page_size from 2MiB literal to /proc/meminfo discovery, ROCm path to `$ROCM_PATH` env, GPU memory fraction to env-configurable, AMD detection to runtime probing. All `toadstool-core` execution types given full doc comments. Unsafe audit: all 65+ files justified (hardware I/O containment). External deps audit: already pure Rust (zero libc/nix/C). Production mocks audit: all proper null-object patterns. Zero errors, zero warnings.
- **S372 (Aug 9, 2026)**: **Vertebrate Evolution — Self-Audit + Types Extraction** — Self-audit aligned `capability_registry.toml` with actual RPC surface (14 missing science/inference methods added → 126 total across 18 capabilities). Phase 1: extracted `workload` module (3,095 lines, 17 files) from monolithic `toadstool` crate to WASM-capable `toadstool-core`. Phase 2: extracted `resources` types, `security` types/context/policy, `encryption` types/config/error, `execution` data types to `toadstool-core` with backward-compatible re-exports. Removed dead `zeroize` + `humantime-serde` deps from main crate. `cargo clean` reclaimed 5.2 GiB. All targets pass.
- **S371 (Aug 8, 2026)**: **Full Tier 3 Abstraction — 24/48 Crates on WASM** — Deep abstraction sprint: `tokio::sync::RwLock` → `std::sync::RwLock` in security-monitoring/adaptive. Feature-gated runtime deps in ember (plx_keepalive), cylinder, glowplug (swap/plx), runtime-universal (rayon+wgpu optional), runtime-orchestration (vestigial tokio removed), runtime-adaptive (etcetera/fs cache), integration-security (discovery module), cross-substrate-validation (bin-only tokio). ComputeUnitDispatch made `#[non_exhaustive]` with wildcard match arms for graceful feature degradation. 50% of codebase now deployable on browser/WASI. See `docs/CROSS_ARCH.md`.
- **S370 (Aug 8, 2026)**: **Tier 3 WASM Compute Subset — 10 Crates on wasm32** — Feature-gated `tokio` behind `toadstool-common`'s `runtime` feature (default-enabled). Crates needing only types/traits use `default-features = false`, eliminating tokio/mio/socket2 from their dep graph. Result: 10 compute-core crates compile on both `wasm32-unknown-unknown` and `wasm32-wasip1` — hw-safe, core, sysmon, hw-learn, nvpmu, akida-chip, akida-models, akida-setup, management-resources, runtime-secure-enclave. See `docs/CROSS_ARCH.md`.
- **S369 (Aug 8, 2026)**: **Full Cross-Architecture Compilation — 16/16 Native Targets** — First primal to achieve full cross-arch type-checking. Node Atomic fleet: Mac M4 Mini, Pixel 8 (GrapheneOS), iPhone XS, Milk-V Jupiter 2 (RISC-V vector), Steam Deck, Raspberry Pi, plus cloud/exotic (IBM POWER, IBM Z, LoongArch). Key fixes: `akida-driver` `#[cfg(unix)]`→`#[cfg(target_os = "linux")]` (VFIO is Linux-only, not macOS), `fork`/`exit_group` arch-gated fallbacks (ppc64le/s390x/loongarch lack `rustix::runtime`), ioctl `Opcode` cast as `Opcode` not `u32` (powerpc uses u64), `MacOSSandboxManager` stub, `seccompiler` arch-restricted, consumer crate `not(unix)`→`not(target_os = "linux")` stubs. WASM (Tier 3) documented as compute-only subset (tokio/mio blocking). See `docs/CROSS_ARCH.md`.
- **S368 (Aug 8, 2026)**: **hw-safe Layer 2 Internal Gating — 4 G68 Violations → 0** — Final G68 convergence. Layer 2 modules (`vfio_setup`, `vfio_dma`, `drm_ioctl`, `platform_backends`) now unconditional at module level with kernel ABI structs available cross-platform and Linux-specific functions internally `#[cfg(target_os = "linux")]` gated.
- **S367 (Aug 8, 2026)**: **hw-safe Cross-Arch Abstraction — Layer 0/1 Unconditional, Layer 2 Gated** — Restructured `toadstool-hw-safe` into explicit layers. Layer 0 (pure Rust: `aligned_alloc`, `volatile_mmio`, `exclusive_ptr`, `contiguous`) fully unconditional. Layer 1 (memory: `locked_memory`, `huge_page`, `safe_mmap`, `device_mmap`) types unconditional, constructors return `Err(Unsupported)` on non-Linux. Layer 2 (`vfio_setup`, `vfio_dma`, `drm_ioctl`, `platform_backends`, `systemd_fds`) remains `#[cfg(target_os = "linux")]` gated.
- **S366 (Aug 8, 2026)**: **musl ioctl fix + libc migration + L2 false positive** — Fixed musl compile divergence in `akida-driver/mmio.rs:191` (`VFIO_DEVICE_GET_REGION_INFO` u64 vs i32). Migrated remaining `libc` calls in `hw-safe` to `rustix`. Renamed `HybridEsn::mode()`→`substrate_mode()` to resolve sourDough scanner false positive.
- **S365 (Aug 7, 2026)**: **G68 Complete — 0 rustix Outside hw-safe** — The final abstraction push. ALL `rustix::` code imports now live exclusively in `toadstool-hw-safe` (the designated containment zone). Every other crate in the workspace uses hw-safe's safe wrappers. New hw-safe APIs: `LinuxPrivilegeProbeBackend` (capabilities), `LinuxFilesystemIsolation` (mount/unmount), `LinuxDeviceIo` (read/write/pread/pwrite/poll), `vfio_bar_map`/`vfio_bar_unmap`, `lock_memory`/`unlock_memory`, `pipe_cloexec`, `fork`/`exit_group`/`kill_process`/`waitpid_nohang`, `recv_with_fds`/`sendmsg_with_fds`, `mknod_char`, `open_path`, `clock_monotonic_ns`, `fs_stats`, `seek_end`, `ioctl_infra` re-exports (Ioctl/Getter/Setter/Updater/opcode). Migrated: sandbox (3 files), akida-driver (4 files), cylinder (18+ files), nvpmu (1 file), display/v4l2 (1 file), sysmon (1 file), monitoring (1 file), server (2 files), cli tests (2 files). Platform isolation is now complete: new architectures (graftGate, riscGate) implement hw-safe backends without touching any other crate.
- **S364 (Aug 7, 2026)**: **G68 L3 Full Trait Surface — 6 New Cross-Platform Abstractions** — Every remaining Linux-specific device operation now has a platform-agnostic trait contract in `platform::device_io`. New traits: `DeviceIoctl` (typed control commands — ioctl/DeviceIoControl/IOConnectCallMethod), `PrivilegeProbe` (capabilities/entitlements/privileges), `FilesystemIsolation` (mount namespaces/sandbox/AppContainer), `FdPassing` (handle transfer — SCM_RIGHTS/Mach ports/DuplicateHandle), `SystemParameters` (clock ticks, page size, huge pages). New impl: `LinuxSystemParameters` (clock_ticks + page_size via rustix::param). Total L3 trait surface: 11 traits, 7 concrete Linux implementations. The porting contract for graftGate (G12) and riscGate (G42) is now fully specified — new architectures implement traits, not raw syscalls.
- **S363 (Aug 7, 2026)**: **Windows Cross-Compile + akida DeviceFile Migration** — `neurobench-runner` `select_backend` import gated behind `#[cfg(unix)]` (was failing Windows cross-compile). `akida-driver/device.rs` migrated from `rustix::fs::OFlags` + `OpenOptions::custom_flags` to `LinuxDeviceFile` trait. Full workspace now passes `cargo check --workspace --target x86_64-pc-windows-msvc`. **toadStool: 15/15 cross-arch** (was 14/15). All remaining 23 L3 rustix sites confirmed properly gated (`#[cfg(unix)]` / `#[cfg(target_os = "linux")]`) at module level — they compile away on non-Unix targets.
...
|----|-------------|--------|
| D-COV | Test coverage → 90% | Active — 8,521 lib tests; ~85%+ lib-only line (185K instrumented); remaining gap: hardware-dependent paths (VFIO, DRM, V4L2, akida) |
