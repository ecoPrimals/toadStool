...
- **P1 FIX cont. (Wave 157k ortho, Aug 12)**: southGate canary confirmed crash persists after feature fix. Deeper root cause: statically-linked musl binary cannot `dlopen("libvulkan.so.1")` — Vulkan ICD loader is glibc-linked, ABI mismatch causes crash/segfault. `catch_unwind` useless with `panic = "abort"`. Fixed: all 9 `Instance::new` call sites guarded with `#[cfg(target_env = "musl")]` early-return. On musl, GPU discovery reports "unavailable" and toadStool runs as CPU-only. Verified: glibc (8,446/0), x86_64-musl (0 warnings), aarch64-apple-darwin (clean).
