...
- **P1 FIX (Wave 157k, Aug 12)**: southGate canary found musl depot binary crash — `wgpu-28.0.0/src/api/instance.rs:64:13: No wgpu backend feature`. Root cause: wgpu 22 `vulkan-portability` implied `vulkan`; wgpu 28 separated them — `vulkan-portability` is macOS/MoltenVK-only. Fixed: workspace feature changed from `vulkan-portability` to `vulkan`.
- **P1 FIX cont. (Wave 157k ortho, Aug 12)**: southGate canary confirmed crash persists after feature fix. Deeper root cause: statically-linked musl binary cannot `dlopen("libvulkan.so.1")` — Vulkan ICD loader is glibc-linked, ABI mismatch causes crash/segfault. `catch_unwind` useless with `panic = "abort"`. Initial fix: `#[cfg(target_env = "musl")]` early-return at all 9 `Instance::new` call sites. Refined: blanket musl ban replaced with runtime `vulkan_loader_available()` probe — tries `dlopen("libvulkan.so.1")` via `libloading` (already in dep tree via ash). On Linux, returns false if loader can't be opened (static musl, no Vulkan); on macOS/Windows returns true (Metal/DX12 don't need dlopen). Dead `catch_unwind` wrappers removed. `OnceLock`-cached. Dynamic musl + GPU now works. 10 call sites across 8 files. `universal`/`adaptive` crates consume probe from `toadstool-runtime-gpu` dep (respecting `#![forbid(unsafe_code)]`).
...
- **Vulkan loader runtime probe** — all `Instance::new` call sites across GPU, universal, adaptive, server crates protected by `vulkan_loader_available()` runtime check. Dead `catch_unwind` wrappers removed (useless with `panic = "abort"`).
