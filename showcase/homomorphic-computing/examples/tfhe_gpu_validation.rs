...
async fn main() -> Result<()> {
    ...
    println!("  • BarraCUDA provides {:.1}x speedup over CPU", 
...
fn print_comparison(cpu: &BenchResult, gpu: &BenchResult) {
    ...
    println!("├──────────────┼─────────────┼─────────────┤");
    println!("│ Throughput   │ {:>9.0}/s │ {:>9.0}/s │", cpu.throughput, gpu.throughput);
    println!("│ Speedup      │     1.0x    │    {:>5.1}x   │", gpu.throughput / cpu.throughput);
    println!("│ Power        │    {:>5.0} W  │   {:>6.0} W  │", cpu.power_estimate_w, gpu.power_estimate_w);
    println!("│ Efficiency   │  {:>7.0}/J  │  {:>7.0}/J  │", cpu.ops_per_joule, gpu.ops_per_joule);
    println!("└──────────────┴─────────────┴─────────────┘");
}
