...
async fn main() -> Result<()> {
    ...
    let gpu_result = bench_gpu_polynomial_add(&gpu_device, polynomial_degree, iterations_gpu).await?;
...
async fn bench_gpu_polynomial_add(
    ...
    println!("  🔧 Setting up GPU buffers (degree={}, iterations={})...", degree, iterations);
...
fn bench_cpu_tfhe_baseline(
    ...
    println!("  🚀 Running {} iterations on CPU (TFHE-rs native)...", iterations);
...
fn print_result(result: &ActualBenchmarkResult) {
    ...
    println!("  ─────────────────────────────────────────────────────────");
    println!("  Total time:      {:.2} ms", result.total_time_us as f64 / 1000.0);
