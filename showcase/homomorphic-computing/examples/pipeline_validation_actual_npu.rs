...
async fn main() -> Result<()> {
    ...
    println!(
        "    Events:          {} / {} ops",
        npu_result.events_processed, npu_result.theoretical_ops
    );
    println!(
        "    Work Reduction:  {:.1}%",
    ...
    println!(
        "  • NPU achieves {:.1}x speedup over CPU for sparse HE operations",
        speedup
    );
    println!(
        "  • Energy efficiency improved {:.1}x (critical for edge deployment)",
        efficiency_gain
    );
    println!(
        "  • Sparse event processing reduces computation by {:.0}%",
        100.0 - (npu_result.events_processed as f64 / npu_result.theoretical_ops as f64 * 100.0)
    );
    println!(
        "  • Ultra-low power: {:.1}W (vs {:.1}W CPU)",
...
async fn bench_cpu_polynomial_add(
    degree: usize,
    iterations: usize,
...
async fn bench_npu_polynomial_add(
    ...
    let spikes_a: Vec<(u32, f32)> = poly_a
        .iter()
        .enumerate()
        .filter(|(_, &val)| val != 0)
    ...
    let spikes_b: Vec<(u32, f32)> = poly_b
        .iter()
        .enumerate()
        .filter(|(_, &val)| val != 0)
        .map(|(idx, &val)| (idx as u32, val as f32 / 1000.0))
...
fn print_result(result: &ActualBenchmarkResult) {
    ...
    println!(
        "  Operations:  {} iterations @ degree {}",
        result.iterations, result.degree
    );
    println!();
    println!("  Performance:");
