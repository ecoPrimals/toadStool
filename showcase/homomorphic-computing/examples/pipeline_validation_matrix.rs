...
impl PipelineConfig {
    ...
    fn chip_ordering(&self) -> Vec<String> {
        match self {
            ...
            PipelineConfig::GpuCpuGpu => vec!["GPU".to_string(), "CPU".to_string(), "GPU".to_string()],
            ...
            PipelineConfig::Npu1Npu2Gpu => vec!["NPU₁".to_string(), "NPU₂".to_string(), "GPU".to_string()],
            PipelineConfig::NpuGpu1Gpu2 => vec!["NPU".to_string(), "GPU₁".to_string(), "GPU₂".to_string()],
        }
    }
}
...
fn main() -> Result<()> {
    ...
    for (p_idx, pipeline) in pipelines.iter().enumerate() {
        println!("═══════════════════════════════════════════════════════════════════");
        println!("Pipeline {}/{}: {}", p_idx + 1, pipelines.len(), pipeline.name());
        println!("═══════════════════════════════════════════════════════════════════\n");
        
        for (w_idx, workload) in workloads.iter().enumerate() {
            println!("  Workload {}/{}: {} ", 
                     w_idx + 1, workloads.len(), workload.name());
...
fn generate_matrix_report(results: &[BenchmarkResult]) -> Result<()> {
    ...
    // Group by pipeline
    let mut pipelines: Vec<String> = results.iter()
    ...
    for pipeline in pipelines {
        ...
        let pipeline_results: Vec<_> = results.iter()
