...
pub mod substrates; // ✅ Real hardware measurement (Phase 4)
...
pub fn generate_encrypted_dataset(size: usize) -> Result<EncryptedDataset> {
    ...
    let expected_sum: Vec<u64> = a
        .iter()
        .zip(b.iter())
        .map(|(&x, &y)| x.wrapping_add(y))
        .collect();

    Ok(EncryptedDataset { a, b, expected_sum })
}
...
pub fn analyze_npu_advantage(cpu: &BenchmarkResult, gpu: &BenchmarkResult, npu: &BenchmarkResult) {
    ...
    println!("\n  Power Savings:");
    println!(
        "    vs CPU: {:.1}% less power ({:.1}W → {:.1}W) ⚡",
        power_savings_vs_cpu, cpu.power_watts, npu.power_watts
    );
    println!(
        "    vs GPU: {:.1}% less power ({:.1}W → {:.1}W) ⚡",
    ...
    println!(
        "    vs GPU: {:.2}x {}",
        speedup_vs_gpu,
        if speedup_vs_gpu > 1.0 {
            "FASTER ✅"
        } else {
            "slower (but way more efficient!)"
        }
    );
