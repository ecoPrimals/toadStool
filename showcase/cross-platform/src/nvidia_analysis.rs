...
async fn benchmark_raw_compute(device: &Arc<WgpuDevice>, size: usize, iterations: usize) -> Result<f64> {
    ...
    device.queue().write_buffer(&buffer_a, 0, bytemuck::cast_slice(&data));
    device.queue().write_buffer(&buffer_b, 0, bytemuck::cast_slice(&data));
...
async fn measure_overheads(device: &Arc<WgpuDevice>, size: usize) -> Result<()> {
    ...
    let pipeline = wgpu_device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
        label: None, layout: Some(&pipeline_layout), module: &shader, entry_point: "main",
    });
    ...
    let buffer_c = wgpu_device.create_buffer(&wgpu::BufferDescriptor {
        label: None, size: (size * 4) as u64, usage: wgpu::BufferUsages::STORAGE, mapped_at_creation: false,
    });
    ...
    let start = Instant::now();
    for _ in 0..iterations {
        let _encoder = wgpu_device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
    }
    ...
    let dispatch_record_time = start.elapsed().as_secs_f64() * 1000.0 / iterations as f64 - encoder_time - pass_time;
    ...
    println!("    Overhead Breakdown ({}M elements, {} iterations):", size / 1_000_000, iterations);
    println!("    ┌──────────────────────────┬────────────┬───────────┐");
    println!("    │ Component                │ Time (μs)  │ % Total   │");
    println!("    ├──────────────────────────┼────────────┼───────────┤");
    println!("    │ Encoder creation         │ {:>8.1}   │ {:>6.1}%   │", encoder_time * 1000.0, encoder_time / total_with_wait * 100.0);
    println!("    │ Compute pass begin/end   │ {:>8.1}   │ {:>6.1}%   │", pass_time * 1000.0, pass_time / total_with_wait * 100.0);
    println!("    │ Dispatch recording       │ {:>8.1}   │ {:>6.1}%   │", dispatch_record_time * 1000.0, dispatch_record_time / total_with_wait * 100.0);
    println!("    │ Queue submit             │ {:>8.1}   │ {:>6.1}%   │", submit_only_time * 1000.0, submit_only_time / total_with_wait * 100.0);
    println!("    │ GPU execution + wait     │ {:>8.1}   │ {:>6.1}%   │", gpu_wait_time * 1000.0, gpu_wait_time / total_with_wait * 100.0);
    println!("    ├──────────────────────────┼────────────┼───────────┤");
    println!("    │ TOTAL per operation      │ {:>8.1}   │ 100.0%    │", total_with_wait * 1000.0);
    println!("    └──────────────────────────┴────────────┴───────────┘");

    // Calculate theoretical vs actual
    let bytes_transferred = size * 3 * 4; // 3 buffers, 4 bytes each
    let theoretical_peak = if device.name().contains("3090") { 936.0 } else { 576.0 }; // GB/s
    ...
    println!("\n    Theoretical minimum: {:.3} ms at {:.0} GB/s", theoretical_time_ms, theoretical_peak);
    println!("    Achieved:            {:.3} ms at {:.1} GB/s ({:.1}% efficiency)", 
        total_with_wait, achieved_bandwidth, achieved_bandwidth / theoretical_peak * 100.0);
...
async fn main() -> Result<()> {
    ...
    for device in &devices {
        ...
        println!("══════════════════════════════════════════════════════════════════════════════");
        println!("  {} [{}]", name, if is_nvidia { "Vulkan/NVIDIA" } else if is_amd { "Vulkan/RADV" } else { "Unknown" });
        println!("══════════════════════════════════════════════════════════════════════════════\n");
        ...
        for (size, label) in [(1_000_000, "1M"), (5_000_000, "5M"), (10_000_000, "10M")] {
            ...
            println!("  │ {:>14} │ {:>7.2} ms │ {:>7.1} GB/s│ {:>7.1}%   │", 
