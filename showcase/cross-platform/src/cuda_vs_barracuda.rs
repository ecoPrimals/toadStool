...
mod barracuda_bench {
    ...
    pub async fn run_barracuda_benchmark(size: usize, iterations: usize) -> anyhow::Result<(String, f64, f64)> {
        ...
        let nvidia_idx = pool.devices().iter().position(|d| d.vendor == GpuVendor::Nvidia);
        let device_idx = nvidia_idx.unwrap_or(0);
        let device = pool.device(device_idx).ok_or_else(|| anyhow::anyhow!("No GPU"))?;
...
fn print_comparison_table(size: usize, cuda_times: Option<(f64, f64)>, barracuda_times: (String, f64, f64)) {
    println!("\n┌─────────────────┬─────────────────────────┬─────────────┬─────────────┐");
    println!("│ Backend         │ Device                  │ Add (ms)    │ Mul (ms)    │");
    println!("├─────────────────┼─────────────────────────┼─────────────┼─────────────┤");
    
    if let Some((add, mul)) = cuda_times {
        println!("│ Native CUDA     │ NVIDIA RTX 3090 (CUDA)  │ {:>11.4} │ {:>11.4} │", add, mul);
    }
    ...
    println!("│ BarraCUDA/wgpu  │ {:23} │ {:>11.4} │ {:>11.4} │", 
    ...
    if let Some((cuda_add, cuda_mul)) = cuda_times {
        ...
        let add_status = if add_ratio > 1.0 { "BarraCUDA faster" } else { "CUDA faster" };
        let mul_status = if mul_ratio > 1.0 { "BarraCUDA faster" } else { "CUDA faster" };
        ...
        if overall > 0.9 {
            println!("\n✓ BarraCUDA achieves {:.0}% of native CUDA performance!", overall * 100.0);
...
async fn main() -> Result<()> {
    ...
    for &size in &sizes {
        println!("\n═══ Vector Size: {} ({:.1}M elements) ═══", size, size as f64 / 1e6);
