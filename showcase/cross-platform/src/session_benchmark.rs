...
async fn benchmark_unbatched(device: &WgpuDevice, size: usize, num_ops: usize) -> Result<(f64, f64)> {
...
async fn main() -> Result<()> {
    ...
    for idx in 0..pool.devices().len() {
        ...
        for (size, num_ops) in &test_configs {
            // Run unbatched
            let (unbatched_total, unbatched_per_op) = benchmark_unbatched(&wgpu_device, *size, *num_ops).await?;
            ...
            println!("│ {:>10} │ {:>6} │ {:>9.1} ms │ {:>9.1} ms │ {:>7.1}/{:>3.1}μs │ {:>5.1}x │",
                size_str,
                num_ops,
    ...
    println!("\n═══ ANALYSIS ═══\n");
    println!("  Unbatched: Each operation submits its own command buffer (~200-300μs overhead each)");
