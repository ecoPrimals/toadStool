...
async fn main() -> Result<()> {
    ...
    for device in &devices {
        ...
        let theoretical_bw = theoretical.iter()
        ...
        for (size, label) in &sizes {
            let iterations = if *size < 1_000_000 { 100 } else if *size < 10_000_000 { 20 } else { 5 };
            ...
            match benchmark_at_scale(device, *size, iterations).await {
                Ok((total_ms, gops, bandwidth)) => {
                    ...
                    println!("  │ {:>14} │ {:>7.0} μs │ {:>7.2} GOP/s│ {:>7.1} GB/s│ {:>7.1}%   │",
        ...
        if scale_results.len() >= 2 {
            ...
            println!("  Analysis:");
            println!("    Size increased:     {:>8.0}x ({} → {})", 
                size_ratio, sizes[0].1, sizes[scale_results.len()-1].1);
            println!("    Time increased:     {:>8.1}x ({:.0}μs → {:.0}μs)", 
            ...
            println!("\n    At 100M elements: {:.1} GB/s = {:.1}% of theoretical peak", 
                large_bw, large_bw / theoretical_bw * 100.0);
        }
        ...
        match benchmark_at_scale(device, size, 10).await {
            Ok((total_ms, _, bandwidth)) => {
                ...
                println!("  │ {:>14} │ {:>7.2} ms │ {:>7.0} μs │ {:>7.1} GB/s│ {:>7}    │",
                    "1 (baseline)", base_time, base_time * 1000.0, bandwidth, "1.0x");
            }
            Err(e) => {
                println!("  │ Error: {} │", e);
                continue;
        ...
        for batch_size in [10, 50, 100, 500] {
            ...
            match benchmark_batched_at_scale(device, size, batch_size, iterations).await {
                Ok((total_ms, _, bandwidth)) => {
                    ...
                    println!("  │ {:>14} │ {:>7.2} ms │ {:>7.0} μs │ {:>7.1} GB/s│ {:>7.1}x   │",
        ...
        println!("    Bandwidth:      {:.1} GB/s ({:.1}% of theoretical)", 
            bandwidth, bandwidth / theoretical_bw * 100.0);
