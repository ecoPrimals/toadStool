...
fn run_gpu_f32(ctx: &GpuContext, a: &[f32], b: &[f32]) -> Vec<f32> {
    ...
    let mut encoder = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor::default());
    {
        ...
        pass.dispatch_workgroups((a.len() as u32 + 255) / 256, 1, 1);
    }
...
fn validate_cpu_gpu_equivalence(ctx: &GpuContext) {
    ...
    for (name, a_val, b_val) in &test_cases {
        ...
        println!("    {}: CPU={:.6e}, GPU={:.6e} {}", 
            name, cpu_out[0], gpu_out[0], 
            if matches { "✅" } else { "❌" });
    }
...
fn benchmark_precisions(ctx: &GpuContext) {
    ...
    println!("\n    F32:");
    println!("      CPU: {:>8.2} ms  ({:>6.1} GB/s)", cpu_f32_time.as_secs_f64() * 1000.0, cpu_f32_bandwidth);
    println!("      GPU: {:>8.2} ms  ({:>6.1} GB/s)", gpu_f32_time.as_secs_f64() * 1000.0, gpu_f32_bandwidth);
    println!("      Speedup: {:.1}x", cpu_f32_time.as_secs_f64() / gpu_f32_time.as_secs_f64());
    ...
    if ctx.has_f64 {
        ...
        println!("\n    F64:");
        println!("      CPU: {:>8.2} ms  ({:>6.1} GB/s)", cpu_f64_time.as_secs_f64() * 1000.0, cpu_f64_bandwidth);
        println!("      GPU: {:>8.2} ms  ({:>6.1} GB/s)", gpu_f64_time.as_secs_f64() * 1000.0, gpu_f64_bandwidth);
        println!("      Speedup: {:.1}x", cpu_f64_time.as_secs_f64() / gpu_f64_time.as_secs_f64());
