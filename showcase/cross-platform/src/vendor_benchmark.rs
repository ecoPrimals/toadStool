...
async fn bench_barracuda(device: std::sync::Arc<WgpuDevice>, name: &str, size: usize, iterations: usize) -> Result<Vec<BenchResult>> {
    let mut results = Vec::new();
    
    // Create test data
    let data_a: Vec<f32> = (0..size * size).map(|i| (i % 1000) as f32 * 0.001).collect();
    let data_b: Vec<f32> = (0..size * size).map(|i| ((i + 500) % 1000) as f32 * 0.001).collect();
    ...
    results.push(BenchResult::new("BarraCUDA", name, "vector_add", size, elapsed));
    ...
    let elapsed = start.elapsed().as_secs_f64() * 1000.0 / iterations as f64;
    results.push(BenchResult::new("BarraCUDA", name, "vector_mul", size, elapsed));
    ...
    let elapsed = start.elapsed().as_secs_f64() * 1000.0 / iterations as f64;
    results.push(BenchResult::new("BarraCUDA", name, "reduction_sum", size, elapsed));
...
fn bench_cpu_baseline(size: usize, iterations: usize) -> Vec<BenchResult> {
    ...
    let elapsed = start.elapsed().as_secs_f64() * 1000.0 / iterations as f64;
    results.push(BenchResult::new("CPU", "EPYC (rayon)", "vector_add", size, elapsed));
    ...
    let elapsed = start.elapsed().as_secs_f64() * 1000.0 / iterations as f64;
    results.push(BenchResult::new("CPU", "EPYC (rayon)", "vector_mul", size, elapsed));
    ...
    let elapsed = start.elapsed().as_secs_f64() * 1000.0 / iterations as f64;
    results.push(BenchResult::new("CPU", "EPYC (rayon)", "reduction_sum", size, elapsed));
...
fn print_speedup_analysis(results: &[BenchResult]) {
    ...
    for op in ops {
        let cpu_result = results.iter()
            .find(|r| r.backend == "CPU" && r.operation == op);
        
        if let Some(cpu) = cpu_result {
            println!("{}:", op);
            for r in results.iter().filter(|r| r.operation == op && r.backend != "CPU") {
                ...
                println!("  {} {:30} {:>6.2}x {}", 
                    ...
                    speedup,
                    if speedup > 10.0 { "EXCELLENT" } 
                    else if speedup > 5.0 { "GREAT" }
                    else if speedup > 1.0 { "faster" }
                    else { "slower" }
                );
            }
            println!();
...
fn print_vendor_parity(results: &[BenchResult]) {
    ...
    for op in ops {
        ...
        if let (Some(n), Some(a)) = (nvidia, amd) {
            ...
            let parity = if (ratio - 1.0).abs() < 0.3 { "✓ PARITY" } else { "≠ differs" };
            println!("{}: NVIDIA {:.2}ms vs AMD {:.2}ms (ratio {:.2}x) {}",
...
async fn main() -> Result<()> {
    ...
    for &size in &sizes {
        println!("\n═══ Benchmarking {}×{} matrices ({} iterations) ═══", size, size, iterations);
        
        // BarraCUDA on each GPU
        for (i, gpu) in pool.devices().iter().enumerate() {
            if let Some(device) = pool.device(i) {
                let device_name = format!("{} ({:?})", 
                    if gpu.name.len() > 20 { &gpu.name[..20] } else { &gpu.name },
                    gpu.vendor);
    ...
    for &size in &sizes {
        ...
        let size_results: Vec<_> = all_results.iter()
    ...
    let analysis_results: Vec<_> = all_results.iter()
