...
async fn benchmark_cold_vs_warm(device: Arc<WgpuDevice>) -> Result<()> {
    ...
    println!("  Speedup:               {:>8.1}x", cold_time.as_micros() as f64 / warm_avg);
...
async fn main() -> Result<()> {
    ...
    let results = warmup_pool(&devices, &warmup_config)?;
    ...
    for device in &devices {
        ...
        println!("    {} - All warm: min={:.0}μs, avg={:.0}μs", 
            device.name(), min, avg);
    }
    ...
    for (name, mut config) in presets {
        ...
        println!("  {:15} {:>3} ops × {:>2} wg_sizes = {:>3} pipelines in {:>6.1}ms",
            name,
            config.ops.len(),
