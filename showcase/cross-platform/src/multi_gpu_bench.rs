...
async fn main() -> Result<()> {
    ...
    for (i, device) in pool.devices().iter().enumerate() {
        ...
        println!("  [{i}] {} ({vendor_str}, ~{:.0} GFLOPS)", device.name, device.gflops);
    }
    println!();
    ...
    if let Some(device) = pool.device(0) {
        ...
        println!("  Result[0]: {:.6} (expected: {:.6})", output[0], data[0] * 2.0);
    ...
    if pool.device_count() >= 2 {
        ...
        let results = pool.parallel_map(workloads, |device, work_id| {
            // Each GPU processes a workload
            let data: Vec<f32> = (0..512 * 512).map(|i| (i + work_id * 1000) as f32 * 0.001).collect();
        ...
        println!("  Average per workload: {:.2}ms", elapsed.as_secs_f64() * 1000.0 / results.len() as f64);
    ...
    if has_nvidia && has_amd {
        ...
        // Find indices
        let nvidia_idx = pool.devices().iter().position(|d| d.vendor == GpuVendor::Nvidia).unwrap();
        let amd_idx = pool.devices().iter().position(|d| d.vendor == GpuVendor::Amd).unwrap();
