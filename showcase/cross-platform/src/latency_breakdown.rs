...
async fn main() -> Result<()> {
    ...
    for idx in 0..pool.devices().len() {
        ...
        println!("  ══════════════════════════════════════════════════════════════");
        println!(
            "  Shader compilation:     {:>8.1} μs ({:>5.1}% of full)",
            shader_avg,
            100.0 * shader_avg / full_avg
        );
        println!(
            "  Pipeline creation:      {:>8.1} μs ({:>5.1}% of full)",
            pipeline_avg,
            100.0 * pipeline_avg / full_avg
        );
        println!(
            "  Bind group layout:      {:>8.1} μs ({:>5.1}% of full)",
            bgl_avg,
            100.0 * bgl_avg / full_avg
        );
        println!(
            "  Bind group:             {:>8.1} μs ({:>5.1}% of full)",
            bg_avg,
            100.0 * bg_avg / full_avg
        );
        println!(
            "  Encoding:               {:>8.1} μs ({:>5.1}% of full)",
            encode_avg,
            100.0 * encode_avg / full_avg
        );
        println!(
            "  Submit + GPU + Wait:    {:>8.1} μs ({:>5.1}% of full)",
            submit_avg,
