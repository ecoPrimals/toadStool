...
async fn run_benchmark(
    ...
    // Warmup
    {
        ...
        let workgroups = if is_vectorized { (size / 4 / 64) as u32 } else { (size / 256) as u32 };
...
async fn run_all_benchmarks() -> Result<()> {
        ...
        for wg_size in [64, 128, 256] {
            ...
            match run_benchmark(device, queue, &shader, size, iterations, false).await {
                Ok(time_us) => {
                    ...
                    println!("    WG={:>3}: {:>8.1}μs  ({:>6.1} GB/s)", wg_size, time_us, bw);
    ...
    for gpu in ["NVIDIA RTX 3090", "AMD RX 6950 XT"] {
        ...
        if let Some(best) = gpu_results.iter().max_by(|a, b| a.bandwidth_gbps.partial_cmp(&b.bandwidth_gbps).unwrap()) {
            println!("{} - Best configuration:", gpu);
            println!("  Variant: {} (WG={}, {}/thread)", 
                best.variant, best.workgroup_size, best.elements_per_thread);
            println!("  Time: {:.1}μs, Bandwidth: {:.1} GB/s", best.time_us, best.bandwidth_gbps);
