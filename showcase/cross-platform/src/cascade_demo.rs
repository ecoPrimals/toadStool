...
fn infer_npu(input: &[f32], num_classes: usize) -> StageResult {
    ...
    output.iter_mut().for_each(|x| *x = (*x - max_o).exp() / exp_sum);
...
fn postprocess_cpu(probabilities: &[f32], k: usize) -> StageResult {
    ...
    let output: Vec<f32> = indexed.iter()
        .take(k)
...
async fn main() -> Result<()> {
    ...
    println!("           Output: {} normalized values", stage1.output.len());
    println!();
    ...
    println!("           Output: {} class probabilities", stage2.output.len());
    println!();
