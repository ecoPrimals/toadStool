//! Cross-substrate validation for AdamW optimizer
//!
//! **Deep Debt**: Validates identical AdamW behavior on all hardware
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("═══════════════════════════════════════════════════════════");
    println!("  ADAMW OPTIMIZER - Cross-Substrate Validation");
    println!("═══════════════════════════════════════════════════════════\n");
    ...
    let beta2 = 0.999;
    let epsilon = 1e-8;
    let weight_decay = 0.01; // Decoupled weight decay!
    let step = 1;
    ...
    let ref_params = Tensor::from_vec_on(
        vec![1.0; size],
        vec![size],
        ref_device.clone(),
    )
    ...
    let (ref_p_out, ref_m_out, ref_v_out) = ref_params
        .adamw(&ref_gradients, &ref_m, &ref_v, learning_rate, beta1, beta2, epsilon, weight_decay, step)?;

    let ref_p_data = ref_p_out.to_vec()?;
    ...
    for (i, substrate) in substrates.iter().enumerate() {
        ...
        // Execute AdamW
        let (p_out, m_out, v_out) = params
            .adamw(&gradients, &m, &v, learning_rate, beta1, beta2, epsilon, weight_decay, step)?;

        let p_data = p_out.to_vec()?;
        ...
        for j in 0..size {
            max_diff_p = max_diff_p.max((ref_p_data[j] - p_data[j]).abs());
            max_diff_m = max_diff_m.max((ref_m_data[j] - m_data[j]).abs());
            max_diff_v = max_diff_v.max((ref_v_data[j] - v_data[j]).abs());
        }

        let tolerance = 1e-5;
        let passed = max_diff_p < tolerance && max_diff_m < tolerance && max_diff_v < tolerance;

        if passed {
            println!(" ✓ PASS (max diff: p={:.2e}, m={:.2e}, v={:.2e})", 
                     max_diff_p, max_diff_m, max_diff_v);
        } else {
            println!(" ✗ FAIL (max diff: p={:.2e}, m={:.2e}, v={:.2e})", 
                     max_diff_p, max_diff_m, max_diff_v);
            all_passed = false;
        }
    }
    ...
    println!();
    if all_passed {
        println!("✓ ALL SUBSTRATES PASSED - Identical AdamW behavior!");
        println!("✓ Decoupled weight decay working correctly across all hardware!");
    } else {
