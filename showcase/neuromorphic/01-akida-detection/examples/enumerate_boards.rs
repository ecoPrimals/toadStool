...
async fn main() -> anyhow::Result<()> {
    ...
    // Strandgate boards
    let strandgate_boards: Vec<_> = mesh.boards
        .iter()
    ...
    if !strandgate_boards.is_empty() {
        ...
        for (i, board) in strandgate_boards.iter().enumerate() {
            let tree_char = if i == strandgate_boards.len() - 1 { "└" } else { "├" };
            println!("    {}── Slot {}: akida{} (PCIe Gen{} x{})",
                tree_char,
                i + 1,
                board.index,
    ...
    for board in &remote {
        ...
        println!("    └── Slot 1: akida{} (PCIe Gen{} x{})",
            board.index,
            board.pcie_generation,
            board.pcie_lanes
        );
    ...
    println!("  - Dense neuromorphic compute → Strandgate ({} boards, low latency)", local.len());
    println!("  - Real-time classification → Southgate ({} boards, near GPU)", remote.len());
    println!("  - Fault tolerance → All {} boards (automatic failover)", mesh.boards.len());
