...
fn bench_matmul_memory_optimization(c: &mut Criterion) {
    ...
    eprintln!(
        "🎯 Benchmarking MatMul Memory Optimization on: {}",
        executor.gpu_info()
    );
    ...
    for (name, size) in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("Naive", name), &size, |bench, &size| {
            bench.iter(|| {
                rt.block_on(async {
                    executor
        ...
        group.bench_with_input(BenchmarkId::new("Tiled", name), &size, |bench, &size| {
            bench.iter(|| {
                rt.block_on(async {
                    executor
