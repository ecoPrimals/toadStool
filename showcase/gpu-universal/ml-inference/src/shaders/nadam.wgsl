...
struct Params {
    learning_rate: f32,
    beta1: f32,           // First moment decay, typically 0.9
    beta2: f32,           // Second moment decay, typically 0.999
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let beta1_t = pow(params.beta1, step_f);
    let beta2_t = pow(params.beta2, step_f);
    ...
    weights_out[idx] = w - params.learning_rate * gradient_nesterov / (sqrt(v_hat) + params.epsilon);
}
