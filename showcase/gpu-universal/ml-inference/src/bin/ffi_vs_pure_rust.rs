...
async fn main() -> Result<()> {
    ...
    for &size in &sizes {
        ...
        println!("  {:>10} elements: {:>8.3} ms ({:>8.2} M elem/s)",
    ...
    #[cfg(feature = "opencl")]
    {
        ...
        for &size in &sizes {
            ...
            let start = Instant::now();
            let _output: Vec<f32> = input.iter().map(|&x| if x > 0.0 { x } else { 0.0 }).collect();
            let elapsed = start.elapsed();
        ...
        let avg_overhead = wgpu_times.iter().zip(opencl_times.iter())
