...
use anyhow::Result;
#[cfg(feature = "opencl")]
use ml_inference_showcase::conv2d_kernels::{conv2d_cpu, Conv2DExecutor, Conv2DParams};
#[cfg(feature = "opencl")]
...
fn main() -> Result<()> {
    ...
    println!(
        "  Input:   {}x{}x{} (CHW)",
        params.in_channels, params.in_height, params.in_width
    );
    ...
    println!(
        "  Input size:   {} elements ({:.2} KB)",
        input.len(),
        input.len() as f32 * 4.0 / 1024.0
    );
    println!(
        "  Weight size:  {} elements ({:.2} KB)",
        weights.len(),
        weights.len() as f32 * 4.0 / 1024.0
    );
    println!(
        "  Output size:  {} elements ({:.2} KB)",
        params.output_size(),
        params.output_size() as f32 * 4.0 / 1024.0
    );
    println!();
    ...
    println!(
        "  Time:       {:.2} ms",
        cpu_time.as_micros() as f64 / 1000.0
    );
    ...
    #[cfg(feature = "opencl")]
    {
        ...
        match Conv2DExecutor::new() {
            Ok(executor) => {
                ...
                println!(
                    "  Time:       {:.2} ms",
                    gpu_time.as_micros() as f64 / 1000.0
                );
                ...
                println!(
                    "  Output:     {}x{}x{}",
                    params.out_channels, pool_h, pool_w
                );
                println!(
                    "  Time:       {:.2} ms",
