...
fn main() -> Result<()> {
    ...
    for &batch_size in &config.batch_sizes {
        ...
        for _ in 0..config.warmup_runs {
            let (images, _labels) = test_data.batch(0, batch_size)
        ...
        for i in 0..config.bench_runs {
            ...
            let (images, _labels) = test_data.batch(start_idx, batch_size)
                .ok_or_else(|| anyhow::anyhow!("Failed to get batch"))?;
            ...
            if let Some(device) = gpu_device {
                match (Conv2DExecutor::new(), OpenCLExecutor::new(&device)) {
                    (Ok(_conv_executor), Ok(_opencl_executor)) => {
                        ...
                        println!("  Note: Using CPU for full pipeline (GPU ops verified individually)");
                        println!();
    ...
    println!("{:<30} {:<20} {:<10} {:>15} {:>12}", 
             "Workload", "GPU", "Backend", "Throughput", "Latency");
    println!("{}", "─".repeat(95));

    for result in &all_results {
        println!("{:<30} {:<20} {:<10} {:>12.0} img/s {:>9.2} ms",
                 result.name,
                 result.gpu,
                 result.backend,
                 result.throughput,
