...
async fn main() -> Result<()> {
    ...
    let network = SimpleNetwork::load_pretrained()
        .unwrap_or_else(|_| SimpleNetwork::new());
    ...
    if gpus.len() >= 2 {
        ...
        let dual_result = run_dual_gpu_parallel(&gpus[0], &gpus[1], &network, &test_data, 5000).await?;
        println!();
...
async fn run_dual_gpu_parallel(
    ...
    println!("  ═══ Results ═══");
    println!("  Samples:    {} ({} + {})", stats.samples, split_point, num_samples - split_point);
    println!("  Correct:    {} ({} + {})", stats.correct, correct1, correct2);
    println!("  Accuracy:   {:.2}%", stats.accuracy * 100.0);
    println!();
    println!("  ═══ Performance ═══");
    println!("  Total time:    {:.2}s", stats.total_time_ms / 1000.0);
    println!("  Avg latency:   {:.3}ms", stats.avg_latency_ms);
    println!("  Throughput:    {:.0} images/sec", stats.throughput_per_sec);
    println!("  Speedup:       {:.2}x (parallel efficiency)", 
