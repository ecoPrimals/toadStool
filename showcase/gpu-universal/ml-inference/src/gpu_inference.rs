...
use crate::{InferenceResult, network::SimpleNetwork};
use anyhow::Result;
use ndarray::Array1;
use std::time::Instant;
use toadstool_runtime_gpu::{
    scheduler::{UniversalComputeScheduler, SchedulingPolicy},
    types::GpuFramework,
...
impl GpuInference {
    ...
    pub async fn infer(&self, input: &Array1<f32>) -> Result<InferenceResult> {
        ...
        let backend = if let Some(ref backend) = self.preferred_backend {
            ...
            strategy.select_framework(Some(&workload_type), &available)
                .unwrap_or_else(|| {
...
mod tests {
    ...
    async fn test_gpu_inference_creation() {
        ...
        assert!(inference.current_backend().contains("Automatic") || 
    ...
    async fn test_gpu_inference_with_backend() {
        let network = SimpleNetwork::new();
        let inference = GpuInference::with_backend(network, GpuFramework::Cuda).await.unwrap();
