...
async fn test_fused_v2_layernorm_correctness() {
    gpu_test_resilient_async(async {
        ...
        let result_original = executor
            ...
            .await
            .unwrap();
        let result_fused_v2 = executor
            .execute_layernorm_fused_v2(&input, config.clone())
            .await
            .unwrap();
        ...
        for (i, (orig, fused)) in result_original
            .iter()
            .zip(result_fused_v2.iter())
            .enumerate()
        {
...
async fn test_fused_v2_layernorm_llama_scale() {
    gpu_test_resilient_async(async {
        ...
        println!(
            "✅ Fused LayerNorm V2 handles LLaMA-scale correctly (4096 * 256 = 1,048,576 elements)"
        );
...
async fn test_fused_v2_normalization_properties() {
    gpu_test_resilient_async(async {
        ...
        let result = executor
            .execute_layernorm_fused_v2(&input, config)
            .await
            .unwrap();
