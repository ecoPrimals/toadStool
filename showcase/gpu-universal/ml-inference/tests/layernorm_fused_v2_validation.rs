...
//! accurate results matching the original (3-pass) implementation.

use ml_inference_showcase::wgpu::{WgpuExecutor, NormConfig};

#[tokio::test]
async fn test_fused_v2_layernorm_correctness() {
    ...
    let result_original = executor.execute_layernorm(&input, config.clone()).await.unwrap();
    let result_fused_v2 = executor.execute_layernorm_fused_v2(&input, config.clone()).await.unwrap();
    
    // They should produce accurate results (within 0.1% tolerance)
    assert_eq!(result_original.len(), result_fused_v2.len());
    
    let mut max_diff = 0.0f32;
    let mut max_rel_error = 0.0f32;
    
    for (i, (orig, fused)) in result_original.iter().zip(result_fused_v2.iter()).enumerate() {
        ...
        assert!(
            rel_error < 0.001,  // 0.1% tolerance (much better than v1!)
            "Mismatch at index {}: original={}, fused_v2={}, diff={}, rel_error={}",
            i, orig, fused, diff, rel_error
        );
    }
    
    println!("✅ Fused LayerNorm V2 accuracy validated (GPT-2 scale)");
    println!("   Max absolute diff: {}", max_diff);
    println!("   Max relative error: {:.4}%", max_rel_error * 100.0);
}

#[tokio::test]
async fn test_fused_v2_layernorm_llama_scale() {
    let executor = WgpuExecutor::new().await.unwrap();
    ...
    println!("✅ Fused LayerNorm V2 handles LLaMA-scale correctly (4096 * 256 = 1,048,576 elements)");
    println!("   Max absolute diff (sampled): {}", max_diff);
    println!("   Max relative error (sampled): {:.4}%", max_rel_error * 100.0);
}

#[tokio::test]
async fn test_fused_v2_layernorm_with_gamma_beta() {
    let executor = WgpuExecutor::new().await.unwrap();
    ...
    println!("✅ Fused LayerNorm V2 correctly applies gamma and beta parameters");
    println!("   Max relative error: {:.4}%", max_rel_error * 100.0);
}

#[tokio::test]
async fn test_fused_v2_normalization_properties() {
    let executor = WgpuExecutor::new().await.unwrap();
    ...
    let result = executor.execute_layernorm_fused_v2(&input, config).await.unwrap();
    ...
    println!("✅ Fused LayerNorm V2 normalization properties verified");
