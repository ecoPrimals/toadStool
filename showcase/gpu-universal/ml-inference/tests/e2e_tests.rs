...
use ml_inference_showcase::wgpu_executor::{
    WgpuExecutor, NormConfig, BinaryOp, ReduceOp, CrossEntropyConfig, LossReduction, AdamConfig
};
...
async fn test_simple_training_step() {
    ...
    assert!((prob_sum - 1.0).abs() < FP32_TOLERANCE,
    ...
    let loss = executor.execute_cross_entropy(&probs, &targets_onehot, 1, 2, config).await.unwrap();
...
async fn test_batch_normalization_training_pipeline() {
    ...
    let normalized = executor.execute_layernorm(&sample, norm_config).await.unwrap();
    ...
    assert!(mean.abs() < FP32_TOLERANCE, 
        "Mean should be ~0, got {}", mean);
    assert!((variance - 1.0).abs() < FP32_TOLERANCE,
...
async fn test_elementwise_reduction_pipeline() {
    ...
    // Add
    let added = executor.execute_elementwise_binary(&a, &b, BinaryOp::Add).await.unwrap();
    assert_eq!(added, vec![3.0, 4.0, 5.0, 6.0, 7.0]);
    
    // Multiply by scale factor
    let scale = vec![0.5, 0.5, 0.5, 0.5, 0.5];
    let scaled = executor.execute_elementwise_binary(&added, &scale, BinaryOp::Mul).await.unwrap();
    assert_eq!(scaled, vec![1.5, 2.0, 2.5, 3.0, 3.5]);
    
    // Reduce to sum
    let sum = executor.execute_reduce(&scaled, ReduceOp::Sum).await.unwrap();
    assert!((sum - 12.5).abs() < FP32_TOLERANCE, "Sum should be 12.5, got {}", sum);
    
    // Reduce to mean
    let mean = executor.execute_reduce(&scaled, ReduceOp::Mean).await.unwrap();
    assert!((mean - 2.5).abs() < FP32_TOLERANCE, "Mean should be 2.5, got {}", mean);
    
    println!("Elementwise-Reduce pipeline complete: sum={}, mean={}", sum, mean);
}
...
async fn test_activation_comparison_pipeline() {
    ...
    assert!((softmax_sum - 1.0).abs() < FP32_TOLERANCE,
        "Softmax should sum to 1, got {}", softmax_sum);
...
async fn test_adam_optimizer_integration() {
    ...
    // Step 1
    executor.execute_adam_step(&grads, &mut params, &mut m, &mut v, 1, config).await.unwrap();
    
    // Verify parameters decreased (gradient descent)
    for i in 0..4 {
        assert!(params[i] < initial_params[i], 
            "Param {} should decrease with positive gradient", i);
    }
    ...
    executor.execute_adam_step(&grads, &mut params, &mut m, &mut v, 2, config).await.unwrap();
    
    // Verify continued descent
    for i in 0..4 {
        assert!(params[i] < params_after_1[i],
            "Param {} should continue decreasing", i);
    }
...
async fn test_large_batch_processing() {
    ...
    let mean = executor.execute_reduce(&activated, ReduceOp::Mean).await.unwrap();
    assert!(mean > 0.0 && mean.is_finite(),
        "Mean activation should be positive and finite");
    
    println!("Large batch processing complete: {} samples × {} features", 
        batch_size, features);
