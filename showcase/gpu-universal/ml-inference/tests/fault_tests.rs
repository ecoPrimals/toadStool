...
use ml_inference_showcase::wgpu_executor::{WgpuExecutor, BinaryOp, ReduceOp, NormConfig, CrossEntropyConfig, LossReduction};
...
async fn test_mismatched_sizes_elementwise() {
    ...
    let result = executor.execute_elementwise_binary(&a, &b, BinaryOp::Add).await;
    
    // Should return error, not panic
    assert!(result.is_err(), "Mismatched sizes should return error");
    
    let err_msg = format!("{:?}", result.unwrap_err());
    assert!(err_msg.contains("size") || err_msg.contains("length") || err_msg.contains("must equal"),
...
async fn test_invalid_matmul_dimensions() {
    ...
    // Test matmul with correct buffer sizes but semantically wrong dimensions
    // A: 2x3, B: 3x2 (valid dimensions)
    let a = vec![1.0; 6]; // 2x3
    let b = vec![1.0; 6]; // 3x2
    
    let result = executor.execute_matmul(&a, &b, 2, 3, 2).await;
    
    // This should succeed (dimensions are compatible)
    // Testing that valid dimensions work correctly
    assert!(result.is_ok(), "Valid matmul dimensions should succeed");
    assert_eq!(result.unwrap().len(), 4); // 2x2 output
...
async fn test_negative_reduction() {
    ...
    let sum = executor.execute_reduce(&input, ReduceOp::Sum).await.unwrap();
    assert!(sum < 0.0, "Sum of negatives should be negative");
    
    let mean = executor.execute_reduce(&input, ReduceOp::Mean).await.unwrap();
    assert!(mean < 0.0, "Mean of negatives should be negative");
    
    let max = executor.execute_reduce(&input, ReduceOp::Max).await.unwrap();
...
async fn test_cross_entropy_invalid_sizes() {
    ...
    let result = executor.execute_cross_entropy(&predictions, &targets, 2, 3, config).await;
...
async fn test_layernorm_all_zeros() {
    ...
    assert!(result.iter().all(|&x| x.abs() < 0.1),
...
async fn test_softmax_all_same_values() {
    ...
    for &prob in &result {
        assert!((prob - 0.1).abs() < 1e-4,
            "Uniform logits should give uniform probabilities, got {}", prob);
    }
...
async fn test_softmax_single_element() {
    ...
    assert!((result[0] - 1.0).abs() < 1e-5,
        "Single element softmax should be 1.0, got {}", result[0]);
}
...
async fn test_sigmoid_extreme_values() {
    ...
    // sigmoid(-1000) ≈ 0
    assert!(result[0] < 0.01, "Sigmoid of large negative should be near 0");
    
    // sigmoid(0) = 0.5
    assert!((result[1] - 0.5).abs() < 0.01, "Sigmoid of 0 should be 0.5");
    
    // sigmoid(1000) ≈ 1
    assert!(result[2] > 0.99, "Sigmoid of large positive should be near 1");
}
...
async fn test_tanh_extreme_values() {
    ...
    // tanh(-1000) ≈ -1
    assert!(result[0] < -0.99, "Tanh of large negative should be near -1");
...
async fn test_dropout_rate_zero() {
    ...
    let result = executor.execute_dropout(&input, 0.0, true, Some(12345)).await.unwrap();
...
async fn test_dropout_rate_one() {
    ...
    let result = executor.execute_dropout(&input, 1.0, true, Some(12345)).await.unwrap();
...
async fn test_scatter_overlapping_indices() {
    ...
    assert!(
        ...
        "Index 1 should have one of the written values, got {}", result[1]
    );
...
async fn test_multiple_operations_after_error() {
    ...
    let _err = executor.execute_elementwise_binary(&bad_a, &bad_b, BinaryOp::Add).await;
    
    // Verify executor still works after error
    let good_a = vec![1.0, 2.0, 3.0];
    let good_b = vec![4.0, 5.0, 6.0];
    let result = executor.execute_elementwise_binary(&good_a, &good_b, BinaryOp::Add).await.unwrap();
