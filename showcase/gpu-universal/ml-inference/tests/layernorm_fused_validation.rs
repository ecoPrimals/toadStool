...
async fn test_fused_layernorm_correctness() {
    ...
    let result_fused = executor.execute_layernorm_fused(&input, config.clone()).await.unwrap();
...
async fn test_fused_layernorm_llama_scale() {
    ...
    let result = executor.execute_layernorm_fused(&input, config).await.unwrap();
    ...
    println!("✅ Fused LayerNorm handles LLaMA-scale correctly (4096 * 256 = 1,048,576 elements)");
}
