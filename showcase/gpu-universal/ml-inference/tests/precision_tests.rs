...
use ml_inference_showcase::wgpu_executor::{WgpuExecutor, NormConfig, BinaryOp, ReduceOp};
...
async fn test_relu_fp32_precision() {
    ...
    for (i, (&out, &exp)) in result.iter().zip(expected.iter()).enumerate() {
        ...
        assert!(error < FP32_TOLERANCE, 
            ...
            i, out, exp, error, FP32_TOLERANCE);
    }
}
...
async fn test_matmul_fp32_precision() {
    ...
    for (i, (&out, &exp)) in result.iter().zip(expected.iter()).enumerate() {
        let error = (out - exp).abs();
        assert!(error < FP32_TOLERANCE_RELAXED, 
            "MatMul precision error at index {}: got {}, expected {}, error = {} (tolerance = {})",
            i, out, exp, error, FP32_TOLERANCE_RELAXED);
    }
}
...
async fn test_reduce_fp32_precision() {
    ...
    assert!((sum - expected_sum).abs() < FP32_TOLERANCE_RELAXED,
        "Sum precision error: got {}, expected {}", sum, expected_sum);
    assert!((mean - expected_mean).abs() < FP32_TOLERANCE_RELAXED,
        "Mean precision error: got {}, expected {}", mean, expected_mean);
    assert!((max - expected_max).abs() < FP32_TOLERANCE,
        "Max precision error: got {}, expected {}", max, expected_max);
}
...
async fn test_relu_edge_cases() {
    ...
    assert_eq!(result[5], f32::MIN_POSITIVE, "MIN_POSITIVE should pass through");
...
async fn test_reduce_accumulation_error() {
    ...
    assert!(relative_error < 0.01, // 1% relative error
        ...
        sum, expected_sum, relative_error);
}
...
async fn test_matmul_accumulation_error() {
    ...
    for (i, &val) in result.iter().enumerate() {
        ...
        assert!(relative_error < 0.05, // 5% relative error
            "MatMul accumulation error at index {}: got {}, expected {}, relative error = {}",
            i, val, expected_value, relative_error);
    }
}
...
async fn test_layernorm_numerical_stability() {
    ...
    let mean: f32 = result.iter().sum::<f32>() / result.len() as f32;
    assert!(mean.abs() < FP32_TOLERANCE_RELAXED,
        "LayerNorm mean should be ~0 even with large inputs, got {}", mean);
}
