...
async fn test_tiled_matmul_small() {
    ...
    let result_tiled = executor.execute_matmul_tiled(&a, &b, m, k, n).await.unwrap();
...
async fn test_tiled_matmul_medium() {
    ...
    let b: Vec<f32> = (0..k * n).map(|i| (((i + 1) % 1000) as f32) * 0.001).collect();
...
async fn test_tiled_matmul_large() {
    ...
    let b: Vec<f32> = (0..k * n).map(|i| (((i + 1) % 10000) as f32) * 0.0001).collect();
