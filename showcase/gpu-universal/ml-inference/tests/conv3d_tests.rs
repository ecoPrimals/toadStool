...
async fn test_conv3d_basic_3x3x3() {
    ...
    let result = executor
        .execute_conv3d(
            &input, &weights, &bias, batch, in_channels, out_channels,
            input_d, input_h, input_w, config,
        )
        .await
        .unwrap();
    ...
    println!("   Input: {}x{}x{} → Output: 2x2x2", input_d, input_h, input_w);
}
...
async fn test_conv3d_video_classification() {
    ...
    let weights: Vec<f32> = (0..3 * 16 * 3 * 3 * 3).map(|i| (i as f32) * 0.0001).collect();
    ...
    println!("   Video: {}x{}x{} ({}ch) → {}x{}x{} ({}ch)", 
             input_d, input_h, input_w, in_channels,
             8, 16, 16, out_channels);
}
