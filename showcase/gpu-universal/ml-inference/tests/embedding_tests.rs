...
async fn test_embedding_simple() {
    ...
    let result = executor
        .execute_embedding(
            &indices, &weight, batch_size, seq_length,
...
async fn test_embedding_batch() {
    ...
    println!("   {} sequences × {} tokens = {} total", batch_size, seq_length, batch_size * seq_length);
}
...
async fn test_embedding_bert_size() {
    ...
    println!("   Batch: {}, SeqLen: {}, EmbedDim: {}", batch_size, seq_length, embedding_dim);
}
