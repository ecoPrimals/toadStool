...
async fn test_transformer_attention_block() {
    gpu_test_resilient_async(async {
        ...
        let q = executor
            .execute_matmul(&input, &w_q, seq_len, d_model, d_model)
            .await
            .unwrap();
        let k = executor
            .execute_matmul(&input, &w_k, seq_len, d_model, d_model)
            .await
            .unwrap();
        let v = executor
            .execute_matmul(&input, &w_v, seq_len, d_model, d_model)
            .await
            .unwrap();
        ...
        let context = executor
            .execute_matmul(&attention_weights, &v, seq_len, d_model, seq_len)
            .await
            .unwrap();
        ...
        let projected = executor
            .execute_matmul(&context, &w_o, seq_len, d_model, d_model)
            .await
            .unwrap();
        ...
        let normalized1 = executor
            .execute_rms_norm(&residual1, seq_len, d_model, norm_config.clone())
            .await
            .unwrap();
        ...
        let ff_hidden = executor
            .execute_matmul(&normalized1, &w_ff1, seq_len, d_ff, d_model)
            .await
            .unwrap();
        ...
        let ff_output = executor
            .execute_matmul(&ff_activated, &w_ff2, seq_len, d_model, d_ff)
            .await
            .unwrap();
        ...
        let residual2 = executor
            .execute_add(&normalized1, &ff_output, 1.0)
            .await
            .unwrap();
        let final_output = executor
            .execute_rms_norm(&residual2, seq_len, d_model, norm_config)
            .await
            .unwrap();
...
async fn test_cnn_forward_pipeline() {
    ...
    let bn_out = executor
        .execute_batchnorm(&conv_out, batch, out_channels, conv_length, bn_config)
        .await
        .unwrap();
    ...
    let pool_out = executor
        .execute_max_pool_2d(&relu_out, batch, out_channels, height, width, pool_config)
        .await
        .unwrap();
    ...
    let gap_out = executor
        .execute_global_avg_pool(&hardswish_out, batch, out_channels, pool_h, pool_w)
        .await
        .unwrap();
...
async fn test_training_loop_integration() {
    gpu_test_resilient_async(async {
        ...
        for iter in 1..=3 {
            ...
            let logits = executor
                .execute_matmul(&input, &weights, batch_size, output_dim, input_dim)
                .await
                .unwrap();
            ...
            let loss = executor
                .execute_mse_loss(&predictions, &target, loss_config)
                .await
                .unwrap();
            println!("  Loss: {:.6}", loss);
            ...
            executor
                .execute_adam_step(&gradients, &mut weights, &mut m, &mut v, iter, adam_config)
                .await
                .unwrap();
...
async fn test_data_processing_pipeline() {
    gpu_test_resilient_async(async {
        ...
        let squared = executor
            .execute_map(&gathered, MapOp::Square)
            .await
            .unwrap();
        ...
        for (i, (&orig, &sq)) in gathered.iter().zip(squared.iter()).enumerate() {
            ...
            assert!(
                (sq - expected).abs() < 1e-5,
                "Square mismatch at {}: {} vs {}",
                i,
                sq,
                expected
            );
        ...
        let total = executor
            .execute_reduce(&squared, ReduceOp::Sum)
            .await
            .unwrap();
        ...
        let scan_result = executor
            .execute_scan(&squared, ScanOp::Sum, false)
            .await
            .unwrap();
        ...
        for (i, (&val, &scan_val)) in squared.iter().zip(scan_result.iter()).enumerate() {
            running_sum += val;
            assert!(
                (scan_val - running_sum).abs() < 1e-3,
                "Prefix sum mismatch at {}",
                i
            );
        }
        ...
        let scattered = executor
            .execute_scatter(&scan_result, &write_indices, size)
            .await
            .unwrap();
        ...
        let negated = executor
            .execute_map(&gathered, MapOp::Negate)
            .await
            .unwrap();
        ...
        for (i, (&orig, &final_val)) in gathered.iter().zip(abs_values.iter()).enumerate() {
            assert!(
                (final_val - orig).abs() < 1e-5,
                "Map chain mismatch at {}",
                i
            );
        }
...
async fn test_multi_loss_training() {
    gpu_test_resilient_async(async {
        ...
        let mse = executor
            .execute_mse_loss(&predictions, &targets, mse_config)
            .await
            .unwrap();
        ...
        let mae = executor
            .execute_mae_loss(&predictions, &targets, mae_config)
            .await
            .unwrap();
        ...
        let huber = executor
            .execute_huber_loss(&predictions, &targets, huber_config)
            .await
            .unwrap();
        ...
        let bce = executor
            .execute_bce_loss(&predictions, &targets, bce_config)
            .await
            .unwrap();
        ...
        let dice = executor
            .execute_dice_loss(&predictions, &targets, batch_size, num_classes, dice_config)
            .await
            .unwrap();
        ...
        let ce_result = executor
            .execute_cross_entropy(&predictions, &targets, batch_size, num_classes, ce_config)
            .await
            .unwrap();
        ...
        let focal = executor
            .execute_focal_loss(&predictions, &targets, focal_config)
            .await
            .unwrap();
