...
async fn test_2dispatch_layernorm_correctness() {
    ...
    let result_2dispatch = executor.execute_layernorm_2dispatch(&input, config.clone()).await.unwrap();
    ...
    for (i, (orig, two_d)) in result_original.iter().zip(result_2dispatch.iter()).enumerate() {
...
async fn test_2dispatch_layernorm_llama_scale() {
    ...
    println!("✅ 2-Dispatch LayerNorm handles LLaMA-scale correctly (4096 * 256 = 1,048,576 elements)");
...
async fn test_2dispatch_normalization_properties() {
    ...
    let result = executor.execute_layernorm_2dispatch(&input, config).await.unwrap();
