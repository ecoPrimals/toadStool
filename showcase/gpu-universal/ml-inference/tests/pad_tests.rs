...
async fn test_pad_zero_simple() {
    ...
    let result = executor.execute_pad(&input, 2, 2, 1, 1, 1, 1, 0.0).await.unwrap();
...
async fn test_pad_asymmetric() {
    ...
    let result = executor.execute_pad(&input, 3, 3, 1, 2, 0, 1, 0.0).await.unwrap();
...
async fn test_pad_custom_value() {
    ...
    let input = vec![1.0, 2.0, 3.0, 4.0];
    let result = executor.execute_pad(&input, 2, 2, 1, 1, 1, 1, -1.0).await.unwrap();
...
async fn test_pad_same_padding() {
    ...
    let result = executor.execute_pad(&input, input_h, input_w, 1, 1, 1, 1, 0.0).await.unwrap();
