...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "\nSynchronous Duration: {:.2}ms",
        sync_duration.as_secs_f64() * 1000.0
    );
    println!(
        "Concurrent Duration:  {:.2}ms",
        concurrent_duration.as_secs_f64() * 1000.0
    );
    println!(
        "\nActual Speedup: {:.2}x",
    ...
    println!(
        "4. Expected {:.1}x speedup for {} operations",
        stats.speedup_factor, stats.total_ops
    );
