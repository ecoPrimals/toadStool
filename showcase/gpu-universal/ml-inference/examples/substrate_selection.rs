...
async fn main() -> anyhow::Result<()> {
    ...
    let gpu_substrates: Vec<_> = all_substrates
        .iter()
    ...
    if gpu_substrates.len() >= 2 {
        println!(
            "Found {} GPUs - running cross-validation...\n",
            gpu_substrates.len()
        );
        ...
        println!(
            "Only {} GPU(s) available - skipping cross-validation",
            gpu_substrates.len()
        );
    }
    println!();
...
async fn create_executor_for_substrate(
    substrate: &ProcessingSubstrate,
) -> anyhow::Result<WgpuExecutor> {
    match substrate {
