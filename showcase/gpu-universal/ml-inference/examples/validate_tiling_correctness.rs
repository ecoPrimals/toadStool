...
async fn main() -> Result<()> {
    ...
    for &(m, k, n, label) in &test_sizes {
        ...
        if mismatches == 0 {
            println!("✅ PASS (max_diff={:.2e}, max_rel={:.2e})", max_diff, max_rel_error);
        } else {
            println!("❌ FAIL ({} mismatches, max_diff={:.2e}, max_rel={:.2e})", 
    ...
    for (m, k, n, label) in &test_sizes {
        ...
        if mismatches == 0 {
            ...
            println!("❌ FAIL ({} mismatches, max_diff={:.2e})", mismatches, max_diff);
            all_passed = false;
        }
    }
