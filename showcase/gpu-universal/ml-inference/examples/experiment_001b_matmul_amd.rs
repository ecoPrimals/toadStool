...
async fn main() -> anyhow::Result<()> {
    ...
    for &n in &matrix_sizes {
        ...
        let results: Vec<_> = all_results.iter().filter(|r| r.matrix_size == n).collect();
    ...
    for result in &all_results {
        csv.push_str(&format!(
            "{},{},{},{}\n",
            result.matrix_size, result.workgroup_size, result.mean_us, result.std_dev_us
        ));
    }
