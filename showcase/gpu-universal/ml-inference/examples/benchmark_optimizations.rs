...
async fn main() -> anyhow::Result<()> {
    ...
    println!("✅ Synchronous: {:.2}ms", sync_duration.as_secs_f64() * 1000.0);
    ...
    println!("✅ Naive MatMul: {:.2}ms", naive_duration.as_secs_f64() * 1000.0);

    // Tiled MatMul
    let start = Instant::now();
    let _r2 = executor.execute_matmul_tiled(&a, &b, size, size, size).await?;
    let tiled_duration = start.elapsed();
    println!("✅ Tiled MatMul: {:.2}ms", tiled_duration.as_secs_f64() * 1000.0);
    ...
    println!("Combined MatMul Impact     {:.2}x (async + tiling)", combined_matmul);
    println!("Combined LayerNorm Impact  {:.2}x (async + 2-dispatch)", combined_layernorm);
