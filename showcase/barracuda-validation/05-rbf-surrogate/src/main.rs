...
async fn main() -> Result<()> {
    ...
    let device = barracuda::device::WgpuDevice::new().await?;
    
    let x_train = Tensor::from_vec_on(
        x_train_data.clone(),
        vec![n_train, 1],
        device.clone()
    ).await?;
    
    let y_train = Tensor::from_vec_on(
        y_train_data.clone(),
        vec![n_train],
        device.clone()
    ).await?;
    ...
    println!("  Prediction time: {:.2} ms", pred_time.as_secs_f64() * 1000.0);
    println!("  Throughput: {:.0} predictions/sec", n_eval as f64 / pred_time.as_secs_f64());
